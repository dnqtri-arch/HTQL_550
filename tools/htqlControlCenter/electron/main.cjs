...
ipcMain.handle('api:fetchHtqlClientRegistry', async () => {
  ...
  return httpGetJson(`http://${host}:3001/api/htql-client-registry`, 8000)
})

ipcMain.handle('api:fetchHtqlMeta', async () => {
  const host = connectionState.host
  if (!host) return { ok: false, error: 'Smart Connect offline.' }
  return httpGetJson(`http://${host}:3001/api/htql-meta`, 8000)
})
...
ipcMain.handle('ssh:runServerUpdate', async (event, payload) => {
  ...
  const inner = [
    ...
    'echo ">>> [Update 7/7] Dọn rác: apt autoremove, autoclean, sao chép zip vào thư mục update server..."',
    'sudo DEBIAN_FRONTEND=noninteractive apt-get autoremove -y',
    'sudo apt-get autoclean',
    `sudo mkdir -p '${PATH_UPDATE_SERVER}' 2>/dev/null; sudo cp -f '${escapeSingleQuotes(remote)}' '${PATH_UPDATE_SERVER}/htql_server_update.zip' 2>/dev/null || true`,
