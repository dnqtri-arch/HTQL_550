...
export type PhuLucHopDongBanChungTuKyValue = 'tat-ca' | 'tuan-nay' | 'thang-nay' | 'quy-nay' | 'nam-nay'

/** Một file đính kèm chứng từ (HĐ) (lưu trong JSON — data URL + tên đã chuẩn hóa). */
export interface PhuLucHopDongBanChungTuAttachmentItem {
  name: string
  data: string
  saved_at?: string
  virtual_path?: string
...
export interface PhuLucHopDongBanChungTuRecord {
  id: string
  ...
  phieu_chi_attachments?: PhuLucHopDongBanChungTuAttachmentItem[]
  /** Hợp đồng bán gốc (đang thực hiện) mà phụ lục bổ sung phát sinh. */
...
export interface PhuLucHopDongBanChungTuChiTiet {
  id: string
...
export interface PhuLucHopDongBanChungTuCreatePayload {
  loai_khach_hang?: 'ca_nhan' | 'doanh_nghiep' | null
  ten_nguoi_lien_he?: string
  so_dien_thoai_lien_he?: string
  tinh_trang: string
...
export interface PhuLucHopDongBanChungTuFilter {
  ky: PhuLucHopDongBanChungTuKyValue
  tu: string
  den: string
}

/** Draft các dòng đang nhập trong form (chỉ lưu cột hiển thị). */
export type PhuLucHopDongBanChungTuDraftLine = Record<string, string> & { _dvtOptions?: string[] }
