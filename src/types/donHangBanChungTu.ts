...
export type DonHangBanChungTuKyValue = 'tat-ca' | 'tuan-nay' | 'thang-nay' | 'quy-nay' | 'nam-nay'

/** Một file đính kèm chứng từ (ĐHB) (lưu trong JSON — data URL + tên đã chuẩn hóa). */
export interface DonHangBanChungTuAttachmentItem {
  name: string
  data: string
  saved_at?: string
  virtual_path?: string
...
export interface DonHangBanChungTuRecord {
  id: string
  /** Loại khách (form Đơn hàng bán): cá nhân / doanh nghiệp — tối đa một giá trị. */
  ...
  /** Theo dõi báo giá nguồn khi lập ĐHB từ BG. */
  bao_gia_id?: string
...
export interface DonHangBanChungTuFilter {
  ky: DonHangBanChungTuKyValue
  tu: string
  den: string
}

/** Draft các dòng đang nhập trong form (chỉ lưu cột hiển thị). */
export type DonHangBanChungTuDraftLine = Record<string, string> & { _dvtOptions?: string[] }
