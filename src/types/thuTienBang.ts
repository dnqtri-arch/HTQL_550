/** Kiểu kỳ lọc danh sách báo giá (đồng bộ với KY_OPTIONS trong thuTienBangApi). */
export type ThuTienBangKyValue = 'tat-ca' | 'tuan-nay' | 'thang-nay' | 'quy-nay' | 'nam-nay'

/** Một file đính kèm chứng từ (lưu trong JSON — data URL + tên đã chuẩn hóa). */
export interface ThuTienBangAttachmentItem {
  name: string
  data: string
  saved_at?: string
  virtual_path?: string
...
export interface ThuTienBangRecord {
  id: string
  ...
  tinh_trang: string
  ngay_thu_tien_bang: string
  so_thu_tien_bang: string
...
export interface ThuTienBangFilter {
  ky: ThuTienBangKyValue
  tu: string
  den: string
}

/** Draft các dòng đang nhập trong form (chỉ lưu cột hiển thị). */
export type ThuTienBangDraftLine = Record<string, string> & { _dvtOptions?: string[] }
