/** Kiểu kỳ lọc danh sách hợp đồng mua (đồng bộ với KY_OPTIONS trong hopDongMuaApi). */
export type HopDongMuaKyValue = 'tat-ca' | 'tuan-nay' | 'thang-nay' | 'quy-nay' | 'nam-nay'

/** Một file đính kèm chứng từ mua hàng (lưu trong JSON đơn — data URL + tên đã chuẩn hóa). */
export interface HopDongMuaAttachmentItem {
  name: string
  data: string
  saved_at?: string
  virtual_path?: string
...
export interface HopDongMuaRecord {
  id: string
  tinh_trang: string
  ...
  /** `true` = có VAT (cột thuế trên lưới); `false` = không VAT. Bản ghi cũ thiếu → coi như `true`. */
  ...
  /** Hình thức giao hàng / nhập kho (form HĐM). `ca_hai` = chọn cả nhập kho và không nhập kho. */
  ...
  phieu_chi_attachments?: HopDongMuaAttachmentItem[]
}
...
export interface HopDongMuaCreatePayload {
  tinh_trang: string
  ngay_don_hang: string
  so_don_hang: string
  ngay_giao_hang: string | null
...
export interface HopDongMuaFilter {
  ky: HopDongMuaKyValue
  tu: string
  den: string
}

/** Draft các dòng đang nhập trong form (chỉ lưu cột hiển thị). */
export type HopDongMuaDraftLine = Record<string, string> & { _dvtOptions?: string[] }
