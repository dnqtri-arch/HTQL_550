...
import type { HopDongMuaChiTiet } from '../types/hopDongMua'
...
export function migrateHopDongMuaLinesToCoDonGia(prev: HopDongMuaGridLineRow[], vatTuList: VatTuHangHoaRecord[]): HopDongMuaGridLineRow[] {
  return prev.map((line) => {
    ...
    const row: HopDongMuaGridLineRow = {
      'Mã': line['Mã'] ?? '',
...
export function chiTietToLines(ct: HopDongMuaChiTiet[]): HopDongMuaGridLineRow[] {
  return ct.map((c) => {
...
export function lineDonGiaCell(line: HopDongMuaGridLineRow): string {
  return (line['Đơn giá'] ?? line['ĐG mua'] ?? '') as string
}

export function computeHopDongMuaFooterTotals(
  lines: HopDongMuaGridLineRow[],
...
export function formatHopDongMuaLineThanhTienDisplay(line: HopDongMuaGridLineRow): string {
  const thanhTien = parseFloatVN(lineDonGiaCell(line)) * parseFloatVN(line['Số lượng'] ?? '')
  return formatSoTienHienThi(thanhTien)
}

export function formatHopDongMuaLineTienThueDisplay(line: HopDongMuaGridLineRow): string {
  const thanhTien = parseFloatVN(lineDonGiaCell(line)) * parseFloatVN(line['Số lượng'] ?? '')
...
export function formatHopDongMuaLineTongTienDisplay(line: HopDongMuaGridLineRow): string {
  const thanhTien = parseFloatVN(lineDonGiaCell(line)) * parseFloatVN(line['Số lượng'] ?? '')
  const pt = parsePctThueGtgtFromLine(line['% thuế GTGT'] ?? '')
  const tienThue = pt != null ? (thanhTien * pt) / 100 : 0
...
export function enrichHopDongMuaGridLinesWithVthh(
  prev: HopDongMuaGridLineRow[],
  vatTuList: VatTuHangHoaRecord[]
): HopDongMuaGridLineRow[] {
  return prev.map((line) => {
    const ma = (line['Mã'] ?? '').trim()
    if (!ma) return line
