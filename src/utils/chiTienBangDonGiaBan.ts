/**
 * Đơn giá bán trên form Chi tiền (ChiTienBang) — tách biệt module Thu tiền.
 */
...
export function getBaseDonGiaBanTheoBacGiaChi(vthh: VatTuHangHoaRecord, soLuong: number): number {
...
export function getDonGiaBanChiTienBangLine(vthh: VatTuHangHoaRecord, dvtMa: string, soLuong: number): string {
  const dvtChinh = (vthh.dvt_chinh ?? '').trim()
  const sl = Number.isFinite(soLuong) && soLuong > 0 ? soLuong : 1
  const baseBan = getBaseDonGiaBanTheoBacGiaChi(vthh, sl)
  if (dvtMa === dvtChinh) {
...
export function migrateChiTienBangLinesToCoDonGia(
  ...
  return prev.map((line) => {
    ...
    if (vthh) {
      ...
      row['Đơn giá'] = getDonGiaBanChiTienBangLine(vthh, dvt, sl)
...
export function enrichChiTienBangGridLinesWithVthh(
  prev: DonHangMuaGridLineRow[],
  vatTuList: VatTuHangHoaRecord[]
): DonHangMuaGridLineRow[] {
  return prev.map((line) => {
    ...
    const donGiaStr = getDonGiaBanChiTienBangLine(vthh, dvt, soLuong > 0 ? soLuong : 1)
