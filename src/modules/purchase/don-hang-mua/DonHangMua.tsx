...
import {
  ...
  type KyValue,
  donHangMuaBuildCreatePayloadFromRecord,
...
import { HTQL_MUA_HANG_TAB_EVENT } from '../mua-hang/muaHangTabEvent'
...
import { nhaCungCapGetAll } from '../mua-hang/nhaCungCapApi'
import { formatNumberDisplay, formatSoThapPhan } from '../../../utils/numberFormat'
import { exportCsv } from '../../../utils/exportCsv'
import styles from './DonHangMua.module.css'

function DonHangMuaAttachmentListPanel({ don }: { don: DonHangMuaRecord | null }) {
  ...
  const files: DonHangMuaAttachmentItem[] = don?.attachments?.length ? don.attachments : []
  const soDon = (don?.so_don_hang ?? '').trim() || 'DHM'
...
function DonHangMuaContent() {
  const api = useDonHangMuaApi()
  const [filter, setFilter] = useState<DonHangMuaFilter>(api.getDefaultFilter)
  const [danhSach, setDanhSach] = useState<DonHangMuaRecord[]>([])
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [chiTiet, setChiTiet] = useState<DonHangMuaChiTiet[]>([])
  const [dropdownXuatKhau, setDropdownXuatKhau] = useState(false)
  const [showForm, setShowForm] = useState(false)
  const [viewDon, setViewDon] = useState<DonHangMuaRecord | null>(null)
  /** Tăng mỗi lần bấm Thêm để form remount → reset tab và nội dung phía dưới. */
  const [addFormKey, setAddFormKey] = useState(0)
  const [phucHoiTarget, setPhucHoiTarget] = useState<DonHangMuaRecord | null>(null)
  ...
  const [contextMenu, setContextMenu] = useState<{ open: boolean; x: number; y: number; row: DonHangMuaRecord | null }>({ open: false, x: 0, y: 0, row: null })
  ...
  const thucHienXoaDon = (row: DonHangMuaRecord) => {
    api.delete(row.id)
    loadData()
    if (selectedId === row.id) setSelectedId(null)
  }

  const columnsDon: DataGridColumn<DonHangMuaRecord>[] = [
    { key: 'so_don_hang', label: 'Mã ĐHM', width: 56 },
  ...
  const columnsChiTietCodongia: DataGridColumn<DonHangMuaChiTiet>[] = [
    { key: 'stt' as keyof DonHangMuaChiTiet, label: 'STT', width: COL_STT, align: 'center', renderCell: (_v, _r, idx) => idx != null ? String(idx + 1) : '' },
    ...
    { key: 'tong_tien' as keyof DonHangMuaChiTiet, label: 'Tổng tiền', width: 100, align: 'right', renderCell: (_v, row) => formatNumberDisplay((row.thanh_tien ?? 0) + (row.tien_thue_gtgt ?? 0), 0) },
  ...
  const columnsChiTietKhongdongia: DataGridColumn<DonHangMuaChiTiet>[] = [
    { key: 'stt' as keyof DonHangMuaChiTiet, label: 'STT', width: COL_STT, align: 'center', renderCell: (_v, _r, idx) => idx != null ? String(idx + 1) : '' },
    { key: 'ma_hang', label: 'Mã VTHH', width: COL_MA },
    { key: 'ten_hang', label: 'Tên VTHH', width: COL_TEN, renderCell: renderCellTenVthhTooltip },
    { key: 'dvt', label: 'ĐVT', width: COL_DVT, renderCell: (v) => dvtHienThiLabel(v as string, dvtList) },
  ...
  const sortedDanhSach = [...danhSach].sort((a, b) => {
    ...
    for (const s of effectiveSort) {
      const key = s.key as keyof DonHangMuaRecord
      const va = a[key]
      const vb = b[key]
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const dataRows = danhSach.map((d) => [
                    ...columnsDon.map((col) => {
                      const v = d[col.key as keyof DonHangMuaRecord]
                ...
                onClick={() => {
                  ...
                  const dataRows = chiTiet.map((row, idx) =>
                    columnsChiTiet.map((col) => {
                      ...
                      const v = row[col.key as keyof DonHangMuaChiTiet]
                  ...
                  onClick={() => {
                    ...
                    window.dispatchEvent(
                      new CustomEvent(HTQL_MUA_HANG_TAB_EVENT, {
                        detail: { tab: 'nhan-vat-tu-hang-hoa', nhanHangTuDonMua: { don: row, chiTiet: chiTietTuDon } },
                      })
                    )
                  }}
                >
          ...
          <>
            ...
            <button
              ...
              onClick={() => {
                ...
                const base = donHangMuaBuildCreatePayloadFromRecord(phucHoiTarget, ct)
          ...
          <>
            ...
            <button
              ...
              onClick={() => {
                ...
                const base = donHangMuaBuildCreatePayloadFromRecord(huyBoModalRow, ct)
      ...
      >
        {huyBoModalRow ? (
          <p style={{ margin: 0, fontSize: 12, color: 'var(--text-primary)', lineHeight: 1.5 }}>
            Hủy bỏ đơn hàng mua <strong>{huyBoModalRow.so_don_hang}</strong>
...
export function DonHangMua() {
  return (
    <DonHangMuaApiProvider api={apiDon}>
      <DonHangMuaContent />
