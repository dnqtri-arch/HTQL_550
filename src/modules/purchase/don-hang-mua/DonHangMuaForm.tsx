...
import { loadDieuKhoanThanhToan, saveDieuKhoanThanhToan, nhaCungCapGetAll, type DieuKhoanThanhToanItem } from '../mua-hang/nhaCungCapApi'
import { NhaCungCap } from '../mua-hang/NhaCungCap'
import type { NhaCungCapRecord } from '../mua-hang/nhaCungCapApi'
...
} from '../../inventory/nhan-vat-tu-hang-hoa/nhanVatTuHangHoaApi'
import type { NhanVatTuHangHoaApi } from '../../inventory/nhan-vat-tu-hang-hoa/NhanVatTuHangHoaApiContext'
import { NhanVatTuHangHoaApiProvider } from '../../inventory/nhan-vat-tu-hang-hoa/NhanVatTuHangHoaApiContext'
import { NhanVatTuHangHoaFormModal } from '../../inventory/nhan-vat-tu-hang-hoa/NhanVatTuHangHoaFormModal'
...
import { ThemDieuKhoanThanhToanModal } from '../mua-hang/ThemDieuKhoanThanhToanModal'
import { ThemKhoModal } from '../../inventory/kho/ThemKhoModal'
import { hinhThucGetAll, type HinhThucRecord } from '../mua-hang/hinhThucApi'
import { ThemHinhThucModal } from '../mua-hang/ThemHinhThucModal'
import { suggestAddressVietnam } from '../mua-hang/addressAutocompleteApi'
...
/** Thứ tự ô Địa điểm GH: luôn **kho nhập** trước **tên công trình** (cùng hàng hai trường như UI). */
...
export function DonHangMuaForm({ onClose, onSaved, onHeaderPointerDown, dragging, readOnly = false, initialDon, initialChiTiet, prefillDon, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: formTitleProp, soDonLabel: soDonLabelProp, doiChieuSource, phieuNhanTuDonHangMua = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false }: DonHangMuaFormProps) {
  ...
  const donDaNhanHangXem = readOnly && initialDon != null && initialDon.tinh_trang === TINH_TRANG_DON_HANG_MUA_DA_NHAN_HANG
  ...
  const [tieuDeNguonDhm] = useState(() => {
    const p = prefillDon as { _tieu_de_nguon_dhm?: string } | null | undefined
    return (p?._tieu_de_nguon_dhm ?? '').trim()
  })
  ...
  useEffect(() => {
    ...
    if (prefillDon) {
      ...
      if (prefillDon.so_chung_tu_cukcuk != null) setThamChieu(prefillDon.so_chung_tu_cukcuk)
  ...
  /** Đồng bộ id + mã NCC từ danh bạ khi có tên NCC (đơn mở sẵn, prefill, hoặc sau khi nạp nccList). */
  useEffect(() => {
    const name = (initialDon?.nha_cung_cap ?? prefillDon?.nha_cung_cap ?? '').trim()
    if (!name || nccList.length === 0) return
    const found = nccList.find((n) => (n.ten_ncc || '').trim() === name)
    if (found != null) {
      setNhaCungCapId(found.id)
      setNhaCungCapMa((found.ma_ncc ?? '').trim())
    }
  }, [initialDon?.nha_cung_cap, prefillDon?.nha_cung_cap, nccList])
  ...
  const buildPayload = (): DonHangMuaCreatePayload => {
    ...
    const payload: DonHangMuaCreatePayload = {
      ...
      ngay_giao_hang: ngayGiaoHang ? toIsoDateTime(ngayGiaoHang) : null,
  ...
  const validateBeforeSave = (): boolean => {
    ...
    if (!thamChieu.trim()) {
      return fail('doi_chieu', 'Vui lòng nhập Tiêu đề.', refTieuDeInput.current)
    }
    ...
    if (!ngayGiaoHang) {
      return fail('tg_nhan', 'Vui lòng chọn TG nhận hàng.', refTgNhanDmhInput.current)
    }
    if (!soDonHang.trim()) {
      return fail('so_don', `${soDonLabel} không được để trống.`, null)
    }
    ...
    if (ngayGiaoHang && ngayDonHang && ngayGiaoHang.getTime() < ngayDonHang.getTime()) {
      return fail('tg_nhan', 'TG nhận hàng không được nhỏ hơn TG tạo.', refTgNhanDmhInput.current)
    }
    setLoi('')
    return true
  }
  ...
  const tinhTrangSelectOptions = useMemo(() => {
    const base = TINH_TRANG_OPTIONS_FORM.map((o) => ({ value: o.value, label: o.label }))
  ...
  const currentColumns = (coBangCoDonGiaTuDau ? mauDhmCoDonGiaDisplay : mauHienTai === 'codongia' ? mauDhmCoDonGiaDisplay : mauDhmKhongDonGiaDisplay) as readonly string[]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, background: 'var(--bg-primary)' }}>
      <div style={titleBarWrap}>
        ...
        >
          {phieuNhanTuDonHangMua
            ? (tieuDeNguonDhm || thamChieu || 'Phiếu nhận')
      ...
      <div style={toolbarWrap}>
        ...
        <div style={toolbarDinhKemWrap}>
          <button
            ...
            onClick={() => setShowDinhKemModal((o) => !o)}
          >
      ...
      {doiChieuSource === 'don_mua_hang' && (
        ...
        >
          <div style={{ maxHeight: 320, overflowY: 'auto' }}>
            {donHangMuaListForDoiChieuNhanHang().map((dmh) => {
              ...
              return (
                <div
                  ...
                  onMouseDown={(e) => {
                    ...
                    setHinhThucSelectedIds(deriveHinhThucSelectedIdsFromRecord(dmh as DonHangMuaRecord))
                    ...
                    setDiaDiemGiaoHangList((() => { const s = (dmh.dia_diem_giao_hang ?? '').trim(); return s ? s.split(/\r?\n/).map((x) => x.trim()).filter(Boolean) : [''] })())
            ...
            {donHangMuaListForDoiChieuNhanHang().length === 0 && (
              <div style={{ padding: 16, fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>Không có đơn mua hàng đủ điều kiện đối chiếu.</div>
            )}
          </div>
        </Modal>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, padding: '8px 12px' }}>
          ...
          >
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, width: '100%' }}>
              {(() => {
                ...
                const chungTuFieldW = Math.min(Math.max(chungTuMuaHienThi.length * 7 + LOOKUP_CHEVRON_WIDTH + 36, 160), 480)
                const chungTuInputStyle: React.CSSProperties = {
                  ...inputStyle,
                  ...lookupInputWithChevronStyle,
                  width: chungTuFieldW,
                  minWidth: chungTuFieldW,
                  maxWidth: chungTuFieldW,
              ...
              <div style={{ flex: '0 1 auto', display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                ...
                >
                  <div
                    style={{ position: 'relative', display: 'flex', height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT, cursor: hinhThucLocked ? 'default' : 'pointer' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 16, marginBottom: 2, flexWrap: 'wrap', flex: '1 1 0', minHeight: 0, alignItems: 'stretch' }}>
          {(() => {
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0, position: 'relative', display: 'flex', height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT }}>
                      <input
                        ...
                        value={thamChieu}
                        onChange={(e) => {
                          setThamChieu(e.target.value.toUpperCase())
                          if (valErrKey === 'doi_chieu') setValErrKey(null)
                        }}
                        readOnly={false}
                        disabled={false}
                      ...
                      />
                      {doiChieuSource === 'don_mua_hang' && !effectiveReadOnly ? (
                        <span
                          style={{ ...lookupChevronOverlayStyle, pointerEvents: 'auto', cursor: 'pointer' }}
                          role="button"
                          tabIndex={-1}
                          title="Chọn đơn mua hàng đối chiếu"
                          aria-label="Chọn đơn mua hàng đối chiếu"
                          onMouseDown={(e) => {
                            e.preventDefault()
                            setShowTimDonMuaHangPopup(true)
                          }}
                        >
            ...
            <div style={{ ...fieldRow, alignItems: 'center', width: '100%', minWidth: 0 }}>
            ...
            <div style={{ ...fieldRow, flexWrap: 'wrap', gap: 8, width: '100%', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <input type="text" inputMode="numeric" className="htql-no-spinner" style={{ ...inputStyle, width: 52, minWidth: 52, maxWidth: 52, boxSizing: 'border-box' }} value={soNgayDuocNo} onChange={(e) => setSoNgayDuocNo(formatSoNguyenInput(e.target.value))} readOnly={effectiveReadOnly} disabled={effectiveReadOnly} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto', minWidth: 0 }}>
                ...
                <input
                  style={{ ...inputStyle, width: 112, minWidth: 112, flex: '0 0 auto', cursor: 'default', color: hanThanhToanDayDu ? 'var(--text-primary)' : 'var(--text-muted)' }}
            ...
            {(hasKhongNhapKho || hasNhapKho) && (
              <div style={{ ...fieldRow, flexWrap: 'wrap', gap: 8 }}>
                {(() => {
                  const order = buildHinhThucDiaDiemOrder(hinhThucSelectedIds, hinhThucList)
                    ...
                    type === 'ten_cong_trinh' ? (
                      ...
                      <div key={`ht-kho-${idxHt}`} style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                        ...
                        <div
                          ref={refKhoWrap}
                          style={{
                            flex: 1, minWidth: 0, display: 'flex', gap: 2, alignItems: 'center', position: 'relative',
                            height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT,
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {(() => {
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, minWidth: 0 }}>
                    <label style={{ ...labelStyle, minWidth: 90, paddingTop: 4 }}>Địa điểm GH {i + 1}</label>
                    <div
                ...
                {!effectiveReadOnly && (
                  <button
                    ...
                    style={{ alignSelf: 'flex-start', marginLeft: 98, display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', fontSize: 10, background: 'var(--accent)', color: 'var(--accent-text)', border: 'none', borderRadius: 4, cursor: 'pointer' }}
                  >
            ...
            return phieuNhanTuDonHangMua ? (
              <div style={{ flex: 1, minWidth: 560, display: 'flex', flexDirection: 'column', alignSelf: 'stretch', marginBottom: 8, minHeight: 0 }}>
                ...
                >
                  <div
                    role="tabpanel"
                    hidden={phieuNhanTabChung !== 'phieu-nhap'}
                    style={{
                      display: phieuNhanTabChung === 'phieu-nhap' ? 'flex' : 'none',
                      flexDirection: 'column',
                      flex: '0 0 auto',
                      overflow: 'visible',
                      padding: '8px 10px',
                  ...
                  <div
                    role="tabpanel"
                    hidden={phieuNhanTabChung !== 'phieu-chi'}
                    style={{
                      display: phieuNhanTabChung === 'phieu-chi' ? 'flex' : 'none',
                      flex: '0 0 auto',
                      overflow: 'auto',
                      padding: 12,
                      fontSize: 11,
                  ...
                  >
                    Chưa có nội dung phiếu chi.
                  </div>
                  <div
                    role="tabpanel"
                    hidden={phieuNhanTabChung !== 'hoa-don'}
                    style={{
                      display: phieuNhanTabChung === 'hoa-don' ? 'flex' : 'none',
                      flex: '0 0 auto',
                      overflow: 'auto',
                      padding: 12,
                      fontSize: 11,
                  ...
                  >
                    Chưa có nội dung hóa đơn.
                  </div>
                </div>
              </div>
            ) : (
              <div style={{ ...groupBox, flex: 1, minWidth: 560, display: 'flex', flexDirection: 'column', alignSelf: 'stretch' }}>
          ...
          <div style={{ ...groupBox, width: 268, minWidth: 268, flexShrink: 0, display: 'flex', flexDirection: 'column', alignSelf: 'stretch' }}>
            <div style={groupBoxTitle}>Đơn hàng</div>
            <div style={fieldRow}>
              <label style={{ ...labelStyle, minWidth: 82 }}>TG tạo</label>
            ...
            <div style={fieldRow}>
              ...
              <div style={{ flex: '0 0 auto' }}>
                <select className="htql-don-hang-select" style={{ ...inputStyle, width: 120, minWidth: 120, paddingRight: 6, boxSizing: 'border-box' }} value={tinhTrang} onChange={(e) => setTinhTrang(e.target.value)} disabled={effectiveReadOnly}>
                  {tinhTrangSelectOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
            </div>
            <div style={fieldRow}>
              <label style={{ ...labelStyle, minWidth: 82 }}>TG nhận hàng</label>
              <div
                style={{
                  width: 140, minWidth: 140, flex: '0 0 auto', marginLeft: -2,
                  borderRadius: 4, boxSizing: 'border-box',
              ...
              >
                <DatePicker
                  ...
                  className="htql-datepicker-inline htql-datepicker-tg-nhan-input"
                  calendarClassName="htql-datepicker-tg-nhan"
                  customInput={
            ...
            <div style={fieldRow}>
              <label style={{ ...labelStyle, minWidth: 82 }}>NV mua hàng</label>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 2 }}>
                <div style={{ flex: 1, minWidth: 0, position: 'relative', display: 'flex' }}>
                  <input style={{ ...inputStyle, ...lookupInputWithChevronStyle, width: '100%', minWidth: 0 }} value={nvMuaHang} onChange={(e) => setNvMuaHang(e.target.value)} readOnly={effectiveReadOnly} disabled={effectiveReadOnly} />
                  <span style={lookupChevronOverlayStyle} aria-hidden><ChevronDown size={12} style={{ color: 'var(--accent-text)' }} /></span>
                </div>
                {!effectiveReadOnly && (
        ...
        >
          ...
          >
            <div style={{ flex: 1, minHeight: 0, minWidth: 0, overflow: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, tableLayout: 'fixed', minWidth: tableMinWidth }}>
                <colgroup>
                ...
                <thead>
                  <tr>
                    <th style={{ ...gridThStyle, textAlign: 'center', width: COL_WIDTH_ACTION, minWidth: COL_WIDTH_ACTION, maxWidth: COL_WIDTH_ACTION, borderRight: '0.5px solid var(--border)' }} title="Xóa dòng" />
                    <th style={{ ...gridThStyle, textAlign: 'center', width: COL_WIDTH_STT, minWidth: COL_WIDTH_STT, maxWidth: COL_WIDTH_STT }}>STT</th>
                    {currentColumns.map((col) => (
                      <th key={col} style={{ ...gridThStyle, ...colWidthStyle(col, col === 'Ghi chú' ? ghiChuFill : undefined), paddingLeft: 5, ...(col === currentColumns[currentColumns.length - 1] ? { borderRight: 'none' } : {}) }}>{gridColumnHeaderLabel(col)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {lines.length === 0 ? (
                    ...
                    lines.map((line, idx) => (
                      <tr key={idx}>
                        <td style={{ ...gridTdStyle, textAlign: 'center', whiteSpace: 'nowrap', width: COL_WIDTH_ACTION, minWidth: COL_WIDTH_ACTION, maxWidth: COL_WIDTH_ACTION, borderRight: '0.5px solid var(--border)' }}>
                        ...
                        <td style={{ ...gridTdStyle, textAlign: 'center', whiteSpace: 'nowrap', width: COL_WIDTH_STT, minWidth: COL_WIDTH_STT, maxWidth: COL_WIDTH_STT }}>{idx + 1}</td>
                        {currentColumns.map((col) => {
                          const showColAsReadOnly = chiTietReadOnly || (chiTietGridLocked && col !== 'Số lượng')
                          return (
                          <td key={col} style={{ ...gridTdStyle, ...colWidthStyle(col, col === 'Ghi chú' ? ghiChuFill : undefined), whiteSpace: col === 'Ghi chú' ? 'normal' : 'nowrap', ...(col === currentColumns[currentColumns.length - 1] ? { borderRight: 'none' } : {}) }} {...(col === 'Mã' && !showColAsReadOnly ? { 'data-vthh-cell': true } : {})}>
                            {showColAsReadOnly ? (
                              <input
                                readOnly
                                tabIndex={-1}
                                className="misa-input-solo"
                                style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22, cursor: 'default' }}
                                value={
                                  col === 'ĐVT'
                              ...
                              <div style={{ position: 'relative', width: '100%', display: 'inline-block' }}>
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const nextRow = { ...prev, [col]: ma, 'Tên VTHH': cleared ? '' : (item ? item.ten : prev['Tên VTHH']), 'ĐVT': cleared ? '' : (item ? (item.dvt_chinh ?? '') : prev['ĐVT']) } as unknown as DonHangMuaGridLineRow
                                    const isCodongia = initialDon != null || mauHienTai === 'codongia'
                                    if (cleared || !item) {
              ...
              {!chiTietReadOnly && !chiTietGridLocked && (
                <div style={{ padding: '4px 8px', fontSize: 11, borderTop: '0.5px solid var(--border)', background: 'var(--bg-tab)', display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
                  <div style={{ marginLeft: OFFSET_TRAI_COT_MA_VTHH, display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
                    <button type="button" onClick={() => { setLines([...lines, currentColumns.reduce<Record<string, string>>((acc, c) => ({ ...acc, [c]: '' }), {}) as unknown as DonHangMuaGridLineRow]) }} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 6px', fontSize: 10, background: 'var(--accent)', color: 'var(--accent-text)', border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>
      ...
      {showDonMuaHangPopup && viewDonHangMuaRecord && doiChieuSource === 'don_mua_hang' && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => { setShowDonMuaHangPopup(false); setViewDonHangMuaRecord(null) }}>
          <div
            style={{
