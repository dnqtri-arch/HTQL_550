...
/** 2026/DHM/3 → 2026_dhm_3 */
export function parseSoDonHangForAttachmentFile(soDon: string): string {
  const t = soDon.trim()
  const m = t.match(/^(\d{4})\s*\/\s*DHM\s*\/\s*(\d+)$/i)
  if (m) return `${m[1]}_dhm_${m[2]}`
  const s = t.replace(/[\\/]+/g, '_').replace(/\s+/g, '_').replace(/_+/g, '_').toLowerCase()
  return s || 'dhm'
}

/**
 * Phần thư mục/tên file cho NCC: **chỉ** `ma_ncc` (chuẩn hóa chữ thường, bỏ khoảng trắng).
 * Không dùng tên NCC làm slug — chưa có mã thì `ncc_unknown`.
...
export function formatTgNhanForAttachmentFile(d: Date): string {
  const h = d.getHours()
...
/**
 * `maNccPathPart`: kết quả `partMccForPath(ma_ncc)` — chỉ mã NCC; chưa có mã → `ncc_unknown`.
 * Thời gian trong tên file: TG nhận hàng nếu có, không thì TG tạo đơn, không thì thời điểm hiện tại (không dùng 01/01/1970).
 */
export function buildDhmAttachmentBaseName(
  soDon: string,
  maNccPathPart: string,
  ngayGiao: Date | null,
  ngayDonHang: Date | null,
  index: number
): string {
  const dhmPart = parseSoDonHangForAttachmentFile(soDon)
  const nccPart = (maNccPathPart || 'ncc_unknown').trim().toLowerCase()
  const tgNgay = ngayGiao ?? ngayDonHang ?? new Date()
  const tgPart = formatTgNhanForAttachmentFile(tgNgay)
  return `${dhmPart}_${nccPart}_${tgPart}_${index}`
}

/** Thư mục module (slug) — đồng bộ quy tắc lưu đính kèm ĐHM */
export const DHM_ATTACHMENT_MODULE_FOLDER = 'don_hang_mua'
...
export function buildDhmAttachmentFolderVirtualPath(soDonHang: string, maNccPathPart: string): string {
  ...
  return `${DHM_ATTACHMENT_MODULE_FOLDER}/${ncc}/${dhm}`
}

export function buildDhmAttachmentFullVirtualPath(soDonHang: string, maNccPathPart: string, fileName: string): string {
  const base = fileName.replace(/^\/+/, '')
  return `${buildDhmAttachmentFolderVirtualPath(soDonHang, maNccPathPart)}/${base}`
}

/** Hậu tố chuẩn do `buildDhmAttachmentBaseName`: _HH_mm_dd_MM_yyyy_index */
const RE_DHM_ATT_NAME_TAIL = /_(\d{2})_(\d{2})_(\d{2})_(\d{2})_(\d{4})_(\d+)$/

/**
 * Khi đổi Mã ĐHM / NCC sau khi đã đính kèm: đổi lại `name` cho khớp (trước đó có thể là `…_ncc_unknown_…`).
 * Giữ nguyên phần thời gian + chỉ số từ tên cũ; nếu không khớp định dạng hệ thống thì giữ nguyên tên.
 */
export function rebuildDhmAttachmentStoredFileName(
  currentName: string,
  soDonHang: string,
  maNccPathPart: string
): string {
  ...
  const m = base.match(RE_DHM_ATT_NAME_TAIL)
  if (!m) return raw
  const tgPart = `${m[1]}_${m[2]}_${m[3]}_${m[4]}_${m[5]}`
  const index = m[6]
  const dhmPart = parseSoDonHangForAttachmentFile(soDonHang)
  const nccPart = (maNccPathPart || 'ncc_unknown').trim().toLowerCase()
  return `${dhmPart}_${nccPart}_${tgPart}_${index}${ext}`
}

/**
 * Trước khi lưu / khi đổi Mã ĐHM hoặc mã NCC: gán lại `name` (nếu đúng pattern) và `virtual_path` theo quy tắc hiện tại
 * (don_hang_mua / mã_ncc / mã_đhm / tên_file), tránh lệch khi user đính kèm trước rồi mới nhập NCC/số đơn.
 */
export function chuanHoaDuongDanDinhKemDonHangMua(
  items: DonHangMuaAttachmentItem[],
  soDonHang: string,
  maNccPathPart: string
): DonHangMuaAttachmentItem[] {
  return items.map((a) => {
    const name = rebuildDhmAttachmentStoredFileName(a.name, soDonHang, maNccPathPart)
    return {
      ...a,
      name,
      virtual_path: buildDhmAttachmentFullVirtualPath(soDonHang, maNccPathPart, name),
    }
  })
}
...
/** Đường dẫn hiển thị trong UI: chỉ tới thư mục; luôn tính theo Mã ĐHM + phần NCC hiện tại (không tin virtual_path cũ). */
export function duongDanHienThi(item: DonHangMuaAttachmentItem, soDonHang: string, maNccPathPart: string): string {
  const full = buildDhmAttachmentFullVirtualPath(soDonHang, maNccPathPart, item.name)
  const thuMuc = duongDanThuMucLuuTru(full)
  if (thuMuc) return thuMuc
  return `${buildDhmAttachmentFolderVirtualPath(soDonHang, maNccPathPart)}/`
}
...
export type DonHangMuaDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: DonHangMuaAttachmentItem[]
  onChange: (next: DonHangMuaAttachmentItem[]) => void
  readOnly?: boolean
  soDonHang: string
  /** `partMccForPath(ma_ncc)` — thư mục + tên file (chỉ mã NCC) */
  maNccPathPart: string
  ngayGiaoHang: Date | null
  /** Fallback thời gian trong tên file khi chưa chọn TG nhận hàng */
  ngayDonHang: Date | null
}
...
const DHM_DINH_KEM_TOI_DA_TEP = 10
const DHM_DINH_KEM_TOI_DA_BYTES = 5 * 1024 * 1024
const PANEL_Z = 1060
...
export function DonHangMuaDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if ((t as Element).closest?.('[data-dhm-dinh-kem-viewer]')) return
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const slotConLai = DHM_DINH_KEM_TOI_DA_TEP - attachments.length
      if (slotConLai <= 0) {
        baoLoiDinhKem('Đã đủ 10 file đính kèm. Xóa bớt file nếu cần thêm mới.')
        return
      }
      if (list.length > DHM_DINH_KEM_TOI_DA_TEP) {
        baoLoiDinhKem(
          `Bạn đã chọn ${list.length} file — mỗi lần chỉ được chọn tối đa ${DHM_DINH_KEM_TOI_DA_TEP} file.\n\nKhông xử lý hay tải file nào.`
        )
        return
      }
      ...
      for (const file of list) {
        ...
        else if (file.size >= DHM_DINH_KEM_TOI_DA_BYTES)
      ...
      const hangMoi: DonHangMuaAttachmentItem[] = []
      for (let i = 0; i < listXuLy.length; i++) {
        const file = listXuLy[i]
        const chiSo = attachments.length + hangMoi.length + 1
        const tenGoc = buildDhmAttachmentBaseName(soDonHang, maNccPathPart, ngayGiaoHang, ngayDonHang, chiSo)
        setTienTrinh({
      ...
      const msg =
        n === 1
          ? `Thêm 1 file đính kèm vào đơn?\n\n${hangMoi[0].name}`
          : `Thêm ${n} file đính kèm vào đơn?`
      ...
      const withMeta: DonHangMuaAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
        virtual_path: buildDhmAttachmentFullVirtualPath(soDonHang, maNccPathPart, x.name),
      }))
      onChange([...attachments, ...withMeta])
      setTienTrinh({ phanTram: 100, chu: 'Đã thêm vào đơn' })
      setTimeout(() => setTienTrinh(null), 600)
    },
    [attachments, onChange, readOnly, soDonHang, maNccPathPart, ngayGiaoHang, ngayDonHang, baoLoiDinhKem, xacNhanDefer]
  )
      ...
      <div style={{ padding: '8px 10px', overflowY: 'auto', flex: 1, minHeight: 0 }}>
        {!readOnly && (
          ...
          >
            ...
            <button
              type="button"
              disabled={attachments.length >= DHM_DINH_KEM_TOI_DA_TEP}
              title={attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'Đã đủ 10 file đính kèm' : undefined}
              style={{
                ...nutPhu,
                flexShrink: 0,
                background: attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'var(--border)' : 'var(--accent)',
                color: attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'var(--text-muted)' : 'var(--accent-text)',
                borderColor: attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'var(--border-strong)' : 'var(--accent)',
                boxShadow:
                  attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'none' : '0 1px 3px rgba(230, 126, 34, 0.35)',
                cursor: attachments.length >= DHM_DINH_KEM_TOI_DA_TEP ? 'not-allowed' : 'pointer',
              }}
              onClick={() => {
                if (attachments.length >= DHM_DINH_KEM_TOI_DA_TEP) return
          ...
          <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
            ...
            {viewerKind === 'other' && (
              <div style={{ color: '#ccc', fontSize: 12, textAlign: 'center', padding: 24 }}>
                ...
                <p>Không xem trước DOCX trực tiếp. File đã lưu trong dữ liệu đơn hàng.</p>
              </div>
            )}
          </div>
          <button
