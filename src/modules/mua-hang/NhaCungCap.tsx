...
const COT: { key: keyof NhaCungCapRecord | string; label: string; width?: string; filterable?: boolean; renderCell?: (v: unknown, r: NhaCungCapRecord) => React.ReactNode }[] = [
  { key: 'ma_ncc', label: 'Mã nhà cung cấp', width: '12%' },
  { key: 'ten_ncc', label: 'Tên nhà cung cấp', width: '20%' },
  { key: 'dia_chi', label: 'Địa chỉ', width: '22%' },
  { key: 'nhom_kh_ncc', label: 'Nhóm KH, NCC', width: '12%' },
  { key: 'ma_so_thue', label: 'Mã số thuế', width: '11%' },
  { key: 'dien_thoai', label: 'Điện thoại', width: '11%' },
...
const emptyBankRow: TaiKhoanNganHangItem = { so_tai_khoan: '', ten_ngan_hang: '', chi_nhanh: '', tinh_tp_ngan_hang: '' }
...
export function NhaCungCap({ onQuayLai }: { onQuayLai?: () => void }) {
  ...
  const danhSachLoc = danhSach.filter((r) => {
    ...
    if (nhomLoc && (r.nhom_kh_ncc ?? '') !== nhomLoc) return false
    return true
  })
  ...
  const validateForm = () => {
    ...
    if (!ten) {
      setLoi('Tên nhà cung cấp là bắt buộc.')
      return null
    }
    if (!ma) {
      setLoi('Mã nhà cung cấp là bắt buộc.')
      return null
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      ...
      <div style={{ ...panelChiTiet, flexShrink: 0, marginTop: 'auto', height: 200, maxHeight: 220, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', marginBottom: 8, flexShrink: 0, minHeight: 28, padding: '6px 8px 0' }}>
          <button
            type="button"
            style={{
              display: 'inline-flex',
      ...
      <Modal
        open={!!modalOpen}
        onClose={dongModal}
        title={modalOpen === 'add' ? 'Thêm Nhà cung cấp' : modalOpen === 'clone' ? 'Nhân bản nhà cung cấp' : 'Sửa nhà cung cấp'}
        size="full"
        footer={null}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 400 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
            <button
              ...
              style={{
                ...
                color: formTab === 'thong_tin_chung' ? '#0d0d0d' : 'var(--text-muted)',
                border: '1px solid var(--border)',
                borderRadius: 4,
                cursor: 'pointer',
              }}
            ...
            <button
              ...
              style={{
                ...
                color: formTab === 'khac' ? '#0d0d0d' : 'var(--text-muted)',
                border: '1px solid var(--border)',
                borderRadius: 4,
                cursor: 'pointer',
              }}
          ...
          {formTab === 'thong_tin_chung' && form.loai_ncc === 'to_chuc' && (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8, fontSize: 11 }}>
                ...
                {/* Tên - full */}
                <div style={fieldRow}>
                  <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Tên (*)</label>
                  <input style={{ ...inputStyle, flex: 1 }} value={form.ten_ncc} onChange={(e) => setForm((f) => ({ ...f, ten_ncc: e.target.value }))} placeholder="" />
                </div>
                ...
                <div style={fieldRow}>
                  ...
                  <input style={{ ...inputStyle, flex: 1 }} value={form.dien_giai} onChange={(e) => setForm((f) => ({ ...f, dien_giai: e.target.value }))} placeholder="" />
                </div>
                {/* Nhóm KH, NCC - giống Nhóm VTHH: ô readOnly + nút mũi tên xổ xuống */}
                <div style={fieldRow}>
                  ...
                  >
                    <input
                      readOnly
                      style={{ ...inputStyle, width: '100%', paddingRight: 26, cursor: 'pointer', boxSizing: 'border-box' }}
                ...
                {/* Điện thoại, Website chung 1 dòng */}
                ...
                {/* Email, Điều khoản TT chung 1 dòng */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 20px' }}>
                  <div style={fieldRow}>
                ...
                {/* NV mua hàng chung dòng với Số ngày được nợ; Số nợ tối đa cột 2 phía dưới */}
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 6, marginBottom: 6 }}>
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 4, color: 'var(--text-primary)' }}>Tài khoản ngân hàng</div>
                <div style={{ border: '1px solid var(--border)', borderRadius: 4, overflow: 'visible', background: 'var(--bg-tab)' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <thead>
                      <tr>
                        <th style={{ padding: '4px 6px', textAlign: 'left', background: 'var(--bg-tab)', borderBottom: '1px solid var(--border)', fontWeight: 600, width: 190 }}>Số tài khoản</th>
                    ...
                    <tbody>
                      {form.tai_khoan_ngan_hang.length === 0 ? (
                        <tr>
                          <td colSpan={3} style={{ padding: 8, color: 'var(--text-muted)', cursor: 'pointer', borderBottom: '1px solid var(--border)' }} onClick={themDongNganHang}>
                        ...
                        form.tai_khoan_ngan_hang.map((row, idx) => (
                          <tr key={idx}>
                            ...
                            <td style={{ padding: '4px 6px', borderBottom: '1px solid var(--border)', position: 'relative' }} ref={bankDropdownRow === idx ? bankDropdownWrapRef : undefined}>
                              ...
                              />
                              <span style={{ position: 'absolute', right: 4, top: 4, bottom: 4, width: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', background: 'var(--accent)', color: '#0d0d0d', borderRadius: '0 4px 4px 0' }}>
                                <ChevronDown size={12} style={{ color: '#0d0d0d' }} />
                              </span>
                              {bankDropdownRow === idx && (
                                <div
                                    ...
                                    banksFiltered.map((b) => (
                                      <div
                                        ...
                                        style={{
                                          ...
                                          color: row.ten_ngan_hang === b.name ? '#0d0d0d' : 'var(--text-primary)',
                                        }}
                                      >
          ...
          {formTab === 'thong_tin_chung' && form.loai_ncc === 'ca_nhan' && (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8, fontSize: 11 }}>
                ...
                <div style={fieldRow}>
                  <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Địa chỉ</label>
                  <input style={{ ...inputStyle, flex: 1 }} value={form.dia_chi} onChange={(e) => setForm((f) => ({ ...f, dia_chi: e.target.value }))} placeholder="" />
                </div>
                <div style={fieldRow}>
                  <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Diễn giải</label>
                  <input style={{ ...inputStyle, flex: 1 }} value={form.dien_giai} onChange={(e) => setForm((f) => ({ ...f, dien_giai: e.target.value }))} placeholder="" />
                ...
                <div style={fieldRow}>
                  <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Nhóm KH, NCC</label>
                  <div style={{ flex: 1, minWidth: 0, position: 'relative', cursor: 'pointer' }} onClick={() => setShowNhomKhNccLookup(true)}>
                    <input readOnly style={{ ...inputStyle, width: '100%', paddingRight: 26, cursor: 'pointer', boxSizing: 'border-box' }} value={displayNhomKhNcc} placeholder="Chọn nhóm..." />
                          ...
                          <tr key={idx}>
                            ...
                            <td style={{ padding: '4px 6px', borderBottom: '1px solid var(--border)', position: 'relative' }} ref={bankDropdownRow === idx ? bankDropdownWrapRef : undefined}>
                              <input
                                ...
                                disabled={banksLoading}
                              />
                              <span style={{ position: 'absolute', right: 4, top: 4, bottom: 4, width: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', background: 'var(--accent)', color: '#0d0d0d', borderRadius: '0 4px 4px 0' }}><ChevronDown size={12} style={{ color: '#0d0d0d' }} /></span>
                              {bankDropdownRow === idx && (
                                <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', marginTop: 2, maxHeight: 180, overflowY: 'auto', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 4, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 1000 }}>
                                  {banksFiltered.length === 0 ? <div style={{ padding: '8px 10px', color: 'var(--text-secondary)', fontSize: 12 }}>Không có kết quả</div> : banksFiltered.map((b) => (
                                    <div key={b.id} role="option" onMouseDown={(e) => { e.preventDefault(); capNhatDongNganHang(idx, 'ten_ngan_hang', b.name); setBankFilter(b.name); setBankDropdownRow(null) }} style={{ padding: '6px 10px', cursor: 'pointer', fontSize: 12, background: row.ten_ngan_hang === b.name ? 'var(--accent)' : undefined, color: row.ten_ngan_hang === b.name ? '#0d0d0d' : 'var(--text-primary)' }}>{b.shortName ? `${b.shortName} - ${b.name}` : b.name}</div>
                                  ))}
                                </div>
                              )}
                            </td>
          ...
          {formTab === 'khac' && form.loai_ncc === 'to_chuc' && (
            <>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: 'var(--text-primary)' }}>Vị trí địa lý</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 20px' }}>
                  <div style={fieldRow}>
                  ...
                  <div style={fieldRow}>
                    ...
                    <div ref={tinhTpWrapRef1} style={{ flex: 1, position: 'relative' }}>
                      {form.quoc_gia === 'Việt Nam' ? (
                                ...
                                tinhTpFiltered.map((t) => (
                                  <div
                                    ...
                                    style={{
                                      ...
                                      color: form.tinh_tp === t ? '#0d0d0d' : 'var(--text-primary)',
                                    }}
                                  >
                                    {t}
                                  </div>
                        ...
                        <>
                          <input style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', paddingRight: 26 }} value={form.tinh_tp} onChange={(e) => setForm((f) => ({ ...f, tinh_tp: e.target.value }))} placeholder="Tỉnh/Thành phố" />
                          <span style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', background: 'var(--accent)', color: '#0d0d0d' }}>
                            <ChevronDown size={12} style={{ color: '#0d0d0d' }} />
                          </span>
                        </>
                  ...
                  <div style={fieldRow}>
                    ...
                    <div ref={xaPhuongWrapRef1} style={{ flex: 1, position: 'relative' }}>
                      {form.quoc_gia === 'Việt Nam' && form.tinh_tp && MA_TINH_THEO_TEN[form.tinh_tp] ? (
                                ...
                                xaPhuongFiltered.map((w) => (
                                  <div
                                    ...
                                    style={{
                                      ...
                                      color: form.xa_phuong === w ? '#0d0d0d' : 'var(--text-primary)',
                                    }}
                                  >
                                    {w}
                                  </div>
                        ...
                        <>
                          <input style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', paddingRight: 26 }} value={form.xa_phuong} onChange={(e) => setForm((f) => ({ ...f, xa_phuong: e.target.value }))} placeholder="Xã/Phường" />
                          <span style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', background: 'var(--accent)', color: '#0d0d0d' }}>
                            <ChevronDown size={12} style={{ color: '#0d0d0d' }} />
                          </span>
                        </>
              ...
              {/* Thông tin liên hệ */}
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: 'var(--text-primary)' }}>Thông tin liên hệ</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 20px' }}>
                  <div style={fieldRow}>
                    <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Xưng hô</label>
                    <div style={{ flex: 1, position: 'relative' }}>
                  ...
                  <div style={fieldRow}>
                    <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Họ và tên</label>
                    <input style={{ ...inputStyle, flex: 1 }} value={form.ho_va_ten_lien_he} onChange={(e) => setForm((f) => ({ ...f, ho_va_ten_lien_he: e.target.value }))} />
                  </div>
                  <div style={fieldRow}>
                    <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Email</label>
                  ...
                  <div style={fieldRow}>
                    <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Đại diện theo PL</label>
                    <input style={{ ...inputStyle, flex: 1 }} value={form.dai_dien_theo_pl} onChange={(e) => setForm((f) => ({ ...f, dai_dien_theo_pl: e.target.value }))} />
                  </div>
                  <div style={fieldRow}>
                    <label style={{ ...labelStyle, minWidth: labelMinWidth }}>ĐTDĐ khác</label>
                    <input style={{ ...inputStyle, flex: 1 }} value={form.dtdd_khac} onChange={(e) => setForm((f) => ({ ...f, dtdd_khac: e.target.value }))} />
                  </div>
                  <div />
                </div>
                {/* Địa chỉ - dưới cùng mục Thông tin liên hệ, full width + gợi ý địa chỉ VN */}
                <div style={{ ...fieldRow, display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 6 }}>
                  <label style={{ ...labelStyle, minWidth: labelMinWidth }}>Địa chỉ</label>
                  <div style={{ flex: 1, width: '100%', minWidth: 0, position: 'relative' }}>
                    <input
                      style={{ ...inputStyle, flex: 1, width: '100%', minWidth: 0, boxSizing: 'border-box' }}
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, marginBottom: 6 }}>
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: 'var(--text-primary)' }}>Địa điểm giao hàng</div>
                <div style={{ border: '1px solid var(--border)', borderRadius: 4, overflow: 'visible', background: 'var(--bg-tab)' }}>
                  {form.dia_diem_giao_hang.length === 0 ? (
                    ...
                    <>
                      {form.dia_diem_giao_hang.map((dd, idx) => (
                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderBottom: '1px solid var(--border)', position: 'relative' }}>
          ...
          {loi && <p style={{ fontSize: '11px', color: 'var(--accent)', marginBottom: 8 }}>{loi}</p>}
