...
interface ChonKhachHangNccModalProps {
  ...
  excludeIds?: number[]
  onSelect: (records: NhaCungCapRecord[]) => void
  onClose: () => void
}
...
export function ChonKhachHangNccModal({
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((r) => {
      ...
      if (ma && (r.ma_ncc?.toLowerCase() ?? '').indexOf(ma) === -1) return false
      if (ten && (r.ten_ncc?.toLowerCase() ?? '').indexOf(ten) === -1) return false
                ...
                filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '4px 8px', borderRight: '1px solid var(--border)' }}>{r.ma_ncc}</td>
                      <td style={{ padding: '4px 8px', borderRight: '1px solid var(--border)' }}>{r.ten_ncc}</td>
