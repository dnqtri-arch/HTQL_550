...
import { NhomKhNccInlinePicker } from './nhomKhNccInlinePicker'
import { ThemNhomKhNccModal } from '../../shared/themNhomKhNccModal'
...
import { getWardsByProvinceCode } from '../../shared/wardsApi'
import { suggestAddressVietnam, cleanAddressForDisplay } from '../../shared/addressAutocompleteApi'
import { DiaDiemHangBlock } from '../../shared/diaDiemHangBlock'
import { lookupTaxCode, TaxLookupRateLimitedError } from '../../shared/taxLookupApi'
...
function recordToForm(r: NhaCungCapRecord): FormState {
  return {
    ...
    dia_diem_giao_trung_nhan: Boolean(r.dia_diem_giao_trung_nhan),
...
function formToPayload(f: FormState): Omit<NhaCungCapRecord, 'id'> {
  ...
  const diaDiemNhanHang = f.dia_diem_nhan_hang.filter((s) => (s ?? '').trim() !== '')
  const diaDiemGiaoRieng = f.dia_diem_giao_hang.filter((s) => (s ?? '').trim() !== '')
  const diaDiemGiaoHang =
    f.khach_hang && f.dia_diem_giao_trung_nhan
      ? diaDiemNhanHang
      : diaDiemGiaoRieng
  return {
    ...
    dia_diem_giao_hang: f.khach_hang && diaDiemGiaoHang.length > 0 ? diaDiemGiaoHang : undefined,
    dia_diem_giao_trung_nhan: f.khach_hang ? f.dia_diem_giao_trung_nhan : undefined,
...
export type NhaCungCapFormProps = {
  modalOpen: 'add' | 'edit' | 'clone' | null
  sourceRow: NhaCungCapRecord | null
  danhSachNhom: NhomKhNccItem[]
  setDanhSachNhom: Dispatch<SetStateAction<NhomKhNccItem[]>>
...
export function NhaCungCapForm({
  modalOpen,
  sourceRow,
  danhSachNhom,
  setDanhSachNhom,
  ...
  const [openNhomKhNccDropdown, setOpenNhomKhNccDropdown] = useState(false)
  const nhomKhNccWrapRef = useRef<HTMLDivElement>(null)
  const [showThemNhomKhNccModal, setShowThemNhomKhNccModal] = useState(false)
  ...
  /** Hiển thị tại ô Nhóm KH, NCC: các tên đã chọn */
    ...
    const run = async () => {
      ...
      if (modalOpen === 'edit') {
        ...
        if (row.loai_ncc === 'to_chuc') {
          setFormToChuc(r)
          setFormCaNhan({ ...emptyForm, loai_ncc: 'ca_nhan', ma_ncc: row.ma_ncc } as FormState)
        } else {
          setFormCaNhan(r)
          setFormToChuc({ ...emptyForm, loai_ncc: 'to_chuc', ma_ncc: row.ma_ncc } as FormState)
        }
        return
      }
      if (modalOpen === 'clone') {
        ...
        if (cancelled) return
        const cloned = { ...recordToForm(row), ma_ncc: newMa, ten_ncc: row.ten_ncc + ' (bản sao)' }
        if (row.loai_ncc === 'to_chuc') {
  ...
  const dongY = async () => {
    ...
    try {
      if (modalOpen === 'add' || modalOpen === 'clone') {
        ...
        await nhaCungCapPut(sourceRow.id, payload)
      }
      await onSaved()
      dongModal()
    } catch (e) {
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 400 }} data-error-field={errorFieldTrung ?? ''}>
                ...
                onChange={(e) => {
                  ...
                  setForm((f) => {
                    ...
                    return {
                      ...
                      dia_diem_giao_trung_nhan:
                        f.dia_diem_giao_trung_nhan || (giao.length > 0 && f.dia_diem_nhan_hang.length === 0),
                    }
                  })
                }}
              />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: FORM_ROW_GAP, marginBottom: FORM_SECTION_GAP, fontSize: 11 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: FORM_GRID_GAP }}>
                  ...
                  <div style={fieldRow}>
                    ...
                    <div ref={nhomKhNccWrapRef} style={{ display: 'flex', gap: 2, flex: 1, minWidth: 0, alignItems: 'center' }}>
                      <div
                        style={{ flex: 1, minWidth: 0, position: 'relative', cursor: 'pointer', height: FORM_FIELD_HEIGHT }}
                        onClick={() => setOpenNhomKhNccDropdown(true)}
                      >
                        <input
                          readOnly
                          ...
                          onClick={(e) => { e.stopPropagation(); setOpenNhomKhNccDropdown(true) }}
                        />
                        <span style={lookupChevronOverlayStyle}>
                          <ChevronDown size={12} style={{ color: 'var(--accent-text)' }} />
                        </span>
                        <NhomKhNccInlinePicker
                          open={openNhomKhNccDropdown}
                          anchorRef={nhomKhNccWrapRef}
                          items={danhSachNhom}
                          value={form.nhom_kh_ncc}
                          onCommit={(tens) => setForm((f) => ({ ...f, nhom_kh_ncc: tens.join('; ') }))}
                          onOpenChange={setOpenNhomKhNccDropdown}
                          onLiveSelectionChange={(tens) => setForm((f) => ({ ...f, nhom_kh_ncc: tens.join('; ') }))}
                        />
                      </div>
                      <button
                        type="button"
                        style={lookupActionButtonStyle}
                        title="Thêm nhóm KH, NCC"
                        onClick={(e) => {
                          e.stopPropagation()
                          setShowThemNhomKhNccModal(true)
                        }}
                      >
          ...
          {formTab === 'khac' && form.loai_ncc === 'to_chuc' && (
            <>
              ...
              {/* Địa điểm nhận / giao (khách hàng: giao trùng nhận) */}
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: FORM_SECTION_GAP, marginBottom: FORM_SECTION_GAP }}>
                {form.khach_hang && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, cursor: 'pointer', marginBottom: FORM_ROW_GAP }}>
                    <input
                      type="checkbox"
                      checked={form.dia_diem_giao_trung_nhan}
                      onChange={(e) => setForm((f) => ({ ...f, dia_diem_giao_trung_nhan: e.target.checked }))}
                    />
                    Địa điểm giao trùng nhận
                  </label>
                )}
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: FORM_ROW_GAP, color: 'var(--text-primary)' }}>Địa điểm nhận hàng</div>
                <DiaDiemHangBlock
                ...
                {form.khach_hang && (
                  <>
                    <div style={{ fontSize: 11, fontWeight: 600, marginTop: FORM_SECTION_GAP, marginBottom: FORM_ROW_GAP, color: 'var(--text-primary)' }}>Địa điểm giao hàng</div>
                    {form.dia_diem_giao_trung_nhan ? (
                      <div style={{ padding: 10, fontSize: 11, color: 'var(--text-muted)', background: 'var(--bg-tab)', border: '1px solid var(--border)', borderRadius: 4 }}>
                        Đồng bộ với địa điểm nhận hàng
                        {form.dia_diem_nhan_hang.filter((s) => (s ?? '').trim()).length === 0
                          ? ' — chưa có dòng'
                          : ` — ${form.dia_diem_nhan_hang.filter((s) => (s ?? '').trim()).length} dòng`}
                      </div>
                    ) : (
                      <DiaDiemHangBlock
          ...
          {formTab === 'khac' && form.loai_ncc === 'ca_nhan' && (
            <>
              ...
              {/* Địa điểm nhận / giao (khách hàng: giao trùng nhận) — Cá nhân */}
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: FORM_SECTION_GAP, marginBottom: FORM_SECTION_GAP }}>
                {form.khach_hang && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, cursor: 'pointer', marginBottom: FORM_ROW_GAP }}>
                    <input
      ...
      {showThemNhomKhNccModal && (
        <ThemNhomKhNccModal
          parentOptions={danhSachNhom}
          onClose={() => setShowThemNhomKhNccModal(false)}
          onSave={(item) => {
            const list = loadNhomKhNcc()
            if (!list.some((x) => x.ma === item.ma)) {
              saveNhomKhNcc([...list, item])
              setDanhSachNhom(loadNhomKhNcc())
            }
            setShowThemNhomKhNccModal(false)
          }}
          onSaveAndAdd={(item) => {
            const list = loadNhomKhNcc()
            if (!list.some((x) => x.ma === item.ma)) {
