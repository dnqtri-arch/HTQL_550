...
import {
  ...
  migrateHopDongMuaLinesToCoDonGia,
  chiTietToLines,
  getDonGiaMuaTheoDvt,
  computeHopDongMuaFooterTotals,
  enrichHopDongMuaGridLinesWithVthh,
  formatHopDongMuaLineThanhTienDisplay,
  formatHopDongMuaLineTienThueDisplay,
  formatHopDongMuaLineTongTienDisplay,
  parsePctThueGtgtFromLine,
  lineDonGiaCell,
  type HopDongMuaGridLineRow,
} from '../../../../utils/hopDongMuaCalculations'
import {
  hopDongMuaGetAll,
  hopDongMuaGetChiTiet,
  getDefaultHopDongMuaFilter,
  getDateRangeForKy as dhmGetDateRangeForKy,
  KY_OPTIONS as HDM_KY_OPTIONS,
  hopDongMuaPost,
  hopDongMuaPut,
  hopDongMuaDelete,
  getHopDongMuaDraft,
  ...
  type HopDongMuaCreatePayload,
  type HopDongMuaRecord,
  type HopDongMuaChiTiet,
  type HopDongMuaAttachmentItem,
...
import { HopDongMuaDinhKemModal, chuanHoaDuongDanDinhKemHopDongMua, partMccForPath } from './hopDongMuaDinhKemModal'
...
import dmhDetailStyles from './HopDongMua.module.css'
...
export interface HopDongMuaFormProps {
  onClose: () => void
  onSaved?: () => void
  ...
  initialDon?: HopDongMuaRecord | null
  initialChiTiet?: HopDongMuaChiTiet[] | null
  /** Dữ liệu đổ sẵn khi tạo mới (chuyển từ chứng từ khác), không phải chế độ xem */
  prefillDon?: Partial<HopDongMuaRecord> | null
  prefillChiTiet?: HopDongMuaChiTiet[] | null
  /** Sau khi bấm Lưu (chỉ lưu): chuyển form sang chế độ xem đơn vừa lưu, không đóng form */
  onSavedAndView?: (don: HopDongMuaRecord) => void
  formTitle?: string
  soDonLabel?: string
  /** Phiếu nhận hàng: đối chiếu theo đơn mua hàng (chọn ĐMH + link xem). */
  doiChieuSource?: 'don_mua_hang'
  /** Tạo phiếu nhận từ menu HĐM: prefill tiêu đề/HT/kho/ĐĐGH từ HĐM (HT/kho/ĐĐGH cho phép sửa lại); lưới chỉ sửa Số lượng; TG tạo = hiện tại; tiêu đề form = `_tieu_de_nguon_dhm`. */
  phieuNhanTuHopDongMua?: boolean
  /** Phiếu nhận mở từ «Thêm mới» danh sách (không prefill HĐM): lưới chi tiết giống form HĐM (thêm/xóa dòng, sửa đủ cột). */
...
/** Tình trạng hiển thị trên form HĐM / phiếu nhận (đủ bản ghi cũ: thêm option nếu giá trị đang lưu không nằm trong danh sách). */
...
/** Phiếu NVTHH: hiển thị «Đã nhập kho» thay cho cùng giá trị đồng bộ HĐM «Đã nhận hàng». */
...
function deriveApDungVatGtgtTuBanGhi(r: HopDongMuaRecord | Partial<HopDongMuaRecord> | null | undefined): boolean {
  if (r == null) return true
  const v = (r as HopDongMuaRecord).ap_dung_vat_gtgt
...
function deriveHinhThucSelectedIdsFromRecord(
  d: Partial<HopDongMuaRecord> & { hinh_thuc?: string; kho_nhap_id?: string; ten_cong_trinh?: string; dia_chi_cong_trinh?: string } | null | undefined
): string[] {
  if (!d) return []
...
function colWidthStyle(col: string, ghiChuFill?: boolean): React.CSSProperties {
  ...
  if (col === COL_DD_GH) return { width: 120, minWidth: 112, maxWidth: 140 }
  return {}
}
...
/** Cột số / tiền / thuế trong lưới chi tiết HĐM — canh phải (rule canh-le) */
...
/** Đối chiếu Phiếu nhận hàng: mọi HĐM trừ Hủy bỏ */
function hopDongMuaListForDoiChieuNhanHang(): HopDongMuaRecord[] {
  const all = hopDongMuaGetAll({ ...getDefaultHopDongMuaFilter(), tu: '', den: '' })
  return all.filter((dmh) => dmh.tinh_trang !== 'Hủy bỏ')
}

/** Hiển thị ô «Nhận hàng từ»: `mã HĐM: tiêu đề` (tiêu đề = so_chung_tu_cukcuk). */
function formatNhanHangTuTuDonMua(dmh: HopDongMuaRecord): string {
...
/** Phiếu NVTHH — popup «Nhận hàng từ»: chỉ HĐM tình trạng Chưa thực hiện. */
function hopDongMuaListChoPhieuNhanTuHopDongMua(): HopDongMuaRecord[] {
  const all = hopDongMuaGetAll({ ...getDefaultHopDongMuaFilter(), tu: '', den: '' })
...
function mauKhongDonGiaLines(): HopDongMuaGridLineRow[] {
  return []
}

/** Mẫu có đơn giá — không thêm dòng (rule mau_gia.mdc) */
function mauCoDonGiaLines(): HopDongMuaGridLineRow[] {
  return []
}

export function HopDongMuaForm({ onClose, onSaved, onHeaderPointerDown, dragging, readOnly = false, initialDon, initialChiTiet, prefillDon, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: formTitleProp, soDonLabel: soDonLabelProp, doiChieuSource, phieuNhanTuHopDongMua = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false }: HopDongMuaFormProps) {
  const api = useHopDongMuaApi()
  const toastApi = useToastOptional()
  const formTitle = formTitleProp ?? 'Hợp đồng mua'
  const soDonLabel = soDonLabelProp ?? 'Mã HĐM'
  const isViewMode = readOnly && initialDon != null
  const [editingFromView, setEditingFromView] = useState(false)
  const laPhieuNhanNvthh = Boolean(phieuNhanTuHopDongMua || doiChieuSource === 'don_mua_hang')
  ...
  /** Phiếu nhận từ HĐM: không thêm/xóa dòng; chỉ sửa cột Số lượng — trừ khi «Thêm mới» từ danh sách (không prefill HĐM). */
  const chiTietGridLocked = Boolean(phieuNhanTuHopDongMua) && !effectiveReadOnly && !phieuNhanThemMoiTuDanhSach
  ...
  const [attachments, setAttachments] = useState<HopDongMuaAttachmentItem[]>(() => {
  ...
  const [hinhThucSelectedIds, setHinhThucSelectedIds] = useState<string[]>(() => {
    const fromRec = deriveHinhThucSelectedIdsFromRecord((initialDon ?? prefillDon) as HopDongMuaRecord | null | undefined)
    if (fromRec.length > 0) return fromRec
    if (phieuNhanTuHopDongMua && initialDon == null && prefillDon == null) return defaultPhieuNhanHinhThucNhapKhoIds()
    return fromRec
  })
  ...
  const [thamChieu, setThamChieu] = useState(() => {
    ...
    if (prefillDon && phieuNhanTuHopDongMua) {
      const pd = prefillDon as HopDongMuaRecord
      if ((pd.so_don_hang ?? '').trim()) return formatNhanHangTuTuDonMua(pd)
    }
    if (prefillDon?.so_chung_tu_cukcuk != null) {
      return phieuNhanTuHopDongMua
  ...
  const [ngayDonHang, setNgayDonHang] = useState<Date | null>(() => {
    ...
    if (phieuNhanTuHopDongMua) return new Date()
  ...
  const [phieuChiNgay, setPhieuChiNgay] = useState<Date | null>(() => {
    const r = (initialDon ?? prefillDon) as HopDongMuaRecord | undefined
  ...
  const [lines, setLines] = useState<HopDongMuaGridLineRow[]>(() => {
  ...
  const [apDungVatGtgt, setApDungVatGtgt] = useState(() =>
    deriveApDungVatGtgtTuBanGhi((initialDon ?? prefillDon) as HopDongMuaRecord | null | undefined),
  )
  ...
  const [phieuChiAttachments, setPhieuChiAttachments] = useState<HopDongMuaAttachmentItem[]>(() => {
    const r = (initialDon ?? prefillDon) as HopDongMuaRecord | undefined
  ...
  const [viewHopDongMuaRecord, setViewHopDongMuaRecord] = useState<HopDongMuaRecord | null>(null)

  /** Phiếu NVTHH: bỏ margin dưới từng dòng — khoảng cách dọc = `gap` tab panel (cùng kho nhập → ĐĐGH 1). */
  const fieldRowDyn = useMemo(
    (): React.CSSProperties => ({ ...fieldRow, marginBottom: phieuNhanTuHopDongMua ? 0 : FIELD_ROW_GAP }),
    [phieuNhanTuHopDongMua]
  )
  ...
  const soDonLabelHienThi = useMemo(() => {
    if (!phieuNhanTuHopDongMua) return soDonLabel
  ...
  }, [phieuNhanTuHopDongMua, phieuNhanTabChung, soDonLabel])
  ...
  const tieuDePhieuNvthhDayDu = useMemo(() => {
    if (!phieuNhanTuHopDongMua) return ''
    const chungTu =
  ...
  }, [phieuNhanTuHopDongMua, hinhThucSelectedIds, hinhThucList, chungTuMuaPttt, chungTuMuaLoaiHd, nhaCungCapDisplay])
  ...
  }, [phieuNhanTuHopDongMua, thamChieu, phieuSoHoaDon])

  useEffect(() => {
    dienGiaiPhieuTuChinhRef.current = false
  }, [initialDon?.id, phieuNhanTuHopDongMua])

  useEffect(() => {
    if (!phieuNhanTuHopDongMua || initialDon != null) return
    if (dienGiaiPhieuTuChinhRef.current) return
    setDienGiai(dienGiaiPhieuMacDinh)
  }, [phieuNhanTuHopDongMua, initialDon, dienGiaiPhieuMacDinh])

  useEffect(() => {
    if (!phieuNhanTuHopDongMua || phieuNhanTabChung !== 'phieu-chi' || initialDon != null) return
    setPhieuChiNcc(nhaCungCapDisplay)
    setPhieuChiDiaChi(diaChi)
  }, [phieuNhanTuHopDongMua, phieuNhanTabChung, nhaCungCapDisplay, diaChi, initialDon])
  ...
  useEffect(() => {
    ...
    if (prefillDon) {
      ...
      if (prefillDon.ngay_don_hang != null && !phieuNhanTuHopDongMua) setNgayDonHang(parseIsoToDate(prefillDon.ngay_don_hang))
      if (prefillDon.ngay_giao_hang !== undefined) setNgayGiaoHang(parseIsoToDate(prefillDon.ngay_giao_hang ?? null))
      const ext = prefillDon as HopDongMuaRecord
  ...
  }, [isViewMode, prefillDon, prefillChiTiet, laPhieuNhanNvthh, phieuNhanTuHopDongMua, doiChieuSource])
  ...
  const applyChonDonMuaHangDoiChieu = useCallback(
    (dmh: HopDongMuaRecord) => {
      const label = (dmh.so_don_hang ?? '').trim()
      setThamChieu(phieuNhanTuHopDongMua ? formatNhanHangTuTuDonMua(dmh) : label)
      ...
      if (hit != null) {
        ...
        if (phieuNhanTuHopDongMua && chungTuMuaPttt === 'chuyen_khoan') {
      ...
      setKhoNhapId((dmh as HopDongMuaRecord & { kho_nhap_id?: string }).kho_nhap_id ?? '')
      setTenCongTrinh((dmh as HopDongMuaRecord & { ten_cong_trinh?: string }).ten_cong_trinh ?? '')
      setDiaChiCongTrinh((dmh as HopDongMuaRecord & { dia_chi_cong_trinh?: string }).dia_chi_cong_trinh ?? '')
      ...
      const ct = hopDongMuaGetChiTiet(dmh.id)
      setLines(chiTietToLines(ct))
    },
    [phieuNhanTuHopDongMua, nccList, chungTuMuaPttt],
  )

  useEffect(() => {
    if (!phieuNhanTuHopDongMua || chungTuMuaPttt !== 'chuyen_khoan' || effectiveReadOnly) return
  ...
  }, [phieuNhanTuHopDongMua, chungTuMuaPttt, nhaCungCapId, nccList, effectiveReadOnly])
  ...
  /** Đổi đơn (id): nạp file đính kèm từ bản ghi và chuẩn hóa `name`/`virtual_path` theo Mã HĐM + NCC hiện tại (dữ liệu cũ có thể còn `ncc_unknown` trong tên file). */
  useEffect(() => {
    ...
    const so = (initialDon.so_don_hang ?? '').trim() || 'HDM'
    const nccPart = partMccForPath('')
    setAttachments(chuanHoaDuongDanDinhKemHopDongMua(raw, so, nccPart))
  }, [initialDon?.id])

  /** Đồng bộ `name` + `virtual_path` khi đổi Mã HĐM / NCC (đính kèm trước, nhập NCC sau — tránh tên file vẫn `ncc_unknown`). */
  useEffect(() => {
    const so = soDonHang.trim() || 'HDM'
    setAttachments((prev) => {
      if (prev.length === 0) return prev
      const next = chuanHoaDuongDanDinhKemHopDongMua(prev, so, nccPartDinhKem)
  ...
  }, [phieuNhanTuHopDongMua, soDonHang, nccPartDinhKem])
  ...
  useEffect(() => {
    ...
    const d = initialDon as HopDongMuaRecord & { hinh_thuc?: string; kho_nhap_id?: string; ten_cong_trinh?: string; dia_chi_cong_trinh?: string }
    ...
    if (phieuNhanTuHopDongMua && initialDon.doi_chieu_don_mua_id) {
      const linked = hopDongMuaGetAll({ ...getDefaultHopDongMuaFilter(), tu: '', den: '' }).find((x) => x.id === initialDon.doi_chieu_don_mua_id)
      setThamChieu(
    ...
    if (rawAtt.length === 0) setAttachments([])
      ...
      const so = (initialDon.so_don_hang ?? '').trim() || 'HDM'
      const nccPart = partMccForPath('')
      setAttachments(chuanHoaDuongDanDinhKemHopDongMua(rawAtt, so, nccPart))
    }
    setEditingFromView(false)
  }, [readOnly, initialDon?.id, initialDon, initialChiTiet, laPhieuNhanNvthh, phieuNhanTuHopDongMua])
  ...
  useEffect(() => {
    if (khoDropdownOpen && refKhoWrap.current) {
      ...
      setKhoDropdownRect({
        top: r.bottom,
        left: r.left,
        width: phieuNhanTuHopDongMua ? r.width : Math.max(r.width, 200),
      })
    } else {
      setKhoDropdownRect(null)
    }
  }, [khoDropdownOpen, phieuNhanTuHopDongMua])
  ...
  const handleChonNcc = (ncc: NhaCungCapRecord) => {
    ...
    setMaSoThue(
      ncc.loai_ncc === 'ca_nhan'
        ? (ncc.dt_di_dong ?? ncc.dien_thoai ?? '').trim()
        : (ncc.ma_so_thue ?? '').trim(),
    )
  ...
  /** Prefill chi tiết (vd. từ HĐM): tra cứu VTHH để điền ĐG mua, % thuế GTGT và tính tiền. */
  useEffect(() => {
    ...
    setLines((prev) => enrichHopDongMuaGridLinesWithVthh(prev, vatTuList))
  }, [vatTuList, prefillChiTiet])

  /** Khi chọn Đối chiếu từ dropdown: enrich chi tiết với ĐG mua, % thuế từ VTHH (giống Chuyển HĐM). */
    ...
    setLines((prev) => {
      ...
      const next = prev.map((line) => {
        ...
        if (raw === '' || !Number.isFinite(n) || !allowed.has(n)) {
          changed = true
          return { ...line, [COL_DD_GH]: String(opts[0]?.idx ?? 0) } as unknown as HopDongMuaGridLineRow
        }
        return line
      })
  ...
  const buildPayload = (): HopDongMuaCreatePayload => {
    const isCodongia = initialDon != null || mauHienTai === 'codongia'
    const apVat = isCodongia && apDungVatGtgt
    let giaTriDonHang = 0
    const chiTiet = lines
      ...
      .map((line) => {
        ...
        giaTriDonHang += thanhTien + (apVat ? tienThue : 0)
        ...
        return {
          ...
          pt_thue_gtgt: apVat ? pt : null,
          tien_thue_gtgt: apVat && pt != null ? tienThue : null,
          dd_gh_index: ddIdx,
    ...
    const ngayGiaoHangPayload =
      phieuNhanTuHopDongMua ? ngayGiaoHang ?? ngayDonHang : ngayGiaoHang
    const payload: HopDongMuaCreatePayload = {
      tinh_trang: tinhTrang,
      ngay_don_hang: toIsoDate(ngayDonHang) || toIsoDate(new Date()),
      so_don_hang: soDonHang.trim() || 'HDM',
      ...
      ap_dung_vat_gtgt: isCodongia ? apDungVatGtgt : undefined,
      ...
      attachments:
        attachments.length > 0
          ? chuanHoaDuongDanDinhKemHopDongMua(attachments, soDonHang.trim() || 'HDM', nccPartDinhKem)
          : undefined,
      ...(phieuNhanTuHopDongMua
        ? {
            ...
            phieu_chi_attachments:
              phieuChiAttachments.length > 0
                ? chuanHoaDuongDanDinhKemHopDongMua(phieuChiAttachments, soDonHang.trim() || 'HDM', nccPartDinhKem)
                : undefined,
          }
        : {}),
    }
  ...
  const validateBeforeSave = (): boolean => {
    ...
    const tgGiaoHangTenLoi =
      phieuNhanTuHopDongMua && phieuNhanTabChung === 'phieu-chi' ? 'TG chi' : 'TG nhận hàng'
    if (phieuNhanTuHopDongMua) {
      if (!ngayGiaoHang) {
  ...
  const phieuNhanLienKetTuDonMua = useMemo(() => {
    if (!initialDon?.id || phieuNhanTuHopDongMua) return []
  ...
  const nccSelected = useMemo(() => {
    if (nhaCungCapId == null) return null
    return nccList.find((n) => n.id === nhaCungCapId) ?? null
  }, [nhaCungCapId, nccList])
  ...
  const { tongTienHang, tienThue, tongTienThanhToan } = computeHopDongMuaFooterTotals(lines, {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, background: 'var(--bg-primary)' }}>
      ...
      <div style={toolbarWrap}>
        ...
        >
          <button type="button" style={toolbarBtn} disabled={effectiveReadOnly || initialDon != null || phieuNhanTuHopDongMua}>
            <FileText size={14} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>Mẫu <ChevronDown size={10} /></span>
          </button>
          {dropdownMau && !effectiveReadOnly && initialDon == null && !phieuNhanTuHopDongMua && (
                  ...
                  onMouseDown={(e) => {
                    ...
                    if (key === 'codongia') {
                      if (mauHienTai === 'khongdongia') {
                        setLines((prev) => (prev.length > 0 ? migrateHopDongMuaLinesToCoDonGia(prev, vatTuList) : mauCoDonGiaLines()))
                      } else {
      ...
      {doiChieuSource === 'don_mua_hang' && !phieuNhanTuHopDongMua && (
        <Modal
        ...
        >
          <div style={{ maxHeight: 320, overflowY: 'auto' }}>
            {(() => {
              const dmhChonTuDonList = hopDongMuaListForDoiChieuNhanHang()
              return (
                <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1 1 0', minHeight: 0, gap: 2, marginBottom: 2 }}>
          ...
          {(() => {
            const thongTinChungFields = (
              <>
            <div style={fieldRowDyn}>
              <label style={labelStyle}>{phieuNhanTuHopDongMua ? 'Nhận hàng từ:' : 'Tiêu đề'}</label>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                {effectiveReadOnly ? (
                  ...
                  <div
                    ref={phieuNhanTuHopDongMua && doiChieuSource === 'don_mua_hang' ? refNhanHangTuWrap : undefined}
                    style={{
                      flex: 1,
                      minWidth: 0,
                      display: 'flex',
                      ...
                      position: phieuNhanTuHopDongMua && doiChieuSource === 'don_mua_hang' ? 'relative' : undefined,
                    }}
                  >
                    <div style={{ flex: 1, minWidth: 0, position: 'relative', display: 'flex', height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT }}>
                      <input
                        ...
                        style={{
                          ...
                          height: LOOKUP_CONTROL_HEIGHT,
                          minHeight: LOOKUP_CONTROL_HEIGHT,
                          ...(phieuNhanTuHopDongMua ? { textTransform: 'uppercase' as const } : {}),
                        }}
                        value={thamChieu}
                        onChange={(e) => {
                          const raw = e.target.value
                          setThamChieu(phieuNhanTuHopDongMua ? raw.toUpperCase() : raw)
                          if (valErrKey === 'doi_chieu') setValErrKey(null)
                        }}
                        onFocus={() => {
                          if (phieuNhanTuHopDongMua && doiChieuSource === 'don_mua_hang' && !effectiveReadOnly) {
                            setNhanHangTuDropdownOpen(true)
                          }
                        }}
                        onClick={() => {
                          if (doiChieuSource === 'don_mua_hang' && !effectiveReadOnly) {
                            if (phieuNhanTuHopDongMua) setNhanHangTuDropdownOpen(true)
                        ...
                        disabled={false}
                        placeholder={
                          phieuNhanTuHopDongMua ? 'Bấm để chọn đơn Chưa thực hiện…' : 'Nhập tiêu đề'
                        }
                      />
                    </div>
                  </div>
                )}
                {hienThiFooterTongTien && !phieuNhanTuHopDongMua && (
                  <div
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={apDungVatGtgt}
                        disabled={effectiveReadOnly}
                        onChange={() => {
                          setApDungVatGtgt(true)
                          setLines((prev) => enrichHopDongMuaGridLinesWithVthh(prev, vatTuList))
                        }}
                      />
                      Có VAT
                    ...
                    >
                      <input
                        ...
                        disabled={effectiveReadOnly}
                        onChange={() => {
                          setApDungVatGtgt(false)
                          setLines((prev) =>
                            prev.map((line) => {
                              if ((line['Mã'] ?? '').trim() === '') return line
                              const donGia = parseFloatVN(lineDonGiaCell(line))
                              const soLuong = Math.max(0, parseFloatVN(line['Số lượng'] ?? ''))
                              const tt = donGia * soLuong
                              return {
                                ...line,
                                '% thuế GTGT': '',
                                'Tiền thuế GTGT': formatSoTienHienThi(0),
                                'Thành tiền': formatSoTienHienThi(tt),
                                'Tổng tiền': formatSoTienHienThi(tt),
            ...
            !effectiveReadOnly
              ? ReactDOM.createPortal(
                  (() => {
                    ...
                    const listNvthhNhanTu = hopDongMuaListChoPhieuNhanTuHopDongMua().filter((dmh) => {
                      if (!q) return true
            ...
            <div style={{ ...fieldRowDyn, alignItems: 'center', width: '100%', minWidth: 0 }}>
              ...
              <label style={{ ...labelStyle, minWidth: 82, width: 82, flexShrink: 0, textAlign: 'right' }}>
                {nccSelected?.loai_ncc === 'ca_nhan' ? 'SĐT' : 'Mã số thuế'}
              </label>
              <input
                style={{
                  ...inputStyle,
            ...
            {laPhieuNhanNvthh ? (
              ...
              <>
                {nccSelected?.loai_ncc === 'to_chuc' && (
                  <div style={{ ...fieldRowDyn, flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                    <label style={labelStyle}>Người liên hệ</label>
                    <input
                      style={{ ...inputStyle, flex: '1 1 160px', minWidth: 120 }}
                      value={(nccSelected.nguoi_lien_he ?? '').trim()}
                      readOnly
                      disabled
                      tabIndex={-1}
                    />
                    <label style={{ ...labelStyle, minWidth: 48, flexShrink: 0 }}>SĐT</label>
                    <input
                      style={{ ...inputStyle, flex: '0 1 140px', minWidth: 100, maxWidth: 180 }}
                      value={(nccSelected.dien_thoai ?? nccSelected.dt_di_dong ?? '').trim()}
                      readOnly
                      disabled
                      tabIndex={-1}
                    />
            ...
            {(!phieuNhanTuHopDongMua || chungTuMuaCachThanhToan === 'chua_thanh_toan') && (
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                {(() => {
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, minWidth: 0 }}>
                    <label style={{ ...labelStyle, minWidth: 90, paddingTop: phieuNhanTuHopDongMua ? 2 : 4 }}>Địa điểm GH {i + 1}</label>
                    <div
                ...
                {!effectiveReadOnly && (
                  <button
                    ...
                    style={{ alignSelf: 'flex-start', marginLeft: 98, marginTop: phieuNhanTuHopDongMua ? 0 : undefined, display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', fontSize: 10, background: 'var(--accent)', color: 'var(--accent-text)', border: 'none', borderRadius: 4, cursor: 'pointer' }}
                  >
                  ...
                  >
                    <div style={{ ...fieldRowDyn, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      <input
                        ...
                        disabled={effectiveReadOnly}
                      />
                      <label style={{ ...labelStyle, minWidth: 88 }}>
                        {nccSelected?.loai_ncc === 'ca_nhan' ? 'SĐT' : 'Mã số thuế'}
                      </label>
                      <input
          ...
          <div
            style={{
              ...groupBox,
              width: phieuNhanTuHopDongMua ? 266 : 268,
              minWidth: phieuNhanTuHopDongMua ? 266 : 268,
              marginBottom: 0,
              flexShrink: 0,
              ...(phieuNhanTuHopDongMua ? { padding: '6px 8px', gap: FIELD_ROW_GAP } : {}),
              display: 'flex',
              flexDirection: 'column',
              alignSelf: 'stretch',
              minHeight: 0,
          ...
          >
            <div style={{ ...groupBoxTitle, ...(phieuNhanTuHopDongMua ? { marginBottom: 4 } : {}) }}>
              {phieuNhanTuHopDongMua ? (phieuNhanTabChung === 'hoa-don' ? 'Hóa đơn' : 'Chứng từ') : 'Đơn hàng'}
            </div>
            {phieuNhanTuHopDongMua && phieuNhanTabChung === 'hoa-don' ? (
              <>
                <div style={fieldRowDyn}>
            ...
            <div style={fieldRowDyn}>
              ...
              <DonHangBoxValueLikeNvMuaHang
                readOnly={effectiveReadOnly}
                fixedBandWidth={phieuNhanTuHopDongMua ? NVTHH_DON_HANG_BOX_VALUE_BAND_PX : undefined}
                trailingSlot={
                  !effectiveReadOnly ? (
                          ...
                          >
                            {showColAsReadOnly ? (
                              <input
                                ...
                                value={
                                  col === 'ĐVT'
                                    ...
                                    : col === 'Thành tiền'
                                      ? formatHopDongMuaLineThanhTienDisplay(line)
                                      : col === 'Tiền thuế GTGT'
                                        ? formatHopDongMuaLineTienThueDisplay(line)
                                        : col === 'Tổng tiền'
                                          ? formatHopDongMuaLineTongTienDisplay(line)
                                          : col === COL_DD_GH
                                            ? (() => {
                                                const opts = getDiaDiemGhOptions(diaDiemGiaoHangList, effectiveDiaDiemFirst)
                                                const raw = (line[COL_DD_GH] ?? '').trim()
                                ...
                                if (hasDvtDropdown) {
                                  return (
                                    <select
                                      ...
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = r[idx] as HopDongMuaGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], [col]: formatSoTuNhienInput(e.target.value) } as unknown as HopDongMuaGridLineRow
                                  setLines(r)
                                ...
                                onBlur={() => {
                                  ...
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], [col]: formatSoTienHienThi(val) } as unknown as HopDongMuaGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'ĐG mua' ? (
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'ĐG mua': formatSoTien(e.target.value) } as unknown as HopDongMuaGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'Thành tiền' ? (
                              <input
                                ...
                                value={formatHopDongMuaLineThanhTienDisplay(line)}
                              />
                            ) : col === 'Tiền thuế GTGT' ? (
                              <input
                                readOnly
                                ...
                                value={formatHopDongMuaLineTienThueDisplay(line)}
                              />
                            ) : col === 'Tổng tiền' ? (
                              <input
                                readOnly
                                ...
                                value={formatHopDongMuaLineTongTienDisplay(line)}
                              />
                            ) : col === COL_DD_GH ? (
                              chiTietReadOnly ? (
                                <input
                                ...
                                <select
                                  ...
                                  onChange={(e) => {
                                    const r = [...lines]
                                    r[idx] = { ...r[idx], [COL_DD_GH]: e.target.value } as unknown as HopDongMuaGridLineRow
                                    setLines(r)
                                  }}
                                >
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], '% thuế GTGT': e.target.value } as unknown as HopDongMuaGridLineRow
                                  setLines(r)
                                }}
                              >
                              ...
                              <input
                                ...
                                value={line[col] ?? ''}
                                onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], [col]: e.target.value } as unknown as HopDongMuaGridLineRow; setLines(r) }}
                              />
                            )}
                          </td>
                        )
                  ...
                  <div style={{ marginLeft: OFFSET_TRAI_COT_MA_VTHH, display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setLines([
                          ...lines,
                          currentColumns.reduce<Record<string, string>>((acc, c) => ({ ...acc, [c]: '' }), {}) as unknown as HopDongMuaGridLineRow,
                        ])
                      }}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'transparent', zIndex: 4000, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
          ...
          >
            <HopDongMuaApiProvider api={apiHopDongMua}>
              <HopDongMuaForm
                key={`dmh-popup-${viewHopDongMuaRecord.id}`}
                readOnly={true}
                initialDon={viewHopDongMuaRecord}
                initialChiTiet={hopDongMuaGetChiTiet(viewHopDongMuaRecord.id)}
                formTitle="Hợp đồng mua"
                soDonLabel="Mã HĐM"
                onClose={() => { setShowDonMuaHangPopup(false); setViewHopDongMuaRecord(null) }}
                onSaved={() => { setShowDonMuaHangPopup(false); setViewHopDongMuaRecord(null) }}
              />
