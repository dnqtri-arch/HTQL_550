...
/** 2026/HDM/3 → 2026_hdm_3 */
export function parseSoDonHangForAttachmentFile(soDon: string): string {
  const t = soDon.trim()
  const m = t.match(/^(\d{4})\s*\/\s*HDM\s*\/\s*(\d+)$/i)
  if (m) return `${m[1]}_hdm_${m[2]}`
...
const RE_HDM_ATT_NAME_TAIL = /_(\d{2})_(\d{2})_(\d{2})_(\d{2})_(\d{4})_(\d+)$/

/**
 * Khi đổi Mã HĐM / NCC sau khi đã đính kèm: đổi lại `name` cho khớp (trước đó có thể là `…_ncc_unknown_…`).
...
export function rebuildDhmAttachmentStoredFileName(
  ...
  const m = base.match(RE_HDM_ATT_NAME_TAIL)
  if (!m) return raw
...
/**
 * Trước khi lưu / khi đổi Mã HĐM hoặc mã NCC: gán lại `name` (nếu đúng pattern) và `virtual_path` theo quy tắc hiện tại
 * (don_hang_mua / mã_ncc / mã_đhm / tên_file), tránh lệch khi user đính kèm trước rồi mới nhập NCC/số đơn.
 */
export function chuanHoaDuongDanDinhKemHopDongMua(
  items: HopDongMuaAttachmentItem[],
  soDonHang: string,
  maNccPathPart: string
): HopDongMuaAttachmentItem[] {
  return items.map((a) => {
...
/** Đường dẫn hiển thị trong UI: chỉ tới thư mục; luôn tính theo Mã HĐM + phần NCC hiện tại (không tin virtual_path cũ). */
export function duongDanHienThi(item: HopDongMuaAttachmentItem, soDonHang: string, maNccPathPart: string): string {
...
export type HopDongMuaDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: HopDongMuaAttachmentItem[]
  onChange: (next: HopDongMuaAttachmentItem[]) => void
  readOnly?: boolean
  soDonHang: string
...
const HDM_DINH_KEM_TOI_DA_TEP = 10
const HDM_DINH_KEM_TOI_DA_BYTES = 5 * 1024 * 1024
const PANEL_Z = 1060
...
export function HopDongMuaDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const slotConLai = HDM_DINH_KEM_TOI_DA_TEP - attachments.length
      if (slotConLai <= 0) {
        baoLoiDinhKem('Đã đủ 10 file đính kèm. Xóa bớt file nếu cần thêm mới.')
        return
      }
      if (list.length > HDM_DINH_KEM_TOI_DA_TEP) {
        baoLoiDinhKem(
          `Bạn đã chọn ${list.length} file — mỗi lần chỉ được chọn tối đa ${HDM_DINH_KEM_TOI_DA_TEP} file.\n\nKhông xử lý hay tải file nào.`
        )
        return
      }
      ...
      for (const file of list) {
        ...
        else if (file.size >= HDM_DINH_KEM_TOI_DA_BYTES)
      ...
      const hangMoi: HopDongMuaAttachmentItem[] = []
      ...
      const withMeta: HopDongMuaAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
      ...
      <div style={{ padding: '8px 10px', overflowY: 'auto', flex: 1, minHeight: 0 }}>
        {!readOnly && (
          ...
          >
            ...
            <button
              type="button"
              disabled={attachments.length >= HDM_DINH_KEM_TOI_DA_TEP}
              title={attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'Đã đủ 10 file đính kèm' : undefined}
              style={{
                ...nutPhu,
                flexShrink: 0,
                background: attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'var(--border)' : 'var(--accent)',
                color: attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'var(--text-muted)' : 'var(--accent-text)',
                borderColor: attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'var(--border-strong)' : 'var(--accent)',
                boxShadow:
                  attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'none' : '0 1px 3px rgba(230, 126, 34, 0.35)',
                cursor: attachments.length >= HDM_DINH_KEM_TOI_DA_TEP ? 'not-allowed' : 'pointer',
              }}
              onClick={() => {
                if (attachments.length >= HDM_DINH_KEM_TOI_DA_TEP) return
