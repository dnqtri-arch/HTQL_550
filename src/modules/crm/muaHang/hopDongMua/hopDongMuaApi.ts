/**
 * API và types cho Hợp đồng mua (header + chi tiết dòng).
...
import type {
  ...
  HopDongMuaChiTiet,
  HopDongMuaCreatePayload,
  HopDongMuaDraftLine,
  HopDongMuaFilter,
  HopDongMuaKyValue,
  HopDongMuaRecord,
} from '../../../../types/hopDongMua'
...
/** Alias tương thích — cùng nghĩa với HopDongMuaKyValue. */
export type KyValue = HopDongMuaKyValue

const MODULE_PREFIX = 'HDM'

/** Dữ liệu mẫu hợp đồng mua */
const MOCK_DON: HopDongMuaRecord[] = [
  {
    id: 'dhm1',
    tinh_trang: 'Chưa thực hiện',
    ngay_don_hang: '2026-03-10',
    so_don_hang: 'HDM00001',
    ngay_giao_hang: null,
...
/** Chi tiết theo đơn */
const MOCK_CHI_TIET: HopDongMuaChiTiet[] = [
  {
    id: 'ct1',
    hop_dong_mua_id: 'dhm1',
    ma_hang: 'VT00001',
...
const STORAGE_KEY_DON = 'htql_hop_dong_mua_list'
const STORAGE_KEY_CHI_TIET = 'htql_hop_dong_mua_chi_tiet'

function normalizeDon(d: Partial<HopDongMuaRecord> & { id: string; de_xuat_id?: string }): HopDongMuaRecord {
  ...
  return {
    ...
    attachments: Array.isArray((d as { attachments?: HopDongMuaAttachmentItem[] }).attachments)
      ? (d as { attachments: HopDongMuaAttachmentItem[] }).attachments
      : undefined,
...
function loadFromStorage(): { don: HopDongMuaRecord[]; chiTiet: HopDongMuaChiTiet[] } {
  try {
    ...
    if (Array.isArray(don) && Array.isArray(chiTiet)) {
      return { don: don.map((d: Partial<HopDongMuaRecord> & { id: string }) => normalizeDon(d)), chiTiet }
    }
  } catch {
    /* ignore */
  }
...
const STORAGE_KEY_DRAFT = 'htql_hop_dong_mua_draft'

export function getHopDongMuaDraft(): HopDongMuaDraftLine[] | null {
  try {
...
export function setHopDongMuaDraft(lines: Array<Record<string, string> & { _dvtOptions?: string[]; _vthh?: unknown }>): void {
  try {
    if (typeof htqlEntityStorage !== 'undefined') {
...
export function clearHopDongMuaDraft(): void {
  try {
...
let _donList: HopDongMuaRecord[] = _initial.don
let _chiTietList: HopDongMuaChiTiet[] = _initial.chiTiet

export function hopDongMuaReloadFromStorage(): void {
...
export function hopDongMuaGetChiTiet(donId: string): HopDongMuaChiTiet[] {
  return _chiTietList.filter((c) => c.hop_dong_mua_id === donId)
}
...
export function hopDongMuaBuildCreatePayloadFromRecord(row: HopDongMuaRecord, ct: HopDongMuaChiTiet[]): HopDongMuaCreatePayload {
  return {
...
export function hopDongMuaSetTinhTrang(donId: string, tinh_trang: string): void {
  const row = _donList.find((d) => d.id === donId)
  if (!row) return
  const ct = hopDongMuaGetChiTiet(donId)
  hopDongMuaPut(donId, { ...hopDongMuaBuildCreatePayloadFromRecord(row, ct), tinh_trang })
}

/** Xóa hợp đồng mua và toàn bộ chi tiết của đơn. */
export function hopDongMuaDelete(donId: string): void {
  _donList = _donList.filter((d) => d.id !== donId)
  _chiTietList = _chiTietList.filter((c) => c.hop_dong_mua_id !== donId)
  saveToStorage()
}

/** Tạo hợp đồng mua mới (thêm vào danh sách nội bộ). Trả về bản ghi đơn vừa tạo. */
export async function hopDongMuaPost(payload: HopDongMuaCreatePayload): Promise<HopDongMuaRecord> {
...
/** Cập nhật hợp đồng mua (xóa chi tiết cũ, ghi lại theo payload). */
export function hopDongMuaPut(donId: string, payload: HopDongMuaCreatePayload): void {
...
export function getDefaultHopDongMuaFilter(): HopDongMuaFilter {
  const ky = 'thang-nay' as KyValue
  const { tu, den } = getDateRangeForKy(ky)
  return { ky, tu, den }
}

/** Trả về số đơn hàng tiếp theo (2026/HDM/1, 2026/HDM/2...) — reset mỗi năm. */
export function hopDongMuaSoDonHangTiepTheo(): string {
  const year = getCurrentYear()
  let maxSo = 0
