...
export function nhanVatTuHangHoaDelete(donId: string): void {
  ...
  const doiChieu = row?.doi_chieu_don_mua_id?.trim()
  _donList = _donList.filter((d) => d.id !== donId)
  _chiTietList = _chiTietList.filter((c) => c.don_hang_mua_id !== donId)
  saveToStorage()
  if (doiChieu) {
    const conPhieuKhac = _donList.some((d) => (d.doi_chieu_don_mua_id ?? '').trim() === doiChieu)
    if (!conPhieuKhac) {
      donHangMuaSetTinhTrang(doiChieu, 'Chưa thực hiện')
    }
  }
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent(HTQL_DHM_LIST_REFRESH_EVENT))
  }
}
