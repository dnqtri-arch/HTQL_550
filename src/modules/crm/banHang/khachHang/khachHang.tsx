...
export function KhachHang({ onQuayLai, embeddedAddMode, onAddSuccess, onClose }: { onQuayLai?: () => void } & KhachHangEmbeddedProps) {
  ...
  const moSua = () => {
    ...
    setForm(r)
    if (dongChon.loai_kh === 'to_chuc') {
      setFormToChuc(r)
      setFormCaNhan({ ...emptyForm, loai_kh: 'ca_nhan', ma_kh: dongChon.ma_kh } as FormState)
    } else {
      setFormCaNhan(r)
      setFormToChuc({ ...emptyForm, loai_kh: 'to_chuc', ma_kh: dongChon.ma_kh } as FormState)
    }
    setLoi('')
    setModalOpen('edit')
  }
  ...
  const moNhanBan = async () => {
    ...
    const cloned = { ...recordToForm(dongChon), ma_kh: newMa, ten_kh: dongChon.ten_kh + ' (bản sao)' }
    if (dongChon.loai_kh === 'to_chuc') {
  ...
  const moSuaDoubleClick = (row: KhachHangRecord) => {
    ...
    if (row.loai_kh === 'to_chuc') {
      setFormToChuc(r)
      setFormCaNhan({ ...emptyForm, loai_kh: 'ca_nhan', ma_kh: row.ma_kh })
    } else {
      setFormCaNhan(r)
      setFormToChuc({ ...emptyForm, loai_kh: 'to_chuc', ma_kh: row.ma_kh })
    }
    setLoi('')
    setModalOpen('edit')
  }
  ...
  const dongY = async () => {
    ...
    try {
      if (modalOpen === 'add' || modalOpen === 'clone') {
        ...
        await khachHangPut(dongChon.id, payload)
      }
      dongModal()
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 400 }} data-error-field={errorFieldTrung ?? ''}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: FORM_SECTION_GAP, flexWrap: 'wrap' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, cursor: 'pointer', marginLeft: 8 }}>
              <input type="checkbox" checked={form.isNhaCungCap} onChange={(e) => setForm((f) => ({ ...f, isNhaCungCap: e.target.checked }))} />
              Nhà cung cấp
            </label>
          </div>
          ...
          {formTab === 'khac' && form.loai_kh === 'ca_nhan' && (
            <>
              ...
              {/* Địa điểm nhận hàng - giống Tổ chức (gợi ý địa chỉ VN) */}
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: FORM_SECTION_GAP, marginBottom: FORM_SECTION_GAP }}>
                <div style={{ fontSize: 11, fontWeight: 600, marginBottom: FORM_ROW_GAP, color: 'var(--text-primary)' }}>Địa điểm nhận hàng</div>
                <div style={{ border: '1px solid var(--border)', borderRadius: 4, overflow: 'visible', background: 'var(--bg-tab)' }}>
                  {form.dia_diem_giao_hang.length === 0 ? (
