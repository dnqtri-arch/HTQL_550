...
import { ThemNhomKhNccModal as ThemNhomKhModal } from './themNhomKhNccModal'
import { useDraggable } from '../../../../hooks/useDraggable'

interface NhomKhachHangLookupModalProps {
  title?: string
  ...
  onSaveNewGroup?: (item: NhomKhachHangItem) => void
}
...
export function NhomKhachHangLookupModal({
  title = 'Chọn nhóm khách hàng, khách hàng',
  items: itemsProp,
  value = '',
  onSelect,
  onClose,
  onSaveNewGroup,
}: NhomKhachHangLookupModalProps) {
  ...
  const [showThemNhomKhachHang, setShowThemNhomKhachHang] = useState(false)
  ...
  const handleSaveNewGroup = (item: NhomKhachHangItem) => {
    onSaveNewGroup?.(item)
    setItems((prev) => [...prev, item])
    setSelectedTens((prev) => new Set([...prev, item.ten]))
    setShowThemNhomKhachHang(false)
  }
    ...
    >
      <div ref={containerRef} style={{ ...modalBox, ...containerStyle }} onMouseDown={() => { overlayMouseDownRef.current = false }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={footerStyle}>
          ...
          {onSaveNewGroup && (
            <button type="button" style={btnPrimary} onClick={() => setShowThemNhomKhachHang(true)}>
              <Plus size={14} />
      ...
      {showThemNhomKhachHang && onSaveNewGroup && (
        <ThemNhomKhModal
          parentOptions={items}
          onClose={() => setShowThemNhomKhachHang(false)}
          onSave={handleSaveNewGroup}
          onSaveAndAdd={(item: NhomKhachHangItem) => {
            onSaveNewGroup(item)
            setItems((prev) => [...prev, item])
            setSelectedTens((prev) => new Set([...prev, item.ten]))
          }}
