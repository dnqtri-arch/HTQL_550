...
function maDocNhat(baseMa: string, existingItems: NhomKhachHangItem[]): string {
...
interface ThemNhomKhNccModalProps {
  onClose: () => void
  onSave: (item: NhomKhachHangItem) => void
  onSaveAndAdd?: (item: NhomKhachHangItem) => void
  /** Danh sách nhóm có sẵn (cho dropdown Thuộc) */
  parentOptions?: NhomKhachHangItem[]
}
...
export function ThemNhomKhNccModal({
  ...
  const [members, setMembers] = useState<{ id: number; ma_kh: string; ten_kh: string; dia_chi?: string; ma_so_thue?: string; dien_thoai?: string }[]>([])
  const [showChonKhachHang, setShowChonKhachHang] = useState(false)
  const [searchKhNcc, setSearchKhNcc] = useState('')
  const [nccList, setNccList] = useState<KhachHangRecord[]>([])
  ...
  useEffect(() => {
    khachHangGetAll().then(setNccList)
  }, [])
  ...
  const addToMembers = (r: KhachHangRecord) => {
    setMembers((prev) => [
      ...prev,
      {
        id: r.id,
  ...
  const handleLuuVaTiepTuc = () => {
    const trimmedTen = ten.trim()
    if (!ma || !trimmedTen) return
    const item: NhomKhachHangItem = { ma, ten: trimmedTen }
    ...
    <div style={overlay}>
                    ...
                    {members.length === 0 ? null : (
                      members.map((m, idx) => (
                        <tr key={m.id}>
                          ...
                          <td style={tdTable}>{m.ma_kh}</td>
                          <td style={tdTable}>{m.ma_so_thue ?? ''}</td>
                          <td style={tdTable}>{m.ten_kh}</td>
      ...
      {showChonKhachHang && (
        <ChonKhachHangNccModal
          ...
          onSelect={(records: KhachHangRecord[]) => {
            setMembers((prev) => [
              ...prev,
              ...records.map((r) => ({
                id: r.id,
