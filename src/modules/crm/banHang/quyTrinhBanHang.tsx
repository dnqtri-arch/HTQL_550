 ...
 *   Col 3: Công nợ khách hàng (trên) [không có cặp dưới]; Row2: Hợp đồng bán; ngay dưới HĐ: Phụ lục HĐ bán
...
// 4 cột paired bắt đầu sau Báo giá — Phụ lục HĐ bán xếp dọc dưới Hợp đồng bán (cùng cột 0)
const COLSTART = BGX + BGWID + 48
const COLSTEP = NWID + COLG
const COL0 = COLSTART
const COL1 = COL0 + COLSTEP
const COL2 = COL1 + COLSTEP
const COL3 = COL2 + COLSTEP
const COLX = [COL0, COL1, COL2, COL3]
/** Hợp đồng bán (row 2) */
const ROW2_HD_Y = ROW2Y
/** Phụ lục HĐ bán — sát dưới node HĐ */
const ROW2_PL_Y = ROW2_HD_Y + NHID + 14
...
// Canvas dimensions (thêm chỗ cho Phụ lục HĐ bán dưới HĐ)
const DIAGRAMW = BCPX + NWID + PAD
const DIAGRAMH = ROW2_PL_Y + NHID + PAD
...
export function QuyTrinhBanHang({ onNavigate }: QuyTrinhBanHangProps) {
  ...
  const renderNode = (
    ...
    w = NWID,
    h = NHID,
    labelWeights?: { w1?: number; w2?: number },
  ) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: 'center', lineHeight: 1.35 }}>
          <div style={{ fontSize: 10.5, fontWeight: labelWeights?.w1 ?? 600, color: isHov ? T.conn : T.text }}>
            {label}
          </div>
          {label2 && (
            <div style={{ fontSize: 9.5, fontWeight: labelWeights?.w2 ?? 600, color: T.muted, marginTop: 1 }}>{label2}</div>
          )}
        </div>
      </button>
    )
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', width: DIAGRAMW, height: DIAGRAMH }}>
              ...
              >
                ...
                {/* ── Nối dọc HĐ bán → Phụ lục HĐ bán (vbot cột 0 đã nối backbone → đỉnh HĐ) ─ */}
                <line
                  x1={cx(0)} y1={ROW2_HD_Y + NHID}
                  x2={cx(0)} y2={ROW2_PL_Y}
                  stroke={T.conn} strokeWidth={1.5}
                />
              ...
              {renderNode('congno', 'Công nợ', 'khách hàng',
                CreditCard, CLR.green,
                COLX[3], ROW1Y, nav('congno'), NWID, NHID, { w1: 700, w2: 700 })}

              {/* ROW 2 — dưới backbone: HĐ bán; Phụ lục ngay dưới HĐ (YC83) */}
              {renderNode('hopdong', 'Hợp đồng bán', undefined,
                FileSignature, CLR.purple,
                COLX[0], ROW2_HD_Y, nav('hopdong'))}

              {renderNode('phuluchopdong', 'Phụ lục', 'HĐ bán',
                FileStack, CLR.purple,
                COLX[0], ROW2_PL_Y, nav('phuluchopdong'))}
