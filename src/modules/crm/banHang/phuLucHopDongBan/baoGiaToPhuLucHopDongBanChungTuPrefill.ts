/**
 * Map Báo giá → prefill form Hợp đồng bán (ChungTu) — module tách biệt, không import ngược từ baoGiaForm.
 */

import type { BaoGiaChiTiet, BaoGiaRecord } from '../../../../types/baoGia'
import type { PhuLucHopDongBanChungTuChiTiet, PhuLucHopDongBanChungTuRecord } from '../../../../types/phuLucHopDongBanChungTu'

const PREFILL_DON_HANG_BAN_ID = '__prefill_bao_gia__'

function mapAttachments(att: BaoGiaRecord['attachments']): PhuLucHopDongBanChungTuRecord['attachments'] {
  if (!att?.length) return undefined
  return att.map((a) => ({ ...a }))
}

/** Chuẩn bị header + chi tiết để mở form ĐHB mới từ một báo giá (đủ trường đồng dạng BG). */
export function buildPhuLucHopDongBanChungTuPrefillFromBaoGia(
  row: BaoGiaRecord,
  ...
  const prefillHdbCt: Partial<PhuLucHopDongBanChungTuRecord> = {
    ...
    ngay_lap_hop_dong: row.ngay_bao_gia,
  ...
  const prefillChiTiet: PhuLucHopDongBanChungTuChiTiet[] = chiTiet.map((c, i) => ({
    id: `prefill-ct-${i}`,
...
export function buildPhuLucHopDongBanChungTuPrefillCloneFromSaved(
  ...
  const prefillHdbCt: Partial<PhuLucHopDongBanChungTuRecord> = { ...rest, so_hop_dong: soHopDongTiepTheo }
  const prefillChiTiet: PhuLucHopDongBanChungTuChiTiet[] = chiTiet.map((c, i) => ({
    ...c,
    id: `prefill-clone-${i}`,
    hop_dong_ban_chung_tu_id: PREFILL_DON_HANG_BAN_ID,
