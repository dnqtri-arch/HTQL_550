...
export function PhuLucHopDongBanForm({ onClose, onSaved, onHeaderPointerDown, dragging, readOnly = false, initialHdbCt, initialChiTiet, prefillHdbCt, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: _formTitle, soDonLabel: soDonLabelProp, phieuNhanTuBaoGia = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false, onMoPhieuThuTuForm, onMoPhieuChiTuForm, choPhepMoPhieuThuTuForm = false, choPhepMoPhieuChiTuForm = false }: PhuLucHopDongBanFormProps) {
  ...
  useEffect(() => {
    ...
    setAttachments(chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(raw, so, khPart))
    setAttachmentsDirty(false)
  }, [initialHdbCt?.id])
  ...
  useEffect(() => {
    ...
    setAttachments((prev) => {
      ...
      const next = chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(prev, so, khPartDinhKem)
  ...
  useEffect(() => {
    ...
    if (rawAtt.length === 0) setAttachments([])
      ...
      const so = (initialHdbCt.so_hop_dong ?? '').trim() || 'BG'
      const khPart = partMccForPath('')
      setAttachments(chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(rawAtt, so, khPart))
    }
    setAttachmentsDirty(false)
