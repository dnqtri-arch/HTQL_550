...
import {
  ...
  type PhuLucHopDongBanChungTuCreatePayload,
  type PhuLucHopDongBanChungTuRecord,
  type PhuLucHopDongBanChungTuChiTiet,
  type PhuLucHopDongBanChungTuAttachmentItem,
...
import {
  ...
  TINH_TRANG_HOP_DONG_BAN_DA_THANH_LY,
  type HopDongBanChungTuRecord,
} from '../hopDongBan/hopDongBanChungTuApi'
import type { HopDongBanChungTuChiTiet } from '../../../../types/hopDongBanChungTu'
import { buildPhuLucHopDongBanChungTuPrefillFromHopDongBanGoc } from './hopDongBanToPhuLucHopDongBanChungTuPrefill'
...
import {
  PhuLucHopDongBanChungTuDinhKemModal,
  partMccForPath,
  type PhuLucHopDongBanChungTuDinhKemPendingRow,
} from './phuLucHopDongBanDinhKemModalFull'
import {
  chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu,
  maKhPathPartTuKhachHangVaDanhBa,
} from '../../../../utils/htqlThietKeDktkNaming'
import { htqlReconcileThietKeServerPaths } from '../../../../utils/htqlReconcileThietKeServerPaths'
import { usePhuLucHopDongBanChungTuApi } from './phuLucHopDongBanChungTuApiContext'
import { TINH_TRANG_NVTHH_DA_NHAP_KHO } from './phuLucHopDongBanChungTuApi'
...
import baoGiaChiTietStyles from './PhuLucHopDongBanChungTu.module.css'

type PhuLucHopDongBanChungTuGridLineRow = DonHangMuaGridLineRow & {
  'mD'?: string
  'mR'?: string
  'Lượng'?: string
}
      ...
      <DonHangBoxValueLikeNvMuaHang readOnly={readOnly} noTrailingPad fixedBandWidth={NVTHH_DON_HANG_BOX_VALUE_BAND_PX}>
        ...
        >
          <DatePicker
            ...
            className={`htql-datepicker-don-hang-box-full htql-datepicker-tg-nhan-input htql-datepicker-nvthh-band${!readOnly ? ' htql-datepicker-tg-nhan-editable' : ''}`}
...
export interface PhuLucHopDongBanFormProps {
  onClose: () => void
  onSaved?: () => void
  ...
  initialHdbCt?: PhuLucHopDongBanChungTuRecord | null
  initialChiTiet?: PhuLucHopDongBanChungTuChiTiet[] | null
  /** Dữ liệu đổ sẵn khi tạo mới (chuyển từ chứng từ khác), không phải chế độ xem */
  prefillHdbCt?: Partial<PhuLucHopDongBanChungTuRecord> | null
  prefillChiTiet?: PhuLucHopDongBanChungTuChiTiet[] | null
  /** Sau khi bấm Lưu (chỉ lưu): chuyển form sang chế độ xem đơn vừa lưu, không đóng form */
  onSavedAndView?: (don: PhuLucHopDongBanChungTuRecord) => void
  formTitle?: string
  soDonLabel?: string
...
const mauHdbCtCoDonGiaDisplay = [...mau_hdbctcodongia, 'Ghi chú'] as const
const mauHdbCtCoDonGiaKhongVatDisplay = [...mau_hdbctcodongia_khong_vat, 'Ghi chú'] as const
const mauHdbCtKhongDonGiaDisplay = [...mau_hdbctkhongdongia] as const

/** Bản ghi cũ không có `ap_dung_vat_gtgt` → coi như có VAT; form thêm mới không có bản ghi → mặc định không VAT (YC44). */
function deriveApDungVatGtgtTuBanGhi(r: PhuLucHopDongBanChungTuRecord | Partial<PhuLucHopDongBanChungTuRecord> | null | undefined): boolean {
  if (r == null) return false
  const v = (r as PhuLucHopDongBanChungTuRecord).ap_dung_vat_gtgt
  if (typeof v === 'boolean') return v
  return true
}

/** [YC37] Một dòng trống theo mẫu — Lượng mặc định 1. */
function createEmptyBaoGiaLine(mau: 'codongia' | 'khongdongia'): PhuLucHopDongBanChungTuGridLineRow {
  ...
  return acc as unknown as PhuLucHopDongBanChungTuGridLineRow
}

/** Chi tiết ĐHB → dòng lưới (SPHH + Đơn giá bán + kích thước). */
function chiTietToBaoGiaLines(ct: PhuLucHopDongBanChungTuChiTiet[]): PhuLucHopDongBanChungTuGridLineRow[] {
  ...
  return baseLines.map((line, i) => {
    ...
    return row as unknown as PhuLucHopDongBanChungTuGridLineRow
  })
}

/** Dòng trùng mã SPHH với HĐ bán gốc: ĐG + % thuế + tiền thuế theo HĐ (không lấy bậc giá VTHH). */
function applyGocHopDongBanPricingToLine(
  row: PhuLucHopDongBanChungTuGridLineRow,
  src: HopDongBanChungTuChiTiet,
  apVat: boolean,
): PhuLucHopDongBanChungTuGridLineRow {
  const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
  const donGia = Math.max(0, Number(src.don_gia ?? 0))
  const thanhTien = donGia * sl
  const ptNum =
    apVat && src.pt_thue_gtgt != null && Number.isFinite(Number(src.pt_thue_gtgt)) && Number(src.pt_thue_gtgt) > 0
      ? Number(src.pt_thue_gtgt)
      : null
  const tienThue = ptNum != null ? (thanhTien * ptNum) / 100 : 0
  const tong = thanhTien + tienThue
  return {
    ...row,
    'Đơn giá': formatSoTienHienThi(donGia),
    '% thuế GTGT': ptNum != null ? formatSoThapPhan(ptNum, 2) : '',
    'Thành tiền': formatSoTienHienThi(thanhTien),
    'Tiền thuế GTGT': ptNum != null ? formatSoTienHienThi(tienThue) : '',
    'Tổng tiền': formatSoTienHienThi(tong),
  } as unknown as PhuLucHopDongBanChungTuGridLineRow
}
...
function deriveHinhThucSelectedIdsFromRecord(
  d: Partial<PhuLucHopDongBanChungTuRecord> & { hinh_thuc?: string; dia_chi_cong_trinh?: string } | null | undefined
): string[] {
  if (!d) return []
...
function baoGiaListForDoiChieuNhanHang(): PhuLucHopDongBanChungTuRecord[] {
  const all = phuLucHopDongBanChungTuGetAll({ ...getDefaultPhuLucHopDongBanChungTuFilter(), tu: '', den: '' })
  return all.filter((bg) => bg.tinh_trang !== 'Hủy bỏ')
}

/** Hiển thị ô «Nhận hàng từ»: `mã BG: tiêu đề` (tiêu đề = so_chung_tu_cukcuk). */
function formatNhanHangTuTuBaoGia(bg: PhuLucHopDongBanChungTuRecord): string {
...
function baoGiaListChoPhieuNhanTuBaoGia(): PhuLucHopDongBanChungTuRecord[] {
  const all = phuLucHopDongBanChungTuGetAll({ ...getDefaultPhuLucHopDongBanChungTuFilter(), tu: '', den: '' })
...
function mauKhongDonGiaLines(): PhuLucHopDongBanChungTuGridLineRow[] {
  return [createEmptyBaoGiaLine('khongdongia')]
}

/** Mẫu có đơn giá — [YC37] mặc định 1 dòng. */
function mauCoDonGiaLines(): PhuLucHopDongBanChungTuGridLineRow[] {
  return [createEmptyBaoGiaLine('codongia')]
}

export function PhuLucHopDongBanForm({ onClose, onSaved, onHeaderPointerDown, dragging, readOnly = false, initialHdbCt, initialChiTiet, prefillHdbCt, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: _formTitle, soDonLabel: soDonLabelProp, phieuNhanTuBaoGia = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false, onMoPhieuThuTuForm, onMoPhieuChiTuForm, choPhepMoPhieuThuTuForm = false, choPhepMoPhieuChiTuForm = false }: PhuLucHopDongBanFormProps) {
  const api = usePhuLucHopDongBanChungTuApi()
  ...
  /** Thêm PL mới: chọn HĐ bán gốc (trừ «Đã thanh lý»), không lập từ báo giá trên ô này. */
  const laMoDuocChonHopDongGoc = !phieuNhanTuBaoGia && initialHdbCt == null && !effectiveReadOnly && prefillHdbCt == null
  ...
  const [attachments, setAttachments] = useState<PhuLucHopDongBanChungTuAttachmentItem[]>(() => {
  ...
  const [dktkPendingUploadRows, setDktkPendingUploadRows] = useState<PhuLucHopDongBanChungTuDinhKemPendingRow[]>([])
  const [phieuChiDktkPendingRows, setPhieuChiDktkPendingRows] = useState<PhuLucHopDongBanChungTuDinhKemPendingRow[]>([])
  const patchAttachmentsFromUser = useCallback((next: SetStateAction<PhuLucHopDongBanChungTuAttachmentItem[]>) => {
    setAttachments(next)
  ...
  const [diaChiNhanHangJoined, setDiaChiNhanHangJoined] = useState(() => {
    if (isViewMode && initialHdbCt) return initialHdbCt.dia_chi_nhan_hang ?? ''
    if (prefillHdbCt?.dia_chi_nhan_hang != null) return prefillHdbCt.dia_chi_nhan_hang
    return ''
  })
  ...
  const [hinhThucSelectedIds, setHinhThucSelectedIds] = useState<string[]>(() => {
    const fromRec = deriveHinhThucSelectedIdsFromRecord((initialHdbCt ?? prefillHdbCt) as PhuLucHopDongBanChungTuRecord | null | undefined)
  ...
  const [thamChieu, setThamChieu] = useState(() => {
    ...
    if (prefillHdbCt && phieuNhanTuBaoGia) {
      const pd = prefillHdbCt as PhuLucHopDongBanChungTuRecord
  ...
  const [tinhTrang, setTinhTrang] = useState(() => {
    if (isViewMode && initialHdbCt) return normalizeTinhTrangPhieuNvthhForForm(initialHdbCt.tinh_trang, laPhieuNhanNvthh)
    if (prefillHdbCt?.tinh_trang != null) return normalizeTinhTrangPhieuNvthhForForm(prefillHdbCt.tinh_trang, laPhieuNhanNvthh)
    return normalizeTinhTrangPhieuNvthhForForm(TINH_TRANG_HOP_DONG_BAN_CT[0], laPhieuNhanNvthh)
  })
  ...
  const [phieuChiNgay, setPhieuChiNgay] = useState<Date | null>(() => {
    const r = (initialHdbCt ?? prefillHdbCt) as PhuLucHopDongBanChungTuRecord | undefined
  ...
  const [gocHopDongLinkId, setGocHopDongLinkId] = useState<string | null>(() => {
    const i = (initialHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim()
    const p = (prefillHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim()
    return i || p || null
  })
  ...
  const [lines, setLines] = useState<PhuLucHopDongBanChungTuGridLineRow[]>(() => {
  ...
  const gocRecord = useMemo(() => {
    if (!gocHopDongLinkId) return null
    return hopDongBanChungTuGetAll({ ky: 'tat-ca', tu: '', den: '' }).find((d) => d.id === gocHopDongLinkId) ?? null
  }, [gocHopDongLinkId])

  const gocChiTietByMa = useMemo(() => {
    const m = new Map<string, HopDongBanChungTuChiTiet>()
    if (!gocHopDongLinkId) return m
    for (const c of hopDongBanChungTuGetChiTiet(gocHopDongLinkId)) {
      const ma = (c.ma_hang ?? '').trim().toUpperCase()
      if (ma) m.set(ma, c)
    }
    return m
  }, [gocHopDongLinkId])

  const apDungVatGtgt = useMemo(() => {
    if (gocRecord) return deriveApDungVatGtgtTuBanGhi(gocRecord)
    return deriveApDungVatGtgtTuBanGhi((initialHdbCt ?? prefillHdbCt) as PhuLucHopDongBanChungTuRecord | null)
  }, [gocRecord, initialHdbCt, prefillHdbCt])

  const hopDongNguyenTac = useMemo(() => {
    if (gocRecord) return Boolean(gocRecord.hop_dong_nguyen_tac)
    return Boolean((initialHdbCt ?? prefillHdbCt)?.hop_dong_nguyen_tac)
  }, [gocRecord, initialHdbCt, prefillHdbCt])
  ...
  useEffect(() => {
    ...
    setLines((prev) => {
      ...
      const next = prev.map((row) => {
        ...
        if (String(cur) !== raw) {
          changed = true
          return { ...row, [BAN_HANG_COL_DCNH]: String(cur) } as unknown as PhuLucHopDongBanChungTuGridLineRow
        }
        if (!(BAN_HANG_COL_DCNH in r) || (r[BAN_HANG_COL_DCNH] ?? '').trim() === '') {
          changed = true
          return { ...row, [BAN_HANG_COL_DCNH]: '0' } as unknown as PhuLucHopDongBanChungTuGridLineRow
        }
        return row
      })
  ...
  const loaiKhachHangHienThi = useMemo((): 'ca_nhan' | 'doanh_nghiep' | null => {
    ...
    const saved = (initialHdbCt ?? prefillHdbCt) as PhuLucHopDongBanChungTuRecord | undefined
    if (saved?.loai_khach_hang === 'ca_nhan' || saved?.loai_khach_hang === 'doanh_nghiep') return saved.loai_khach_hang
    return null
  }, [khachHangId, khList, khachHangDisplay, initialHdbCt?.loai_khach_hang, (prefillHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.loai_khach_hang])
  ...
  const [phieuChiAttachments, setPhieuChiAttachments] = useState<PhuLucHopDongBanChungTuAttachmentItem[]>(() => {
    const r = (initialHdbCt ?? prefillHdbCt) as PhuLucHopDongBanChungTuRecord | undefined
    const a = r?.phieu_chi_attachments
    return Array.isArray(a) && a.length ? a.map((x) => ({ ...x })) : []
  })
  const patchPhieuChiAttachmentsFromUser = useCallback((next: SetStateAction<PhuLucHopDongBanChungTuAttachmentItem[]>) => {
  ...
  // const [viewPhuLucHopDongBanChungTuRecord, setViewPhuLucHopDongBanChungTuRecord] = useState<PhuLucHopDongBanChungTuRecord | null>(null)
  ...
  useEffect(() => {
    ...
    if (prefillHdbCt) {
      ...
      if (prefillHdbCt.dia_chi_nhan_hang != null) setDiaChiNhanHangJoined(prefillHdbCt.dia_chi_nhan_hang)
      if (phieuNhanTuBaoGia) {
        const pd = prefillHdbCt as PhuLucHopDongBanChungTuRecord
      ...
      const ext = prefillHdbCt as PhuLucHopDongBanChungTuRecord
  ...
  const applyChonBaoGiaDoiChieu = useCallback(
    (bg: PhuLucHopDongBanChungTuRecord) => {
      ...
      setDiaChiCongTrinh((bg as PhuLucHopDongBanChungTuRecord & { dia_chi_cong_trinh?: string }).dia_chi_cong_trinh ?? '')
      ...
      setDiaChiNhanHangJoined((bg as PhuLucHopDongBanChungTuRecord & { dia_chi_nhan_hang?: string }).dia_chi_nhan_hang ?? '')
      setNgayDonHang(parseIsoToDate(bg.ngay_lap_hop_dong))
      setNgayGiaoHang(parseIsoToDate(bg.ngay_cam_ket_giao ?? null))
      const ct = phuLucHopDongBanChungTuGetChiTiet(bg.id)
      setLines(chiTietToBaoGiaLines(ct))
    },
    [phieuNhanTuBaoGia, khList, chungTuMuaPttt],
  )

  const applyChonHopDongBanGoc = useCallback(
    (hdb: HopDongBanChungTuRecord) => {
      const so = (soDonHang || api.soHopDongTiepTheo()).trim()
      const { prefillHdbCt: pd } = buildPhuLucHopDongBanChungTuPrefillFromHopDongBanGoc(hdb, so)
      setGocHopDongLinkId(hdb.id)
      if ((hdb.bao_gia_id ?? '').trim()) {
        baoGiaNguonPickRef.current = {
          bao_gia_id: hdb.bao_gia_id!,
          so_bao_gia_goc: (hdb.so_bao_gia_goc ?? '').trim() || undefined,
        }
        setDktkBaoGiaPickTick((t) => t + 1)
      } else {
        baoGiaNguonPickRef.current = null
      }
      ...
      setTinhTrang(normalizeTinhTrangPhieuNvthhForForm(TINH_TRANG_HOP_DONG_BAN_CT[0], laPhieuNhanNvthh))
      if (pd.ngay_lap_hop_dong != null) setNgayDonHang(parseIsoToDate(pd.ngay_lap_hop_dong))
      if (pd.ngay_cam_ket_giao !== undefined) setNgayGiaoHang(parseIsoToDate(pd.ngay_cam_ket_giao ?? null))
      const ext = pd as PhuLucHopDongBanChungTuRecord
      setHinhThucSelectedIds(deriveHinhThucSelectedIdsFromRecord(ext))
      if (ext.dia_chi_cong_trinh != null) setDiaChiCongTrinh(ext.dia_chi_cong_trinh)
      setAttachments([])
      ...
      const ctGoc = hopDongBanChungTuGetChiTiet(hdb.id)
      const coDonGia = ctGoc.some(
        (c) => (c.don_gia != null && Number(c.don_gia) > 0) || Number(c.thanh_tien ?? 0) > 0,
      )
    ...
    [soDonHang, api, laPhieuNhanNvthh, khList],
  )

  const hopDongBanListChoPhuLucGoc = useMemo(
    () =>
      hopDongBanChungTuGetAll({ ky: 'tat-ca', tu: '', den: '' }).filter(
        (h) => (h.tinh_trang ?? '').trim() !== TINH_TRANG_HOP_DONG_BAN_DA_THANH_LY,
      ),
    [],
  )
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      if ((e.target as HTMLElement).closest('[data-hop-dong-goc-pick-dropdown]')) return
      setBaoGiaPickOpen(false)
    }
    window.addEventListener('mousedown', onMouseDown)
    return () => window.removeEventListener('mousedown', onMouseDown)
  ...
  useEffect(() => {
    if (initialHdbCt != null || prefillHdbCt != null) {
      const idGoc =
        (initialHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim() ||
        (prefillHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim() ||
        null
      setGocHopDongLinkId(idGoc)
      baoGiaNguonPickRef.current = null
    }
  }, [initialHdbCt?.id, prefillHdbCt])

  /** Đồng giá theo HĐ gốc khi có liên kết — không chạy ở chế độ chỉ xem (tránh ghi đè chi tiết đã lưu). */
  useEffect(() => {
    if (!gocHopDongLinkId || effectiveReadOnly) return
    const map = new Map<string, HopDongBanChungTuChiTiet>()
    for (const c of hopDongBanChungTuGetChiTiet(gocHopDongLinkId)) {
      const ma = (c.ma_hang ?? '').trim().toUpperCase()
      if (ma) map.set(ma, c)
    }
    ...
    setLines((prev) =>
      prev.map((line) => {
        const ma = (line['Mã'] ?? '').trim().toUpperCase()
        if (!ma) return line
        const src = map.get(ma)
        if (!src) return line
        return applyGocHopDongBanPricingToLine(line, src, apDungVatGtgt)
      }),
    )
  }, [gocHopDongLinkId, apDungVatGtgt, effectiveReadOnly])
  ...
  useEffect(() => {
    ...
    setAttachments(chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(raw, so, khPart, { soBaoGiaGoc: soBg }))
    setAttachmentsDirty(false)
  ...
  useEffect(() => {
    ...
    setAttachments((prev) => {
      ...
      const next = chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(prev, so, khPartDinhKem, { soBaoGiaGoc: soBg })
      const same = next.every(
        (n: PhuLucHopDongBanChungTuAttachmentItem, i: number) => n.virtual_path === prev[i]?.virtual_path && n.name === (prev[i]?.name ?? '')
      )
  ...
  useEffect(() => {
    ...
    const d = initialHdbCt as PhuLucHopDongBanChungTuRecord & { hinh_thuc?: string; dia_chi_cong_trinh?: string }
    ...
    setDiaChiNhanHangJoined(initialHdbCt.dia_chi_nhan_hang ?? '')
    ...
    if (phieuNhanTuBaoGia && initialHdbCt.doi_chieu_don_mua_id) {
      const linked = phuLucHopDongBanChungTuGetAll({ ...getDefaultPhuLucHopDongBanChungTuFilter(), tu: '', den: '' }).find((x) => x.id === initialHdbCt.doi_chieu_don_mua_id)
      setThamChieu(
    ...
    if (rawAtt.length === 0) setAttachments([])
      ...
      const khPart = maKhPathPartTuKhachHangVaDanhBa(initialHdbCt.khach_hang, khList)
      const soBg = (initialHdbCt.so_bao_gia_goc ?? '').trim() || undefined
      setAttachments(chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(rawAtt, so, khPart, { soBaoGiaGoc: soBg }))
    }
    setAttachmentsDirty(false)
  ...
  /** Prefill chi tiết (vd. từ BG): tra cứu VTHH — sau đó dòng trùng HĐ gốc phải giữ ĐG/% thuế/tiền thuế theo HĐ, không theo bậc giá SPHH. */
  useEffect(() => {
    ...
    setLines((prev) => {
      let next = enrichDonHangBanGridLinesWithVthh(prev, vatTuList)
      if (gocHopDongLinkId) {
        next = next.map((line) => {
          const ma = ((line['Mã'] as string) ?? '').trim().toUpperCase()
          const src = ma ? gocChiTietByMa.get(ma) : undefined
          if (!src) return line
          return applyGocHopDongBanPricingToLine(
            line as unknown as PhuLucHopDongBanChungTuGridLineRow,
            src,
            apDungVatGtgt,
          )
        }) as typeof next
  ...
  }, [vatTuList, prefillChiTiet, gocHopDongLinkId, gocChiTietByMa, apDungVatGtgt])
  ...
  const syncDonGiaTheoBacVaSl = (row: PhuLucHopDongBanChungTuGridLineRow): PhuLucHopDongBanChungTuGridLineRow => {
    if (!(initialHdbCt != null || mauHienTai === 'codongia')) return row
    const ma = (row['Mã'] ?? '').trim().toUpperCase()
    const gocLine = ma ? gocChiTietByMa.get(ma) : undefined
    if (gocLine) return applyGocHopDongBanPricingToLine(row, gocLine, apDungVatGtgt)
    if (!row._vthh) return row
    const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
    const dvt = (row['ĐVT'] ?? '').trim() || (row._vthh.dvt_chinh ?? '')
    return { ...row, 'Đơn giá': getDonGiaBanDonHangBanLine(row._vthh, dvt, sl) } as unknown as PhuLucHopDongBanChungTuGridLineRow
  }
  ...
  const handleChonVthh = (vthh: VatTuHangHoaRecord, rowIndex: number) => {
    ...
    if (isCodongia) {
      const maKey = (vthh.ma ?? '').trim().toUpperCase()
      const gocLine = maKey ? gocChiTietByMa.get(maKey) : undefined
      if (gocLine) {
        const priced = applyGocHopDongBanPricingToLine(row as unknown as PhuLucHopDongBanChungTuGridLineRow, gocLine, apDungVatGtgt)
        Object.assign(row, priced)
      } else {
  ...
  const buildPayload = (): PhuLucHopDongBanChungTuCreatePayload => {
    const soBgPayload =
      (initialHdbCt?.so_bao_gia_goc ?? prefillHdbCt?.so_bao_gia_goc ?? baoGiaNguonPickRef.current?.so_bao_gia_goc ?? '').trim() ||
      undefined
    const coBang =
    ...
    const payload: PhuLucHopDongBanChungTuCreatePayload = {
      ...
      hop_dong_nguyen_tac: hopDongNguyenTac,
      tl_ck: (() => {
      ...
      ...(initialHdbCt?.bao_gia_id != null ||
      prefillHdbCt?.bao_gia_id != null ||
      baoGiaNguonPickRef.current?.bao_gia_id != null
        ? {
            bao_gia_id:
              (initialHdbCt?.bao_gia_id ?? prefillHdbCt?.bao_gia_id ?? baoGiaNguonPickRef.current?.bao_gia_id) ||
              undefined,
            so_bao_gia_goc:
              (initialHdbCt?.so_bao_gia_goc ?? prefillHdbCt?.so_bao_gia_goc ?? baoGiaNguonPickRef.current?.so_bao_gia_goc) ||
              undefined,
          }
        : {}),
      ...((): { hop_dong_ban_chung_tu_goc_id?: string } => {
        const goc =
          (gocHopDongLinkId ?? '').trim() ||
          (initialHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim() ||
          (prefillHdbCt as PhuLucHopDongBanChungTuRecord | undefined)?.hop_dong_ban_chung_tu_goc_id?.trim()
        return goc ? { hop_dong_ban_chung_tu_goc_id: goc } : {}
      })(),
      ...(initialHdbCt
        ? {
            thu_tien_bang_id: initialHdbCt.thu_tien_bang_id,
            thu_tien_bang_ids:
              initialHdbCt.thu_tien_bang_ids?.length ? [...initialHdbCt.thu_tien_bang_ids] : undefined,
            tinh_trang_truoc_thu_tien: initialHdbCt.tinh_trang_truoc_thu_tien,
          }
        : {}),
      chiTiet,
      hinh_thuc:
      ...
      attachments:
        attachments.length > 0
          ? chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(attachments, soDonHang.trim() || 'DHM', khPartDinhKem, {
        ...
        ? {
            ...
            phieu_chi_attachments:
              phieuChiAttachments.length > 0
                ? chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(phieuChiAttachments, soDonHang.trim() || 'DHM', khPartDinhKem, {
                    soBaoGiaGoc: soBgPayload,
                  })
                : undefined,
          }
    ...
    if (!thamChieu.trim()) {
      return fail(
        ...
        laPhieuNhanNvthh
          ...
          : 'Vui lòng nhập PL HĐB thay đổi hoặc chọn HĐ bán (chưa thanh lý).',
  ...
  const handleLuuVaDong = async () => {
    ...
    try {
      ...
      if (attachments.length > 0) {
        const norm = chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(attachments, soD, khPartDinhKem, { soBaoGiaGoc: soBgOpt })
        const moved = await htqlReconcileThietKeServerPaths(attachments, norm)
        setAttachments(moved)
      }
      if (phieuChiAttachments.length > 0) {
        const normPc = chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(phieuChiAttachments, soD, khPartDinhKem, {
          soBaoGiaGoc: soBgOpt,
        })
  ...
  return (
    <div className={bgDetailStyles.htqlBanHangChungTuFormShell} style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, background: 'var(--bg-primary)' }}>
      ...
      <div style={toolbarWrap}>
        ...
        {!phieuNhanTuBaoGia && initialHdbCt?.id && (onMoPhieuThuTuForm || onMoPhieuChiTuForm) && (
          <div
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1 1 0', minHeight: 0, gap: 2, marginBottom: 2 }}>
          ...
          {(() => {
            ...
            <div style={{ ...fieldRowDyn, alignItems: 'center' }}>
                      <label style={labelStyle}>{phieuNhanTuBaoGia ? 'Nhận hàng từ:' : 'PL HĐB thay đổi'}</label>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center' }}>
                {effectiveReadOnly ? (
                  ...
                  <div
                    ref={laMoDuocChonHopDongGoc ? refBaoGiaPickWrap : undefined}
                    style={{
                      flex: 1,
                      minWidth: 0,
                      display: 'flex',
                      ...
                      position: laMoDuocChonHopDongGoc ? 'relative' : undefined,
                    }}
                  >
                      ...
                      <input
                        ...
                        placeholder={
                          phieuNhanTuBaoGia
                            ...
                            : laMoDuocChonHopDongGoc
                              ? 'Bấm chọn HĐ bán (không «Đã thanh lý»)…'
                              : 'Nhập PL HĐB thay đổi'
                        }
                      />
                      {laMoDuocChonHopDongGoc && (
            ...
            {laMoDuocChonHopDongGoc && baoGiaPickOpen && baoGiaPickRect
              ? ReactDOM.createPortal(
                  (() => {
                    const q = thamChieu.trim().toLowerCase()
                    const listPick = hopDongBanListChoPhuLucGoc.filter((hdb) => {
                      if (!q) return true
                      const so = (hdb.so_hop_dong ?? '').toLowerCase()
                      const tc = (hdb.so_chung_tu_cukcuk ?? '').toLowerCase()
                      const kh = (hdb.khach_hang ?? '').toLowerCase()
                      return so.includes(q) || tc.includes(q) || kh.includes(q)
                    })
                    return (
                      <div
                        data-hop-dong-goc-pick-dropdown
                        style={{
                          position: 'fixed',
                      ...
                      >
                        ...
                        >
                          <span>Số HĐ</span>
                          <span>Khách hàng / Tham chiếu</span>
                        </div>
                        {listPick.map((hdb) => (
                          <div
                            key={hdb.id}
                            ...
                            onMouseDown={(e) => {
                              e.preventDefault()
                              applyChonHopDongBanGoc(hdb)
                            }}
                          >
                            <span style={{ fontWeight: 600 }}>{(hdb.so_hop_dong ?? '').trim() || '—'}</span>
                            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={hdb.khach_hang ?? ''}>
                              {(hdb.so_chung_tu_cukcuk ?? '').trim()
                                ? `${(hdb.khach_hang ?? '').trim()} — ${(hdb.so_chung_tu_cukcuk ?? '').trim()}`
                                : (hdb.khach_hang ?? '').trim() || '—'}
                            </span>
                          </div>
                        ))}
                        {listPick.length === 0 && (
                          <div style={{ padding: 12, fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>
                            Không có hợp đồng bán phù hợp (đã lọc trừ «Đã thanh lý»).
                          </div>
                        )}
                      </div>
                    )
                      ...
                      <DonHangBoxValueLikeNvMuaHang readOnly={effectiveReadOnly} noTrailingPad>
                        ...
                        >
                          <DatePicker
                            ...
                            scrollableYearDropdown
                            placeholderText="HH:mm dd/mm/yyyy"
                            className={`htql-datepicker-don-hang-box-full htql-datepicker-tg-nhan-input${!effectiveReadOnly ? ' htql-datepicker-tg-nhan-editable' : ''}`}
                            calendarClassName="htql-datepicker-tg-nhan htql-datepicker-calendar"
                            customInput={
                              <DatePickerTgNhanInput
                    ...
                    lines.map((line, idx) => (
                      <tr key={idx} className={baoGiaChiTietStyles.chiTietDataRow}>
                        ...
                        {currentColumns.map((col) => {
                          ...
                          const maGocKey = ((line['Mã'] as string) ?? '').trim().toUpperCase()
                          const dongTheoHdGoc = maGocKey ? gocChiTietByMa.get(maGocKey) : undefined
                          const stickyTrai = cotChiTietLaCotTraiCoDinh(col)
                          return (
                          <td
                            key={col}
                          ...
                          >
                            {showColAsReadOnly ? (
                              ...
                              <div style={{ position: 'relative', width: '100%', display: 'inline-block' }}>
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const prev = r[idx] as PhuLucHopDongBanChungTuGridLineRow
                                    const nextRow = {
                                      ...prev,
                                      [col]: ma,
                                    ...
                                    if (cleared || !item) {
                                      ...
                                      if (isCodongia) {
                                        const maK = (ma ?? '').trim().toUpperCase()
                                        const gl = maK ? gocChiTietByMa.get(maK) : undefined
                                        if (gl) {
                                          Object.assign(nextRow, applyGocHopDongBanPricingToLine(nextRow, gl, apDungVatGtgt))
                                        } else {
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'Nội dung': e.target.value } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'ĐVT' ? (
                                ...
                                if (hasDvtDropdown) {
                                  return (
                                    <select
                                      ...
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = r[idx] as PhuLucHopDongBanChungTuGridLineRow
                                        const newDvt = e.target.value
                                        const updates = { ...row, 'ĐVT': newDvt } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                        updates['Số lượng'] = calculateSoLuongFromKichThuoc(updates)
                                        if ((initialHdbCt != null || mauHienTai === 'codongia') && row._vthh) {
                                          const maK = (row['Mã'] ?? '').trim().toUpperCase()
                                          const gl = maK ? gocChiTietByMa.get(maK) : undefined
                                          if (gl) {
                                            Object.assign(updates, applyGocHopDongBanPricingToLine(updates, gl, apDungVatGtgt))
                                          } else {
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updated = { ...r[idx], 'mD': raw } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                ...
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mD': '' } as unknown as PhuLucHopDongBanChungTuGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const raw = formatSoTien(normalizeKichThuocInput(e.target.value))
                                  const updated = { ...r[idx], 'mR': raw } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mR': '' } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                    row['Số lượng'] = calculateSoLuongFromKichThuoc(row)
                                    row = syncDonGiaTheoBacVaSl(row)
                                    r[idx] = row
                                    setLines(r)
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(e.target.value) } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(String(n)) } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  let row = { ...r[idx], [col]: val } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  if (initialHdbCt != null || mauHienTai === 'codongia') {
                                    const maK = (row['Mã'] ?? '').trim().toUpperCase()
                                    const gl = maK ? gocChiTietByMa.get(maK) : undefined
                                    if (gl) {
                                      row = applyGocHopDongBanPricingToLine(row, gl, apDungVatGtgt)
                                    } else if (row._vthh) {
                                ...
                                onBlur={() => {
                                  ...
                                  const r = [...lines]
                                  let next = { ...r[idx], [col]: formatSoTienHienThi(val) } as unknown as PhuLucHopDongBanChungTuGridLineRow
                              ...
                              dongTheoHdGoc ? (
                                <input
                                  ...
                                  style={{
                                    ...inputStyle,
                                    ...chiTietNumericInputStyle('Đơn giá'),
                                    width: '100%',
                                    boxSizing: 'border-box',
                                    border: '1px solid transparent',
                                    minHeight: 22,
                                  ...
                                  value={line['Đơn giá'] ?? ''}
                                  title="Đơn giá theo HĐ bán gốc (trùng mã SPHH)"
                                />
                              ) : (
                                <input
                                  type="text"
                                  ...
                                  onChange={(e) => {
                                    const r = [...lines]
                                    r[idx] = { ...r[idx], 'Đơn giá': formatSoTien(e.target.value) } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                    setLines(r)
                                  }}
                                />
                              )
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], [BAN_HANG_COL_DCNH]: e.target.value } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                  setLines(r)
                                }}
                              >
                              ...
                              dongTheoHdGoc ? (
                                <input
                                  ...
                                  style={{
                                    ...inputStyle,
                                    ...chiTietNumericInputStyle('% thuế GTGT'),
                                    width: '100%',
                                    minHeight: 22,
                                    height: 22,
                                    cursor: 'default',
                                  ...
                                  title="% thuế GTGT theo HĐ bán gốc (trùng mã SPHH)"
                                />
                              ) : (
                                <select
                                  className="misa-input-solo"
                                  ...
                                  onChange={(e) => {
                                    const r = [...lines]
                                    r[idx] = { ...r[idx], '% thuế GTGT': e.target.value } as unknown as PhuLucHopDongBanChungTuGridLineRow
                                    setLines(r)
                                  }}
                                >
                              ...
                              <input
                                ...
                                style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }}
                                value={line[col] ?? ''}
                                onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], [col]: e.target.value } as unknown as PhuLucHopDongBanChungTuGridLineRow; setLines(r) }}
                              />
                            )}
                          </td>
                        )
