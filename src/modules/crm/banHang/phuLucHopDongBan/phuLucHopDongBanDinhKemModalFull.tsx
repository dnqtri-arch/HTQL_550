...
import type { PhuLucHopDongBanChungTuAttachmentItem } from '../../../../types/phuLucHopDongBanChungTu'
import {
  parseSoPhuLucHopDongBanChungTuForAttachmentFile as parseSoDonHangBanChungTuForAttachmentFile,
  partMccForPath,
  ...
  chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu as chuanHoaPhuLucTuUtil,
...
/**
 * Tên file: mã phụ lục (`plhdb`) + TG tạo (ưu tiên) + chỉ số — không còn segment mã KH trong tên.
...
/** Dưới gốc thiết kế: mã_kh hoặc `_pending_dktk` / mã chứng từ */
...
export function chuanHoaDuongDanDinhKemPhuLucHopDongBanChungTu(
  items: PhuLucHopDongBanChungTuAttachmentItem[],
  soDonHangBanChungTu: string,
  maKhPathPart: string,
  options?: { soBaoGiaGoc?: string | null }
): PhuLucHopDongBanChungTuAttachmentItem[] {
  return chuanHoaPhuLucTuUtil(items, soDonHangBanChungTu, maKhPathPart, options)
}
...
export function duongDanHienThi(item: PhuLucHopDongBanChungTuAttachmentItem, soDonHangBanChungTu: string, maKhPathPart: string): string {
  const full = buildBgAttachmentFullVirtualPath(soDonHangBanChungTu, maKhPathPart, item.name)
  const thuMuc = duongDanThuMucLuuTru(full)
  if (thuMuc) return thuMuc
  return `${buildBgAttachmentFolderVirtualPath(soDonHangBanChungTu, maKhPathPart)}/`
}
...
export type PhuLucHopDongBanChungTuDinhKemPendingRow = {
  id: string
  ten: string
  sizeBytes: number
  phanTram: number
...
export type PhuLucHopDongBanChungTuDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: PhuLucHopDongBanChungTuAttachmentItem[]
  onChange: (next: PhuLucHopDongBanChungTuAttachmentItem[]) => void
  readOnly?: boolean
  soDonHangBanChungTu: string
   ...
   */
  pendingUploadRows?: PhuLucHopDongBanChungTuDinhKemPendingRow[]
  onPendingUploadRowsChange?: React.Dispatch<React.SetStateAction<PhuLucHopDongBanChungTuDinhKemPendingRow[]>>
}
...
export function PhuLucHopDongBanChungTuDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
  ...
  const [pendingTienTrinhLocal, setPendingTienTrinhLocal] = useState<PhuLucHopDongBanChungTuDinhKemPendingRow[]>([])
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const hangMoi: PhuLucHopDongBanChungTuAttachmentItem[] = []
      ...
      for (let i = 0; i < listXuLy.length; i++) {
        ...
        const tenGoc = buildBgAttachmentBaseName(soDonHangBanChungTu, maKhPathPart, ngayGiaoHang, ngayDonHangBanChungTu, chiSo)
      ...
      const withMeta: PhuLucHopDongBanChungTuAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
        virtual_path: buildBgAttachmentFullVirtualPath(soDonHangBanChungTu, maKhPathPart, x.name),
      }))
      onChange([...attachments, ...withMeta])
    },
    [attachments, onChange, readOnly, soDonHangBanChungTu, maKhPathPart, ngayGiaoHang, ngayDonHangBanChungTu, baoLoiDinhKem, xacNhanDefer]
  )
        ...
        {attachments.length === 0 && pendingTienTrinh.length === 0 ? (
          ...
          >
            {attachments.map((item, idx) => {
              ...
              const duongDan = duongDanHienThi(item, soDonHangBanChungTu, maKhPathPart)
              const anhNhoDuoc =
