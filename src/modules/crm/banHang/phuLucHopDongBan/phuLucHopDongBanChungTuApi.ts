...
import type {
  PhuLucHopDongBanChungTuAttachmentItem,
...
import type { PhuLucHopDongBanChungTuApi } from './phuLucHopDongBanChungTuApiContext'
...
const MOCK_CHI_TIET: PhuLucHopDongBanChungTuChiTiet[] = [
  {
    id: 'ct1',
    hop_dong_ban_chung_tu_id: 'plhdb_full1',
    ma_hang: 'VT00001',
...
const STORAGE_KEY_PHU_LUC_HOP_DONG_BAN_CT_LIST = 'htql_phu_luc_hop_dong_ban_chung_tu_list'
const STORAGE_KEY_CHI_TIET = 'htql_phu_luc_hop_dong_ban_chung_tu_chi_tiet'

function normalizePhuLucHopDongBanChungTu(d: Partial<PhuLucHopDongBanChungTuRecord> & { id: string; de_xuat_id?: string }): PhuLucHopDongBanChungTuRecord {
  ...
  return {
    ...
    attachments: Array.isArray((d as { attachments?: PhuLucHopDongBanChungTuAttachmentItem[] }).attachments)
      ? (d as { attachments: PhuLucHopDongBanChungTuAttachmentItem[] }).attachments
      : undefined,
    ...
    hop_dong_ban_chung_tu_goc_id: d.hop_dong_ban_chung_tu_goc_id,
...
function loadFromStorage(): { list: PhuLucHopDongBanChungTuRecord[]; chiTiet: PhuLucHopDongBanChungTuChiTiet[] } {
  try {
    ...
    if (Array.isArray(list) && Array.isArray(chiTiet)) {
      return { list: list.map((d: Partial<PhuLucHopDongBanChungTuRecord> & { id: string }) => normalizePhuLucHopDongBanChungTu(d)), chiTiet }
    }
  } catch {
    /* ignore */
  }
...
const STORAGE_KEY_DRAFT_PLHDB_CT = 'htql_phu_luc_hop_dong_ban_chung_tu_draft'
...
export function phuLucHopDongBanChungTuGetAll(filter: PhuLucHopDongBanChungTuFilter): PhuLucHopDongBanChungTuRecord[] {
  const { tu, den } = filter
  if (!tu || !den) return [..._phuLucHopDongBanChungTuList]
  return _phuLucHopDongBanChungTuList.filter((d) => {
    const ngay = d.ngay_lap_hop_dong
    return ngay >= tu && ngay <= den
  })
}

export function phuLucHopDongBanChungTuGetChiTiet(donHangBanId: string): PhuLucHopDongBanChungTuChiTiet[] {
...
export function phuLucHopDongBanThuTienBangIdsLinked(row: PhuLucHopDongBanChungTuRecord): string[] {
...
export function phuLucHopDongBanChiTienBangIdsLinked(row: PhuLucHopDongBanChungTuRecord): string[] {
...
export function phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(row: PhuLucHopDongBanChungTuRecord, ct: PhuLucHopDongBanChungTuChiTiet[]): PhuLucHopDongBanChungTuCreatePayload {
  return {
    ...
    hop_dong_ban_chung_tu_goc_id: row.hop_dong_ban_chung_tu_goc_id,
...
export function phuLucHopDongBanGanKetThuTienTuPhieu(phuLucId: string, thuTienBangId: string): void {
  const row = _phuLucHopDongBanChungTuList.find((d) => d.id === phuLucId)
  if (!row) return
  const ct = phuLucHopDongBanChungTuGetChiTiet(phuLucId)
  const tid = (thuTienBangId ?? '').trim()
  const nextIds = [...new Set([...phuLucHopDongBanThuTienBangIdsLinked(row), tid])].filter(Boolean)
  phuLucHopDongBanChungTuPut(phuLucId, {
    ...phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(row, ct),
    thu_tien_bang_id: tid,
    thu_tien_bang_ids: nextIds.length ? nextIds : undefined,
    tinh_trang_truoc_thu_tien: row.tinh_trang,
  })
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent(HTQL_PHU_LUC_HOP_DONG_BAN_LIST_REFRESH_EVENT))
  }
}

export function phuLucHopDongBanGanKetChiTienTuPhieu(phuLucId: string, chiTienBangId: string): void {
  const row = _phuLucHopDongBanChungTuList.find((d) => d.id === phuLucId)
  if (!row) return
  const ct = phuLucHopDongBanChungTuGetChiTiet(phuLucId)
  const cid = (chiTienBangId ?? '').trim()
  const nextIds = [...new Set([...phuLucHopDongBanChiTienBangIdsLinked(row), cid])].filter(Boolean)
  phuLucHopDongBanChungTuPut(phuLucId, {
    ...phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(row, ct),
...
export function phuLucHopDongBanKhiPhieuChiGhiSo(chiTienBangId: string): void {
  const id = (chiTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    if (!phuLucHopDongBanChiTienBangIdsLinked(d).includes(id)) continue
    const ct = phuLucHopDongBanChungTuGetChiTiet(d.id)
    phuLucHopDongBanChungTuPut(d.id, {
      ...phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(d, ct),
      chi_tien_bang_id: id,
...
export function phuLucHopDongBanKhiPhieuChiHuyGhiSo(chiTienBangId: string): void {
  const id = (chiTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    if (!phuLucHopDongBanChiTienBangIdsLinked(d).includes(id)) continue
...
export function phuLucHopDongBanQuetChiTienOrphanVaHoanTac(validChiTienIds: Set<string>): void {
  const orphans = new Set<string>()
  for (const d of _phuLucHopDongBanChungTuList) {
    for (const cid of phuLucHopDongBanChiTienBangIdsLinked(d)) {
      if (cid && !validChiTienIds.has(cid)) orphans.add(cid)
    }
  }
  for (const cid of orphans) {
    phuLucHopDongBanHoanTacKhiXoaChiTien(cid)
  }
}

export function phuLucHopDongBanHoanTacKhiXoaChiTien(chiTienBangId: string): void {
  const id = (chiTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    const ids = phuLucHopDongBanChiTienBangIdsLinked(d)
    ...
    const ct = phuLucHopDongBanChungTuGetChiTiet(d.id)
    const payload = phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(d, ct)
    phuLucHopDongBanChungTuPut(d.id, {
      ...payload,
...
export function phuLucHopDongBanKhiPhieuThuGhiSo(thuTienBangId: string): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    if (!phuLucHopDongBanThuTienBangIdsLinked(d).includes(id)) continue
    const ct = phuLucHopDongBanChungTuGetChiTiet(d.id)
    phuLucHopDongBanChungTuPut(d.id, {
      ...phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(d, ct),
      thu_tien_bang_id: id,
...
export function phuLucHopDongBanKhiPhieuThuHuyGhiSo(thuTienBangId: string): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    if (!phuLucHopDongBanThuTienBangIdsLinked(d).includes(id)) continue
...
export function phuLucHopDongBanQuetThuTienOrphanVaHoanTac(validThuTienIds: Set<string>): void {
  const orphans = new Set<string>()
  for (const d of _phuLucHopDongBanChungTuList) {
    for (const tid of phuLucHopDongBanThuTienBangIdsLinked(d)) {
      if (tid && !validThuTienIds.has(tid)) orphans.add(tid)
    }
  }
  for (const tid of orphans) {
    phuLucHopDongBanHoanTacKhiXoaThuTien(tid)
  }
}

export function phuLucHopDongBanHoanTacKhiXoaThuTien(thuTienBangId: string): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  for (const d of _phuLucHopDongBanChungTuList) {
    const ids = phuLucHopDongBanThuTienBangIdsLinked(d)
    if (!ids.includes(id)) continue
    const nextIds = ids.filter((x) => x !== id)
    const cleared = nextIds.length === 0
...
export function phuLucHopDongBanChungTuSetTinhTrang(donHangBanId: string, tinh_trang: string): void {
  const row = _phuLucHopDongBanChungTuList.find((d) => d.id === donHangBanId)
  if (!row) return
  const ct = phuLucHopDongBanChungTuGetChiTiet(donHangBanId)
  phuLucHopDongBanChungTuPut(donHangBanId, { ...phuLucHopDongBanChungTuBuildCreatePayloadFromRecord(row, ct), tinh_trang })
}

/** Xóa hợp đồng bán và chi tiết; đồng bộ hoàn tác trạng thái báo giá nếu hết liên kết (YC51). */
export function phuLucHopDongBanChungTuDelete(donHangBanId: string): void {
  const removed = _phuLucHopDongBanChungTuList.find((d) => d.id === donHangBanId)
  const baoGiaId = removed?.bao_gia_id
  _phuLucHopDongBanChungTuList = _phuLucHopDongBanChungTuList.filter((d) => d.id !== donHangBanId)
...
export async function phuLucHopDongBanChungTuPost(
  ...
  const phuLucHopDongBanChungTuRow: PhuLucHopDongBanChungTuRecord = {
    ...
    hop_dong_ban_chung_tu_goc_id: payload.hop_dong_ban_chung_tu_goc_id,
  ...
  _phuLucHopDongBanChungTuList.push(phuLucHopDongBanChungTuRow)
  ...
  saveToStorage()
  return phuLucHopDongBanChungTuRow
}

/** Cập nhật báo giá (xóa chi tiết cũ, ghi lại theo payload). */
export function phuLucHopDongBanChungTuPut(donHangBanId: string, payload: PhuLucHopDongBanChungTuCreatePayload): void {
  const idx = _phuLucHopDongBanChungTuList.findIndex((d) => d.id === donHangBanId)
  if (idx < 0) return
  _phuLucHopDongBanChungTuList[idx] = {
    id: donHangBanId,
    loai_khach_hang: payload.loai_khach_hang,
    ten_nguoi_lien_he: payload.ten_nguoi_lien_he,
    so_dien_thoai_lien_he: payload.so_dien_thoai_lien_he,
...
export function getDefaultPhuLucHopDongBanChungTuFilter(): PhuLucHopDongBanChungTuFilter {
...
/** Gói API mặc định cho `PhuLucHopDongBanChungTuApiProvider` (danh sách + form ĐHB đầy đủ). */
export const phuLucHopDongBanChungTuApiImpl: PhuLucHopDongBanChungTuApi = {
  getAll: phuLucHopDongBanChungTuGetAll,
  getChiTiet: phuLucHopDongBanChungTuGetChiTiet,
  delete: phuLucHopDongBanChungTuDelete,
  getDefaultFilter: getDefaultPhuLucHopDongBanChungTuFilter,
  getDateRangeForKy,
  KY_OPTIONS,
  post: phuLucHopDongBanChungTuPost,
  put: phuLucHopDongBanChungTuPut,
  soHopDongTiepTheo: phuLucHopDongBanSoHopDongTiepTheo,
  getDraft: getPhuLucHopDongBanChungTuDraft,
  setDraft: setPhuLucHopDongBanChungTuDraft,
  clearDraft: clearPhuLucHopDongBanChungTuDraft,
}
