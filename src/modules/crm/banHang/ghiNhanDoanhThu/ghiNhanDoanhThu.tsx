...
import {
  type GhiNhanDoanhThuRecord,
  type GhiNhanDoanhThuChiTiet,
  type GhiNhanDoanhThuFilter,
  KY_OPTIONS,
  ghiNhanDoanhThuGetAll,
  ghiNhanDoanhThuGetChiTiet,
  ghiNhanDoanhThuDelete,
  getDefaultGhiNhanDoanhThuFilter,
  getDateRangeForKy,
  ghiNhanDoanhThuPost,
  ghiNhanDoanhThuPut,
  getGhiNhanDoanhThuDraft,
  setGhiNhanDoanhThuDraft,
  clearGhiNhanDoanhThuDraft,
  ghiNhanDoanhThuSoDonHangTiepTheo,
  ghiNhanDoanhThuBuildCreatePayloadFromRecord,
...
} from '../../muaHang/donHangMua/donHangMuaApi'
import type { DonHangMuaApi } from '../../muaHang/donHangMua/donHangMuaApiContext'
import { DonHangMuaApiProvider } from '../../muaHang/donHangMua/donHangMuaApiContext'
import { DonHangMuaForm } from '../../muaHang/donHangMua/donHangMuaForm'
import {
  GhiNhanDoanhThuApiProvider,
  useGhiNhanDoanhThuApi,
...
import {
  ...
  buildPrefillDonHeaderTuDhb,
  mapDhbChiTietToMuaChiTiet,
...
const apiNvthh: GhiNhanDoanhThuApi = {
  getAll: ghiNhanDoanhThuGetAll,
  getChiTiet: ghiNhanDoanhThuGetChiTiet,
  delete: ghiNhanDoanhThuDelete,
  getDefaultFilter: getDefaultGhiNhanDoanhThuFilter,
  getDateRangeForKy,
  KY_OPTIONS,
  post: ghiNhanDoanhThuPost,
  put: ghiNhanDoanhThuPut,
  soDonHangTiepTheo: ghiNhanDoanhThuSoDonHangTiepTheo,
  getDraft: getGhiNhanDoanhThuDraft,
  setDraft: setGhiNhanDoanhThuDraft,
  clearDraft: clearGhiNhanDoanhThuDraft,
}

type GhiNhanDoanhThuProps = {
  prefillTuDonHangMua?: { don: DonHangMuaRecord; chiTiet: DonHangMuaChiTiet[] } | null
  onConsumedPrefillTuDonHangMua?: () => void
  prefillTuDonHangBan?: { don: DonHangBanChungTuRecord; chiTiet: DonHangBanChungTuChiTiet[] } | null
  onConsumedPrefillTuDonHangBan?: () => void
  /** Mở form xem phiếu (vd. từ Đơn hàng mua — phiếu liên kết). */
  xemPhieuNhanHangId?: string | null
  onConsumedXemPhieuNhanHang?: () => void
}

function GhiNhanDoanhThuContent({
  prefillTuDonHangMua,
  onConsumedPrefillTuDonHangMua,
  prefillTuDonHangBan,
  onConsumedPrefillTuDonHangBan,
  xemPhieuNhanHangId,
  onConsumedXemPhieuNhanHang,
}: GhiNhanDoanhThuProps) {
  const api = useGhiNhanDoanhThuApi()
  const [filter, setFilter] = useState<GhiNhanDoanhThuFilter>(api.getDefaultFilter)
  const [danhSach, setDanhSach] = useState<GhiNhanDoanhThuRecord[]>([])
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [chiTiet, setChiTiet] = useState<GhiNhanDoanhThuChiTiet[]>([])
  const [dropdownXuatKhau, setDropdownXuatKhau] = useState(false)
  const [showForm, setShowForm] = useState(false)
  const [viewDon, setViewDon] = useState<GhiNhanDoanhThuRecord | null>(null)
  const [addFormKey, setAddFormKey] = useState(0)
  const [deleteTarget, setDeleteTarget] = useState<GhiNhanDoanhThuRecord | null>(null)
  const [formPrefillTuDhm, setFormPrefillTuDhm] = useState<GhiNhanDoanhThuPrefillPayload | null>(null)
  ...
  const [contextMenu, setContextMenu] = useState<{ open: boolean; x: number; y: number; row: GhiNhanDoanhThuRecord | null }>({ open: false, x: 0, y: 0, row: null })
  ...
  const [phucHoiTargetNvthh, setPhucHoiTargetNvthh] = useState<GhiNhanDoanhThuRecord | null>(null)
  const [huyBoTargetNvthh, setHuyBoTargetNvthh] = useState<GhiNhanDoanhThuRecord | null>(null)
  ...
  const thucHienNhapHang = (ids: string[]) => {
    ids.forEach((id) => {
      ...
      if (row && row.tinh_trang !== TINH_TRANG_NVTHH_DA_NHAP_KHO) {
        ...
        const base = ghiNhanDoanhThuBuildCreatePayloadFromRecord(row, ct)
  ...
  const giaTriXuatDanhSachPhieu = (d: GhiNhanDoanhThuRecord, col: DataGridColumn<GhiNhanDoanhThuRecord>) => {
    const key = col.key as keyof GhiNhanDoanhThuRecord
    const v = d[key]
  ...
  const columnsDon: DataGridColumn<GhiNhanDoanhThuRecord>[] = [
  ...
  const columnsChiTiet: DataGridColumn<GhiNhanDoanhThuChiTiet>[] = [
    { key: 'stt' as keyof GhiNhanDoanhThuChiTiet, label: 'STT', width: 36, align: 'center', renderCell: (_v, _r, idx) => idx != null ? String(idx + 1) : '' },
    ...
    { key: 'tong_tien' as keyof GhiNhanDoanhThuChiTiet, label: 'Tổng tiền', width: 100, align: 'right', renderCell: (_v, row) => formatNumberDisplay((row.thanh_tien ?? 0) + (row.tien_thue_gtgt ?? 0), 0) },
  ...
  useEffect(() => {
    ...
    window.addEventListener(HTQL_GHI_NHAN_DOANH_THU_LIST_REFRESH_EVENT, onRemote)
    return () => window.removeEventListener(HTQL_GHI_NHAN_DOANH_THU_LIST_REFRESH_EVENT, onRemote)
  ...
  const consumePrefillDhbRef = useRef(onConsumedPrefillTuDonHangBan)
  consumePrefillDhbRef.current = onConsumedPrefillTuDonHangBan

  useEffect(() => {
    if (!prefillTuDonHangBan) return
    api.clearDraft()
    setViewDon(null)
    setFormPrefillTuDhm({
      don: buildPrefillDonHeaderTuDhb(prefillTuDonHangBan.don),
      chiTiet: mapDhbChiTietToMuaChiTiet(prefillTuDonHangBan.chiTiet, prefillTuDonHangBan.don.id),
    })
    setAddFormKey((k) => k + 1)
    setShowForm(true)
    consumePrefillDhbRef.current?.()
  }, [prefillTuDonHangBan, api])

  useEffect(() => {
    const id = xemPhieuNhanHangId?.trim()
    if (!id) return
    const all = api.getAll({ ...getDefaultGhiNhanDoanhThuFilter(), tu: '', den: '' })
  ...
  const sortedDanhSach = [...danhSach].sort((a, b) => {
    ...
    for (const s of effectiveSort) {
      const key = s.key as keyof GhiNhanDoanhThuRecord
      const va = a[key]
      const vb = b[key]
    ...
    <div className={styles.root}>
                ...
                onClick={() => {
                  ...
                  const dataRows = chiTiet.map((row, idx) =>
                    columnsChiTiet.map((col) => {
                      ...
                      const v = row[col.key as keyof GhiNhanDoanhThuChiTiet]
      ...
      <div className={styles.contentArea}>
        <div className={styles.gridWrap}>
          <DataGrid<GhiNhanDoanhThuRecord>
            columns={columnsDon}
        ...
        <div className={styles.detailWrap}>
          ...
          <div className={styles.gridWrap}>
            <DataGrid<GhiNhanDoanhThuChiTiet>
          ...
          <>
            ...
            <button
              ...
              onClick={() => {
                ...
                const base = ghiNhanDoanhThuBuildCreatePayloadFromRecord(phucHoiTargetNvthh, ct)
          ...
          <>
            ...
            <button
              ...
              onClick={() => {
                ...
                const base = ghiNhanDoanhThuBuildCreatePayloadFromRecord(huyBoTargetNvthh, ct)
      ...
      <GhiNhanDoanhThuFormModal
        ...
        onSavedAndView={(don) => {
          setViewDon(don as GhiNhanDoanhThuRecord)
          setFormPrefillTuDhm(null)
          loadData()
        }}
      />
...
export function GhiNhanDoanhThu(props: GhiNhanDoanhThuProps) {
  return (
    <GhiNhanDoanhThuApiProvider api={apiNvthh}>
      <GhiNhanDoanhThuContent {...props} />
    </GhiNhanDoanhThuApiProvider>
  )
}
