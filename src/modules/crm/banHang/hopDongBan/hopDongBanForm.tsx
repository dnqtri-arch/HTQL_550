...
function createEmptyBaoGiaLine(mau: 'codongia' | 'khongdongia'): HopDongBanChungTuGridLineRow {
  const cols = insertVariantColumns(mau === 'codongia' ? [...mauHdbCtCoDonGiaDisplay] : [...mauHdbCtKhongDonGiaDisplay])
...
function readKhoGiayChieuDaiByTenForHopDongBan(): Record<string, number> {
...
function buildKhoGiayOptionsForVthh(vthh?: VatTuHangHoaRecord | null): string[] | null {
  if (!vthh) return null
  const daiByTen = readKhoGiayChieuDaiByTenForHopDongBan()
  const set = new Set<string>()
  const matrix = Array.isArray(vthh.pricing_matrix) ? vthh.pricing_matrix : []
  matrix.forEach((row) => {
...
function donGiaContextFromHopDongBanLine(row: HopDongBanChungTuGridLineRow) {
...
export function HopDongBanForm({ onClose, onSaved, onHeaderPointerDown, dragging, readOnly = false, initialHdbCt, initialChiTiet, prefillHdbCt, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: _formTitle, soDonLabel: soDonLabelProp, phieuNhanTuBaoGia = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false, onMoPhieuThuTuForm, onMoPhieuChiTuForm, choPhepMoPhieuThuTuForm = false, choPhepMoPhieuChiTuForm = false }: HopDongBanFormProps) {
  ...
  const laMoDuocChonBaoGia = !phieuNhanTuBaoGia && initialHdbCt == null && !effectiveReadOnly && prefillHdbCt == null
  ...
  const [hopDongNguyenTac, setHopDongNguyenTac] = useState(() =>
    Boolean((initialHdbCt ?? prefillHdbCt)?.hop_dong_nguyen_tac),
  )
  ...
  useEffect(() => {
    ...
    setLines((prev) => {
      ...
      const next = prev.map((row) => {
        ...
        if (String(cur) !== raw) {
          changed = true
          return { ...row, [BAN_HANG_COL_DCNH]: String(cur) } as unknown as HopDongBanChungTuGridLineRow
        }
        if (!(BAN_HANG_COL_DCNH in r) || (r[BAN_HANG_COL_DCNH] ?? '').trim() === '') {
          changed = true
          return { ...row, [BAN_HANG_COL_DCNH]: '0' } as unknown as HopDongBanChungTuGridLineRow
        }
        return row
      })
  ...
  const applyChonBaoGiaDoiChieu = useCallback(
    (bg: HopDongBanChungTuRecord) => {
      ...
      setDiaChiNhanHangJoined((bg as HopDongBanChungTuRecord & { dia_chi_nhan_hang?: string }).dia_chi_nhan_hang ?? '')
  ...
  const applyChonBaoGiaTuBang = useCallback(
    (bg: BaoGiaRecord) => {
      ...
      const { prefillHdbCt: pd, prefillChiTiet: pct } = buildHopDongBanChungTuPrefillFromBaoGia(bg, ct, so)
      ...
      const ext = pd as HopDongBanChungTuRecord
      setHinhThucSelectedIds(deriveHinhThucSelectedIdsFromRecord(ext))
      if (ext.dia_chi_cong_trinh != null) setDiaChiCongTrinh(ext.dia_chi_cong_trinh)
      ...
      if (pd.tl_ck != null && typeof pd.tl_ck === 'number')
        setTlCkInput(pd.tl_ck === 0 ? '0' : formatSoThapPhan(pd.tl_ck, 3))
      if (pd.tien_ck != null && typeof pd.tien_ck === 'number') setTienCkInput(formatSoTienHienThi(pd.tien_ck))
      setApDungVatGtgt(deriveApDungVatGtgtTuBanGhi(pd as HopDongBanChungTuRecord))
      setHopDongNguyenTac(Boolean((pd as HopDongBanChungTuRecord).hop_dong_nguyen_tac))
  ...
  useEffect(() => {
    if (initialHdbCt != null || prefillHdbCt != null) baoGiaNguonPickRef.current = null
  ...
  useEffect(() => {
    ...
    setHopDongNguyenTac(Boolean(initialHdbCt.hop_dong_nguyen_tac))
  ...
  /** [YC30] Số lượng từ kích thước — đồng bộ Báo giá / VTHH (khi không khóa «nguyên tắc HĐ»). */
  const calculateSoLuongFromKichThuoc = (line: HopDongBanChungTuGridLineRow): string => {
    if (hopDongNguyenTac) return line['Số lượng'] ?? ''
    return tinhSoLuongChiTietTuKichThuocVaVthh(line, dvtList)
  }
  ...
  const syncDonGiaTheoBacVaSl = (row: HopDongBanChungTuGridLineRow): HopDongBanChungTuGridLineRow => {
    ...
    const next = { ...row, 'Đơn giá': getDonGiaBanDonHangBanLine(row._vthh, dvt, sl, donGiaContextFromHopDongBanLine(row)) } as unknown as HopDongBanChungTuGridLineRow
    const minB1 = getBanHangB1TtAdjustment(row._vthh, sl)
    if (minB1.ap_dung) {
      next['ĐVT'] = minB1.dvt_goi_y
      next['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, minB1.dvt_goi_y, sl, donGiaContextFromHopDongBanLine(next))
  ...
  const handleChonVthh = (vthh: VatTuHangHoaRecord, rowIndex: number) => {
    ...
    if (isCodongia) {
      ...
      const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
      row['Đơn giá'] = getDonGiaBanDonHangBanLine(vthh, (row['ĐVT'] ?? '').trim() || (vthh.dvt_chinh ?? ''), sl, donGiaContextFromHopDongBanLine(row))
      const minB1 = getBanHangB1TtAdjustment(vthh, sl)
      if (minB1.ap_dung) {
        row['ĐVT'] = minB1.dvt_goi_y
        row['Đơn giá'] = getDonGiaBanDonHangBanLine(vthh, minB1.dvt_goi_y, sl, donGiaContextFromHopDongBanLine(row))
  ...
  const currentColumns = useMemo((): readonly string[] => {
    if (!hienThiFooterTongTien) return insertVariantColumns(mauHdbCtKhongDonGiaDisplay as readonly string[])
    const base = insertVariantColumns((apDungVatGtgt ? mauHdbCtCoDonGiaDisplay : mauHdbCtCoDonGiaKhongVatDisplay) as readonly string[])
    if (hopDongNguyenTac) return base.filter((c) => c !== 'mD' && c !== 'mR' && c !== 'Lượng')
    return base
  }, [hienThiFooterTongTien, apDungVatGtgt, hopDongNguyenTac])
  ...
  /** HĐ nguyên tắc: bỏ cụm cột kích thước — chỉ còn một hàng tiêu đề cột chi tiết. */
  const coCotKichThuocBangHd = currentColumns.includes('mD')
                ...
                {hienThiFooterTongTien && (
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, flexShrink: 0, marginLeft: 8 }}>
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: effectiveReadOnly ? 'default' : 'pointer', whiteSpace: 'nowrap', color: 'var(--text-primary)' }}>
                      <input
                        type="checkbox"
                        checked={hopDongNguyenTac}
                        disabled={effectiveReadOnly}
                        onChange={(e) => setHopDongNguyenTac(e.target.checked)}
                      />
                      Hợp đồng nguyên tắc
                    </label>
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: effectiveReadOnly ? 'default' : 'pointer', whiteSpace: 'nowrap', color: 'var(--text-primary)' }}>
            ...
            >
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 11, tableLayout: 'auto', minWidth: tableMinWidth }}>
                ...
                <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                  ...
                  <tr>
                    <th
                      rowSpan={coCotKichThuocBangHd ? 2 : 1}
                      style={{
                        ...gridThStyle,
                    ...
                    {currentColumns.map((col) => {
                      ...
                      return (
                        <th
                          ...
                          rowSpan={isKichThuoc ? 1 : coCotKichThuocBangHd ? 2 : 1}
                          style={{
                            ...gridThStyle,
                  ...
                  {/* [YC32] Hàng 2: 3 cột con của Kích thước (ẩn khi HĐ nguyên tắc) */}
                  {coCotKichThuocBangHd ? (
                    <tr>
                            ...
                            {showColAsReadOnly ? (
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (cleared || !item) {
                                      ...
                                      if (isCodongia) {
                                        ...
                                        nextRow['Đơn giá'] = getDonGiaBanDonHangBanLine(item, (nextRow['ĐVT'] ?? '').trim() || (item.dvt_chinh ?? ''), sl, donGiaContextFromHopDongBanLine(nextRow))
                              ...
                              (() => {
                                ...
                                if (opts && opts.length > 0) {
                                  return (
                                    <select className="misa-input-solo" style={{ ...inputStyle, width: '100%', minHeight: 22, height: 22, cursor: 'pointer' }} value={line['Độ dày/ ĐL'] ?? opts[0]} onChange={(e) => { const r = [...lines]; const row = { ...r[idx], 'Độ dày/ ĐL': e.target.value } as unknown as HopDongBanChungTuGridLineRow; if ((initialHdbCt != null || mauHienTai === 'codongia') && row._vthh) { const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1; row['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, (row['ĐVT'] ?? '').trim() || (row._vthh.dvt_chinh ?? ''), sl, donGiaContextFromHopDongBanLine(row)) } r[idx] = row; setLines(r) }}>
                                      {opts.map((o) => <option key={o} value={o}>{o}</option>)}
                                    </select>
                                  )
                                }
                                return <input className="misa-input-solo" style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }} value={line['Độ dày/ ĐL'] ?? ''} onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], 'Độ dày/ ĐL': e.target.value } as unknown as HopDongBanChungTuGridLineRow; setLines(r) }} />
                              })()
                            ) : col === 'Khổ giấy' ? (
                              (() => {
                                ...
                                return <input className="misa-input-solo" style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }} value={line['Khổ giấy'] ?? ''} onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], 'Khổ giấy': e.target.value } as unknown as HopDongBanChungTuGridLineRow; setLines(r) }} />
                              })()
                            ) : col === 'Nội dung' ? (
                              <input
                                className="misa-input-solo"
                                    ...
                                    <select
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if ((initialHdbCt != null || mauHienTai === 'codongia') && row._vthh) {
                                          ...
                                          updates['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, newDvt, sl, donGiaContextFromHopDongBanLine(updates))
                                        }
                                        r[idx] = updates
                                        setLines(r)
                                      }}
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], [BAN_HANG_COL_DCNH]: e.target.value } as unknown as HopDongBanChungTuGridLineRow
                                  setLines(r)
                                }}
                              >
