...
import type { HopDongBanChungTuAttachmentItem } from '../../../../types/hopDongBanChungTu'
import {
  parseSoHopDongBanChungTuForAttachmentFile as parseSoDonHangBanChungTuForAttachmentFile,
  partMccForPath,
  ...
  chuanHoaDuongDanDinhKemHopDongBanChungTu as chuanHoaHopDongBanTuUtil,
...
/**
 * Tên file: mã HĐ bán (ASCII `hdb`) + TG tạo (ưu tiên) + chỉ số — không còn segment mã KH trong tên.
...
export function chuanHoaDuongDanDinhKemHopDongBanChungTu(
  items: HopDongBanChungTuAttachmentItem[],
  soDonHangBanChungTu: string,
  maKhPathPart: string,
  options?: { soBaoGiaGoc?: string | null }
): HopDongBanChungTuAttachmentItem[] {
  return chuanHoaHopDongBanTuUtil(items, soDonHangBanChungTu, maKhPathPart, options)
}
...
export function duongDanHienThi(item: HopDongBanChungTuAttachmentItem, soDonHangBanChungTu: string, maKhPathPart: string): string {
...
export type HopDongBanChungTuDinhKemPendingRow = {
  id: string
  ten: string
  sizeBytes: number
  phanTram: number
...
export type HopDongBanChungTuDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: HopDongBanChungTuAttachmentItem[]
  onChange: (next: HopDongBanChungTuAttachmentItem[]) => void
  readOnly?: boolean
  soDonHangBanChungTu: string
   ...
   */
  pendingUploadRows?: HopDongBanChungTuDinhKemPendingRow[]
  onPendingUploadRowsChange?: React.Dispatch<React.SetStateAction<HopDongBanChungTuDinhKemPendingRow[]>>
}
...
export function HopDongBanChungTuDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
...
}: HopDongBanChungTuDinhKemModalProps) {
  ...
  const [pendingTienTrinhLocal, setPendingTienTrinhLocal] = useState<HopDongBanChungTuDinhKemPendingRow[]>([])
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const hangMoi: HopDongBanChungTuAttachmentItem[] = []
      ...
      const withMeta: HopDongBanChungTuAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
