...
export function duongDanHienThi(item: DonHangBanChungTuAttachmentItem, soDonHangBanChungTu: string, maKhPathPart: string): string {
...
export type DonHangBanChungTuDinhKemPendingRow = {
  id: string
  ten: string
  sizeBytes: number
  phanTram: number
...
export type DonHangBanChungTuDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: DonHangBanChungTuAttachmentItem[]
  onChange: (next: DonHangBanChungTuAttachmentItem[]) => void
  readOnly?: boolean
  soDonHangBanChungTu: string
   ...
   */
  pendingUploadRows?: DonHangBanChungTuDinhKemPendingRow[]
  onPendingUploadRowsChange?: React.Dispatch<React.SetStateAction<DonHangBanChungTuDinhKemPendingRow[]>>
  /** Tiêu đề panel (mặc định: đính kèm chứng từ — nút «Đính kèm thiết kế» truyền nhãn riêng). */
  panelTitle?: string
...
export function DonHangBanChungTuDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
  ...
  const [pendingTienTrinhLocal, setPendingTienTrinhLocal] = useState<DonHangBanChungTuDinhKemPendingRow[]>([])
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const hangMoi: DonHangBanChungTuAttachmentItem[] = []
      ...
      const withMeta: DonHangBanChungTuAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
  ...
  const panel = open ? (
    ...
    >
      ...
      >
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-primary)', flexShrink: 0 }}>{panelTitle}</span>
