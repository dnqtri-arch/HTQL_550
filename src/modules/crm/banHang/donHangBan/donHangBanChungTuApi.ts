...
import type { DonHangBanChungTuApi } from './donHangBanChungTuApiContext'
...
function normalizeDonHangBanChungTu(d: Partial<DonHangBanChungTuRecord> & { id: string; de_xuat_id?: string }): DonHangBanChungTuRecord {
  ...
  return {
    ...
    attachments: Array.isArray((d as { attachments?: DonHangBanChungTuAttachmentItem[] }).attachments)
      ? (d as { attachments: DonHangBanChungTuAttachmentItem[] }).attachments
      : undefined,
...
export function donHangBanGetChiTiet(donHangBanId: string): DonHangBanChungTuChiTiet[] {
  return _chiTietList.filter((c) => c.don_hang_ban_id === donHangBanId)
}

/** [YC30] Trạng thái báo giá - 5 options mới */
export const TINH_TRANG_DON_HANG_BAN_CT = [
  'Mới tạo',
  'Đã gửi KH',
  'Đã chuyển ĐHB',
  'Đã chuyển HĐ',
...
export const TINH_TRANG_DON_HANG_BAN_CT_DA_GUI_KHACH = 'Đã gửi KH'
...
export const TINH_TRANG_DON_HANG_BAN_DA_NHAN_HANG = 'Đã nhận hàng'
...
/**
 * ĐHB có liên kết phiếu thu đã không còn trong CSDL — gọi lại luồng hoàn tác theo từng id phiếu (YC68).
 */
...
/** Khi xóa phiếu thu: hoàn tác trạng thái ĐHB đã liên kết. */
export function donHangBanHoanTacKhiXoaThuTien(thuTienBangId: string): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  for (const d of _donHangBanList) {
...
export function donHangBanSetTinhTrang(donHangBanId: string, tinh_trang: string): void {
  const row = _donHangBanList.find((d) => d.id === donHangBanId)
  if (!row) return
  const ct = donHangBanGetChiTiet(donHangBanId)
  donHangBanPut(donHangBanId, { ...donHangBanBuildCreatePayloadFromRecord(row, ct), tinh_trang })
}

/** Xóa đơn hàng bán và chi tiết; đồng bộ hoàn tác trạng thái báo giá nếu hết liên kết (YC51). */
export function donHangBanDelete(donHangBanId: string): void {
  const removed = _donHangBanList.find((d) => d.id === donHangBanId)
...
export function donHangBanPut(donHangBanId: string, payload: DonHangBanChungTuCreatePayload): void {
  const idx = _donHangBanList.findIndex((d) => d.id === donHangBanId)
  if (idx < 0) return
  _donHangBanList[idx] = {
    id: donHangBanId,
    loai_khach_hang: payload.loai_khach_hang,
...
/** Gói API mặc định cho `DonHangBanChungTuApiProvider` (danh sách + form ĐHB đầy đủ). */
export const donHangBanChungTuApiImpl: DonHangBanChungTuApi = {
  getAll: donHangBanGetAll,
  getChiTiet: donHangBanGetChiTiet,
  delete: donHangBanDelete,
  getDefaultFilter: getDefaultDonHangBanChungTuFilter,
  getDateRangeForKy,
  KY_OPTIONS,
  post: donHangBanPost,
  put: donHangBanPut,
  soDonHangTiepTheo: donHangBanSoDonHangTiepTheo,
  getDraft: getDonHangBanChungTuDraft,
  setDraft: setDonHangBanChungTuDraft,
  clearDraft: clearDonHangBanChungTuDraft,
}
