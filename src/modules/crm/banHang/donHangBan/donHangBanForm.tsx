...
import { mau_dhbkhongdongia, mau_dhbcodongia, mau_dhbcodongia_khong_vat } from './donHangBanGridMauFull'
import {
  formatSoNguyenInput,
  formatNumberDisplay,
  formatSoTien,
...
import {
  ...
  type DonHangBanChungTuCreatePayload,
...
import {
  ...
  chuanHoaDuongDanDinhKemDonHangBanChungTu,
  partMccForPath,
...
type DonHangBanGridLineRow = DonHangMuaGridLineRow & {
  'mD'?: string
  'mR'?: string
  'Lượng'?: string
}
...
/** Khối Đơn hàng bán / Chứng từ / Hóa đơn (phiếu NVTHH): đồng bộ nhãn với hàng NV mua hàng */
...
export interface DonHangBanChungTuFormProps {
  onClose: () => void
  onSaved?: () => void
  ...
  initialDhb?: DonHangBanChungTuRecord | null
  initialChiTiet?: DonHangBanChungTuChiTiet[] | null
  /** Dữ liệu đổ sẵn khi tạo mới (chuyển từ chứng từ khác), không phải chế độ xem */
  prefillDhb?: Partial<DonHangBanChungTuRecord> | null
  prefillChiTiet?: DonHangBanChungTuChiTiet[] | null
  /** Sau khi bấm Lưu (chỉ lưu): chuyển form sang chế độ xem đơn vừa lưu, không đóng form */
  onSavedAndView?: (don: DonHangBanChungTuRecord) => void
  formTitle?: string
  soDonLabel?: string
  // [BaoGia] Bỏ logic đối chiếu đơn mua - không áp dụng cho Đơn hàng bán
  // doiChieuSource?: 'don_mua_hang'
  /** Tạo phiếu nhận từ menu BG: prefill tiêu đề/HT/kho/ĐĐNH từ ĐHB (HT/kho/ĐĐNH cho phép sửa lại); lưới chỉ sửa Số lượng; TG tạo = hiện tại; tiêu đề form = `_tieu_de_nguon_dhm`. */
...
function normalizeTinhTrangPhieuNvthhForForm(tinh: string | undefined, laPhieuNhan: boolean): string {
  ...
  return base === TINH_TRANG_DON_HANG_BAN_CT_DA_GUI_KHACH ? TINH_TRANG_NVTHH_DA_NHAP_KHO : base
}
...
function deriveApDungVatGtgtTuBanGhi(r: DonHangBanChungTuRecord | Partial<DonHangBanChungTuRecord> | null | undefined): boolean {
  ...
  const v = (r as DonHangBanChungTuRecord).ap_dung_vat_gtgt
  if (typeof v === 'boolean') return v
  return true
}

/** [YC37] Một dòng trống theo mẫu — Lượng mặc định 1. */
function createEmptyBaoGiaLine(mau: 'codongia' | 'khongdongia'): DonHangBanGridLineRow {
  const cols = insertVariantColumns(mau === 'codongia' ? [...mauDhbCoDonGiaDisplay] : [...mauDhbKhongDonGiaDisplay])
...
function chiTietToBaoGiaLines(ct: DonHangBanChungTuChiTiet[]): DonHangBanGridLineRow[] {
...
function deriveHinhThucSelectedIdsFromRecord(
  d: Partial<DonHangBanChungTuRecord> & { hinh_thuc?: string; dia_chi_cong_trinh?: string } | null | undefined
): string[] {
  if (!d) return []
...
function readKhoGiayChieuDaiByTenForDonHangBan(): Record<string, number> {
...
function buildKhoGiayOptionsForVthh(vthh?: VatTuHangHoaRecord | null): string[] | null {
  if (!vthh) return null
  const daiByTen = readKhoGiayChieuDaiByTenForDonHangBan()
  const set = new Set<string>()
  const matrix = Array.isArray(vthh.pricing_matrix) ? vthh.pricing_matrix : []
  matrix.forEach((row) => {
...
function donGiaContextFromDonHangBanLine(row: DonHangBanGridLineRow) {
...
function baoGiaListForDoiChieuNhanHang(): DonHangBanChungTuRecord[] {
  const all = donHangBanGetAll({ ...getDefaultDonHangBanChungTuFilter(), tu: '', den: '' })
  return all.filter((bg) => bg.tinh_trang !== 'Hủy bỏ')
}

/** Hiển thị ô «Nhận hàng từ»: `mã BG: tiêu đề` (tiêu đề = so_chung_tu_cukcuk). */
function formatNhanHangTuTuBaoGia(bg: DonHangBanChungTuRecord): string {
  const ma = (bg.so_don_hang ?? '').trim()
...
function baoGiaListChoPhieuNhanTuBaoGia(): DonHangBanChungTuRecord[] {
  const all = donHangBanGetAll({ ...getDefaultDonHangBanChungTuFilter(), tu: '', den: '' })
...
function mauKhongDonGiaLines(): DonHangBanGridLineRow[] {
  return [createEmptyBaoGiaLine('khongdongia')]
}

/** Mẫu có đơn giá — [YC37] mặc định 1 dòng. */
function mauCoDonGiaLines(): DonHangBanGridLineRow[] {
...
export function DonHangBanForm({ onClose, onSaved, onHeaderPointerDown, headerDragStyle, dragging, readOnly = false, initialDhb, initialChiTiet, prefillDhb, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: _formTitle, soDonLabel: soDonLabelProp, phieuNhanTuBaoGia = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false, onMoPhieuThuTuForm, onMoPhieuChiTuForm, choPhepMoPhieuThuTuForm = false, choPhepMoPhieuChiTuForm = false }: DonHangBanChungTuFormProps) {
  ...
  const isViewMode = readOnly && initialDhb != null
  ...
  const [khachHangDisplay, setKhachHangDisplay] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.khach_hang
    if (prefillDhb?.khach_hang) return prefillDhb.khach_hang
    return ''
  })
  ...
  const [attachments, setAttachments] = useState<DonHangBanChungTuAttachmentItem[]>(() => {
    if (initialDhb?.attachments?.length) return initialDhb.attachments.map((a) => ({ ...a }))
    if (prefillDhb && 'attachments' in prefillDhb && Array.isArray(prefillDhb.attachments) && prefillDhb.attachments.length > 0) {
      return prefillDhb.attachments.map((a) => ({ ...a }))
    }
    return []
  })
  ...
  const [dktkPendingUploadRows, setDktkPendingUploadRows] = useState<DonHangBanChungTuDinhKemPendingRow[]>([])
  const [phieuChiDktkPendingRows, setPhieuChiDktkPendingRows] = useState<DonHangBanChungTuDinhKemPendingRow[]>([])
  const patchAttachmentsFromUser = useCallback((next: SetStateAction<DonHangBanChungTuAttachmentItem[]>) => {
    setAttachments(next)
    setAttachmentsDirty(true)
  }, [])
  const daDongBoLuuCsdlDktk = useMemo(
    () => Boolean(initialDhb && !attachmentsDirty),
    [initialDhb, attachmentsDirty],
  )
  ...
  const [nguoiGiaoHang, setNguoiGiaoHang] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.nguoi_giao_hang ?? ''
    if (prefillDhb?.nguoi_giao_hang != null) return prefillDhb.nguoi_giao_hang
    return ''
  })
  const [maSoThue, setMaSoThue] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.ma_so_thue ?? ''
    if (prefillDhb?.ma_so_thue != null) return prefillDhb.ma_so_thue
    return ''
  })
  const [dienGiai, setDienGiai] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.dien_giai ?? ''
    if (prefillDhb?.dien_giai != null) return prefillDhb.dien_giai
    return ''
  })
  const [nvMuaHang, setNvMuaHang] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.nv_ban_hang ?? ''
    if (prefillDhb?.nv_ban_hang != null) return prefillDhb.nv_ban_hang
    return ''
  })
  const [dieuKhoanTT, setDieuKhoanTT] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.dieu_khoan_tt ?? ''
    if (prefillDhb?.dieu_khoan_tt != null) return prefillDhb.dieu_khoan_tt
    return ''
  })
  const [soNgayDuocNo, setSoNgayDuocNo] = useState(() => {
    if (isViewMode && initialDhb) return initialDhb.so_ngay_duoc_no ?? '0'
    if (prefillDhb?.so_ngay_duoc_no != null) return prefillDhb.so_ngay_duoc_no
    return '0'
  })
  ...
  const [hinhThucSelectedIds, setHinhThucSelectedIds] = useState<string[]>(() => {
    const fromRec = deriveHinhThucSelectedIdsFromRecord((initialDhb ?? prefillDhb) as DonHangBanChungTuRecord | null | undefined)
    if (fromRec.length > 0) return fromRec
    if (phieuNhanTuBaoGia && initialDhb == null && prefillDhb == null) return defaultPhieuNhanHinhThucNhapKhoIds()
    return fromRec
  })
  ...
  const [thamChieu, setThamChieu] = useState(() => {
    if (isViewMode && initialDhb) {
      if (phieuNhanTuBaoGia) {
        const lk = initialDhb.doi_chieu_don_mua_id
        if (lk) {
          const all = donHangBanGetAll({ ...getDefaultDonHangBanChungTuFilter(), tu: '', den: '' })
          const d = all.find((x) => x.id === lk)
          if (d) return formatNhanHangTuTuBaoGia(d)
        }
        return (initialDhb.so_chung_tu_cukcuk ?? '').toUpperCase()
      }
      return initialDhb.so_chung_tu_cukcuk ?? ''
    }
    if (prefillDhb && phieuNhanTuBaoGia) {
      const pd = prefillDhb as DonHangBanChungTuRecord
      if ((pd.so_don_hang ?? '').trim()) return formatNhanHangTuTuBaoGia(pd)
    }
    if (prefillDhb?.so_chung_tu_cukcuk != null) {
      return phieuNhanTuBaoGia
        ? String(prefillDhb.so_chung_tu_cukcuk).toUpperCase()
        : prefillDhb.so_chung_tu_cukcuk
    }
    return ''
  })
  const [ngayDonHang, setNgayDonHang] = useState<Date | null>(() => {
    if (isViewMode && initialDhb) return parseIsoToDate(initialDhb.ngay_don_hang)
    if (phieuNhanTuBaoGia) return new Date()
    if (prefillDhb?.ngay_don_hang) return parseIsoToDate(prefillDhb.ngay_don_hang)
    return new Date()
  })
  ...
  const [ngayGiaoHang, setNgayGiaoHang] = useState<Date | null>(() => {
    if (isViewMode && initialDhb) return parseIsoToDate(initialDhb.ngay_giao_hang)
    if (prefillDhb?.ngay_giao_hang !== undefined) return parseIsoToDate(prefillDhb.ngay_giao_hang ?? null)
    return null
  })
  const [phieuChiNgay, setPhieuChiNgay] = useState<Date | null>(() => {
    const r = (initialDhb ?? prefillDhb) as DonHangBanChungTuRecord | undefined
  ...
  const [apDungVatGtgt, setApDungVatGtgt] = useState(() => deriveApDungVatGtgtTuBanGhi((initialDhb ?? prefillDhb) as DonHangBanChungTuRecord | null))
  ...
  const [lines, setLines] = useState<DonHangBanGridLineRow[]>(() => {
    ...
    if (!isViewMode && initialDhb == null && (!prefillChiTiet || prefillChiTiet.length === 0)) {
      return [createEmptyBaoGiaLine('codongia')]
    }
    return []
  })
  ...
  /** Mẫu hiện tại — codongia khi có initialDhb (dữ liệu đủ cột), else theo lựa chọn user (rule mau_gia.mdc) */
  ...
  const loaiKhachHangHienThi = useMemo((): 'ca_nhan' | 'doanh_nghiep' | null => {
    ...
    const saved = (initialDhb ?? prefillDhb) as DonHangBanChungTuRecord | undefined
    if (saved?.loai_khach_hang === 'ca_nhan' || saved?.loai_khach_hang === 'doanh_nghiep') return saved.loai_khach_hang
    return null
  }, [khachHangId, khList, khachHangDisplay, initialDhb?.loai_khach_hang, (prefillDhb as DonHangBanChungTuRecord | undefined)?.loai_khach_hang])
  ...
  const [phieuChiAttachments, setPhieuChiAttachments] = useState<DonHangBanChungTuAttachmentItem[]>(() => {
    const r = (initialDhb ?? prefillDhb) as DonHangBanChungTuRecord | undefined
    const a = r?.phieu_chi_attachments
    return Array.isArray(a) && a.length ? a.map((x) => ({ ...x })) : []
  })
  const patchPhieuChiAttachmentsFromUser = useCallback((next: SetStateAction<DonHangBanChungTuAttachmentItem[]>) => {
  ...
  /** Tăng khi chọn báo giá nguồn — để effect chuẩn hóa đính kèm đọc lại `so_bao_gia_goc` từ ref. */
  ...
  // [BaoGia] Bỏ logic đối chiếu đơn mua - không áp dụng cho Đơn hàng bán thông thường
  ...
  // const [viewDonHangBanChungTuRecord, setViewDonHangBanChungTuRecord] = useState<DonHangBanChungTuRecord | null>(null)

  /** Phiếu NVTHH: bỏ margin dưới từng dòng — khoảng cách dọc = `gap` tab panel (cùng kho nhập → ĐĐNH 1). */
  ...
  }, [initialDhb?.id, phieuNhanTuBaoGia])

  useEffect(() => {
    if (!phieuNhanTuBaoGia || initialDhb != null) return
    if (dienGiaiPhieuTuChinhRef.current) return
    setDienGiai(dienGiaiPhieuMacDinh)
  }, [phieuNhanTuBaoGia, initialDhb, dienGiaiPhieuMacDinh])

  useEffect(() => {
    if (!phieuNhanTuBaoGia || phieuNhanTabChung !== 'phieu-chi' || initialDhb != null) return
    setPhieuChiKH(khachHangDisplay)
    setPhieuChiDiaChi(diaChi)
  }, [phieuNhanTuBaoGia, phieuNhanTabChung, khachHangDisplay, diaChi, initialDhb])
  ...
  /** Khi mở form thêm mới: gán số Đơn hàng bán tiếp theo (chỉ một lần). */
  useEffect(() => {
    if (initialDhb != null || didSetSoDonRef.current) return
  ...
  useEffect(() => {
    ...
    if (!prefillDhb && (!prefillChiTiet || prefillChiTiet.length === 0)) return
    didPrefillRef.current = true

    if (prefillDhb) {
      if (prefillDhb.khach_hang != null) setKhachHangDisplay(prefillDhb.khach_hang)
      if (prefillDhb.dia_chi != null) setDiaChi(prefillDhb.dia_chi)
      if (prefillDhb.nguoi_giao_hang != null) setNguoiGiaoHang(prefillDhb.nguoi_giao_hang)
      if (prefillDhb.ma_so_thue != null) setMaSoThue(prefillDhb.ma_so_thue)
      if (prefillDhb.dien_giai != null) setDienGiai(prefillDhb.dien_giai)
      ...
      if (phieuNhanTuBaoGia) {
        ...
        if ((pd.so_don_hang ?? '').trim()) setThamChieu(formatNhanHangTuTuBaoGia(pd))
        else if (prefillDhb.so_chung_tu_cukcuk != null) setThamChieu(prefillDhb.so_chung_tu_cukcuk)
      } else if (prefillDhb.so_chung_tu_cukcuk != null) {
        setThamChieu(prefillDhb.so_chung_tu_cukcuk)
      }
      if (prefillDhb.tinh_trang != null) {
        setTinhTrang(normalizeTinhTrangPhieuNvthhForForm(prefillDhb.tinh_trang, laPhieuNhanNvthh))
      }
      if (prefillDhb.ngay_don_hang != null && !phieuNhanTuBaoGia) setNgayDonHang(parseIsoToDate(prefillDhb.ngay_don_hang))
      if (prefillDhb.ngay_giao_hang !== undefined) setNgayGiaoHang(parseIsoToDate(prefillDhb.ngay_giao_hang ?? null))
      const ext = prefillDhb as DonHangBanChungTuRecord
      ...
      if (prefillDhb.attachments?.length) {
        setAttachments(prefillDhb.attachments.map((a) => ({ ...a })))
      }
      {
        const pd = prefillDhb as DonHangBanChungTuRecord
      ...
      // so_don_hang: luôn dùng số tự sinh của đơn hàng bán / phiếu nhận
    }

    if (prefillChiTiet && prefillChiTiet.length > 0) {
      setLines(chiTietToBaoGiaLines(prefillChiTiet))
    }
  }, [isViewMode, prefillDhb, prefillChiTiet, laPhieuNhanNvthh, phieuNhanTuBaoGia])
  ...
  const applyChonBaoGiaDoiChieu = useCallback(
    (bg: DonHangBanChungTuRecord) => {
      const label = (bg.so_don_hang ?? '').trim()
      ...
      setDiaChiCongTrinh((bg as DonHangBanChungTuRecord & { dia_chi_cong_trinh?: string }).dia_chi_cong_trinh ?? '')
  ...
  useEffect(() => {
    const raw = (initialDhb?.khach_hang ?? prefillDhb?.khach_hang ?? '').trim()
  ...
  }, [initialDhb?.khach_hang, prefillDhb?.khach_hang, khList, laPhieuNhanNvthh, initialDhb, prefillDhb])

  /** Đổi đơn (id): nạp file đính kèm từ bản ghi và chuẩn hóa `name`/`virtual_path` theo Mã ĐHB + KH hiện tại (dữ liệu cũ có thể còn `KH_unknown` trong tên file). */
  useEffect(() => {
    if (initialDhb?.id == null) return
    const a = initialDhb.attachments
    ...
    const so = (initialDhb.so_don_hang ?? '').trim() || 'BG'
    const khPart = maKhPathPartTuKhachHangVaDanhBa(initialDhb.khach_hang, khList)
    const soBg = (initialDhb.so_bao_gia_goc ?? '').trim() || undefined
    setAttachments(chuanHoaDuongDanDinhKemDonHangBanChungTu(raw, so, khPart, { soBaoGiaGoc: soBg }))
    setAttachmentsDirty(false)
  }, [initialDhb?.id, initialDhb?.khach_hang, initialDhb?.so_bao_gia_goc, initialDhb?.so_don_hang, khList])

  /** Đồng bộ `name` + `virtual_path` khi đổi Mã ĐHB / KH (đính kèm trước, nhập KH sau — tránh tên file vẫn `KH_unknown`). */
  useEffect(() => {
    const so = soDonHang.trim() || 'DHM'
    const soBg =
      (initialDhb?.so_bao_gia_goc ?? prefillDhb?.so_bao_gia_goc ?? baoGiaNguonPickRef.current?.so_bao_gia_goc ?? '').trim() ||
      undefined
    setAttachments((prev) => {
      if (prev.length === 0) return prev
      const next = chuanHoaDuongDanDinhKemDonHangBanChungTu(prev, so, khPartDinhKem, { soBaoGiaGoc: soBg })
      const same = next.every(
        (n: DonHangBanChungTuAttachmentItem, i: number) => n.virtual_path === prev[i]?.virtual_path && n.name === (prev[i]?.name ?? '')
      )
      return same ? prev : next
    })
  }, [soDonHang, khPartDinhKem, initialDhb?.so_bao_gia_goc, prefillDhb?.so_bao_gia_goc, dktkBaoGiaPickTick])
  ...
  }, [phieuNhanTuBaoGia, soDonHang, khPartDinhKem, initialDhb?.so_bao_gia_goc, prefillDhb?.so_bao_gia_goc, dktkBaoGiaPickTick])

  /** Khi chuyển sang xem đơn khác (initialDhb/initialChiTiet đổi) mà form vẫn mở: đồng bộ toàn bộ state từ props để hiển thị đúng đơn mới. */
  useEffect(() => {
    if (!readOnly || initialDhb == null) return
    const d = initialDhb as DonHangBanChungTuRecord & { hinh_thuc?: string; dia_chi_cong_trinh?: string }
    setApDungVatGtgt(deriveApDungVatGtgtTuBanGhi(initialDhb))
    setTenNguoiLienHe((initialDhb.ten_nguoi_lien_he ?? '').trim())
    setSoDienThoaiLienHe((initialDhb.so_dien_thoai_lien_he ?? '').trim())
    setSoDienThoaiCaNhan((initialDhb.so_dien_thoai ?? '').trim())
    setTlCkInput(
      initialDhb.tl_ck != null && typeof initialDhb.tl_ck === 'number' && Number.isFinite(initialDhb.tl_ck)
        ? initialDhb.tl_ck === 0
          ? '0'
          : formatSoThapPhan(initialDhb.tl_ck, 3)
        : '0',
    )
    setTienCkInput(
      initialDhb.tien_ck != null && typeof initialDhb.tien_ck === 'number' && Number.isFinite(initialDhb.tien_ck)
        ? formatSoTienHienThi(initialDhb.tien_ck)
    ...
    setNvMuaHang(initialDhb.nv_ban_hang ?? '')
    setDieuKhoanTT(initialDhb.dieu_khoan_tt ?? '')
    ...
    if (phieuNhanTuBaoGia && initialDhb.doi_chieu_don_mua_id) {
      const linked = donHangBanGetAll({ ...getDefaultDonHangBanChungTuFilter(), tu: '', den: '' }).find((x) => x.id === initialDhb.doi_chieu_don_mua_id)
      setThamChieu(
        linked ? formatNhanHangTuTuBaoGia(linked) : (initialDhb.so_chung_tu_cukcuk ?? '').toUpperCase(),
      )
    } else {
      setThamChieu(initialDhb.so_chung_tu_cukcuk ?? '')
    }
    setNgayDonHang(parseIsoToDate(initialDhb.ngay_don_hang))
    setSoDonHang(initialDhb.so_don_hang)
    setTinhTrang(normalizeTinhTrangPhieuNvthhForForm(initialDhb.tinh_trang, laPhieuNhanNvthh))
    setNgayGiaoHang(parseIsoToDate(initialDhb.ngay_giao_hang))
    if (initialDhb.chung_tu_mua_chua_thanh_toan) setChungTuMuaCachThanhToan('chua_thanh_toan')
    else if (initialDhb.chung_tu_mua_thanh_toan_ngay) setChungTuMuaCachThanhToan('thanh_toan_ngay')
    else setChungTuMuaCachThanhToan('thanh_toan_ngay')
    setChungTuMuaPttt(normalizeChungTuMuaPtttStored(initialDhb.chung_tu_mua_pttt))
    setChungTuMuaLoaiHd(initialDhb.chung_tu_mua_loai_hd === 'gtgt' ? 'gtgt' : 'hd_le')
    setPhieuSoHoaDon((initialDhb.chung_tu_mua_so_hoa_don ?? '').trim())
    {
      const iso = (initialDhb.hoa_don_ngay ?? '').trim()
      if (iso) {
        const m = iso.match(/^(\d{4})-(\d{2})-(\d{2})/)
        setHoaDonNgay(
    ...
    setHoaDonKyHieu((initialDhb.hoa_don_ky_hieu ?? '').trim())
    setMauHoaDonMa((initialDhb.mau_hoa_don_ma ?? '').trim())
    setMauHoaDonTen((initialDhb.mau_hoa_don_ten ?? '').trim())
    setPhieuChiKH(initialDhb.phieu_chi_nha_cung_cap ?? '')
    setPhieuChiDiaChi(initialDhb.phieu_chi_dia_chi ?? '')
    setPhieuChiNguoiNhanTien(initialDhb.phieu_chi_nguoi_nhan_tien ?? '')
    setPhieuChiLyDo(initialDhb.phieu_chi_ly_do ?? '')
    setPhieuChiNgay(parseIsoToDate(initialDhb.phieu_chi_ngay ?? null))
    setPhieuChiTaiKhoanChi(initialDhb.phieu_chi_tai_khoan_chi ?? '')
    setPhieuChiNganHangChi(initialDhb.phieu_chi_ngan_hang_chi ?? initialDhb.phieu_chi_ngan_hang ?? '')
    setPhieuChiTenNguoiGui(initialDhb.phieu_chi_ten_nguoi_gui ?? '')
    setPhieuChiTaiKhoanNhan(initialDhb.phieu_chi_tai_khoan_nhan ?? '')
    setPhieuChiNganHangNhan(initialDhb.phieu_chi_ngan_hang_nhan ?? '')
    setPhieuChiTenChuTkNhan(initialDhb.phieu_chi_ten_chu_tk_nhan ?? initialDhb.phieu_chi_ten_nguoi_nhan_ck ?? '')
    const pca = initialDhb.phieu_chi_attachments
    ...
    const att = initialDhb.attachments
    ...
    if (rawAtt.length === 0) setAttachments([])
      ...
      const khPart = maKhPathPartTuKhachHangVaDanhBa(initialDhb.khach_hang, khList)
      const soBg = (initialDhb.so_bao_gia_goc ?? '').trim() || undefined
      setAttachments(chuanHoaDuongDanDinhKemDonHangBanChungTu(rawAtt, so, khPart, { soBaoGiaGoc: soBg }))
    }
    setAttachmentsDirty(false)
    setEditingFromView(false)
  }, [readOnly, initialDhb?.id, initialDhb, initialChiTiet, laPhieuNhanNvthh, phieuNhanTuBaoGia, khList])
  ...
  useEffect(() => {
    if (initialDhb != null || effectiveReadOnly || draftLoadedRef.current || vatTuList.length === 0) return
    ...
    setLines(enriched)
  }, [vatTuList, effectiveReadOnly, initialDhb])

  /** Lưu draft chỉ khi form thêm mới (không sửa đơn có sẵn), mỗi khi các dòng thay đổi. */
  useEffect(() => {
    if (initialDhb != null || effectiveReadOnly || lines.length === 0) return
    api.setDraft(lines)
  }, [lines, effectiveReadOnly, initialDhb])
  ...
  const calculateSoLuongFromKichThuoc = (line: DonHangBanGridLineRow): string =>
  ...
  const syncDonGiaTheoBacVaSl = (row: DonHangBanGridLineRow): DonHangBanGridLineRow => {
    if (!row._vthh || !(initialDhb != null || mauHienTai === 'codongia')) return row
    const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
    const dvt = (row['ĐVT'] ?? '').trim() || (row._vthh.dvt_chinh ?? '')
    const next = { ...row, 'Đơn giá': getDonGiaBanDonHangBanLine(row._vthh, dvt, sl, donGiaContextFromDonHangBanLine(row)) } as unknown as DonHangBanGridLineRow
    const minB1 = getBanHangB1TtAdjustment(row._vthh, sl)
    if (minB1.ap_dung) {
      next['ĐVT'] = minB1.dvt_goi_y
      next['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, minB1.dvt_goi_y, sl, donGiaContextFromDonHangBanLine(next))
  ...
  const handleChonVthh = (vthh: VatTuHangHoaRecord, rowIndex: number) => {
    ...
    if (isCodongia) {
      ...
      const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
      row['Đơn giá'] = getDonGiaBanDonHangBanLine(vthh, (row['ĐVT'] ?? '').trim() || (vthh.dvt_chinh ?? ''), sl, donGiaContextFromDonHangBanLine(row))
      const minB1 = getBanHangB1TtAdjustment(vthh, sl)
      if (minB1.ap_dung) {
        row['ĐVT'] = minB1.dvt_goi_y
        row['Đơn giá'] = getDonGiaBanDonHangBanLine(vthh, minB1.dvt_goi_y, sl, donGiaContextFromDonHangBanLine(row))
        const note = (row['Ghi chú'] ?? '').trim()
        if (!note.toLowerCase().includes('áp dụng đơn giá tối thiểu')) {
          row['Ghi chú'] = note ? `${note}; ${minB1.ghi_chu}` : minB1.ghi_chu
        }
      }
    }
  ...
  const buildPayload = (): DonHangBanChungTuCreatePayload => {
    const soBgPayload =
      ...
      undefined
    const coBang =
      initialDhb != null || Boolean(prefillChiTiet && prefillChiTiet.length > 0) || mauHienTai === 'codongia'
    ...
    const payload: DonHangBanChungTuCreatePayload = {
      ...
      so_don_hang: soDonHang.trim() || api.soDonHangTiepTheo(),
      ...
      attachments:
        attachments.length > 0
          ? chuanHoaDuongDanDinhKemDonHangBanChungTu(attachments, soDonHang.trim() || 'DHM', khPartDinhKem, {
        ...
        ? {
            ...
            phieu_chi_attachments:
              phieuChiAttachments.length > 0
                ? chuanHoaDuongDanDinhKemDonHangBanChungTu(phieuChiAttachments, soDonHang.trim() || 'DHM', khPartDinhKem, {
                    soBaoGiaGoc: soBgPayload,
                  })
                : undefined,
          }
  ...
  const validateBeforeSave = (): boolean => {
    ...
    if (!thamChieu.trim()) {
      return fail(
        ...
        laPhieuNhanNvthh
          ? phieuNhanTuBaoGia
            ? 'Vui lòng chọn hoặc nhập đơn hàng bán (Nhận hàng từ).'
            : 'Vui lòng chọn hoặc nhập mã đơn hàng bán đối chiếu.'
          : 'Vui lòng nhập Đơn hàng bán.',
  ...
  const handleLuuVaDong = async () => {
    ...
    try {
      ...
      if (attachments.length > 0) {
        const norm = chuanHoaDuongDanDinhKemDonHangBanChungTu(attachments, soD, khPartDinhKem, { soBaoGiaGoc: soBgOpt })
        const moved = await htqlReconcileThietKeServerPaths(attachments, norm)
        setAttachments(moved)
      }
      if (phieuChiAttachments.length > 0) {
        const normPc = chuanHoaDuongDanDinhKemDonHangBanChungTu(phieuChiAttachments, soD, khPartDinhKem, {
          soBaoGiaGoc: soBgOpt,
        })
  ...
  /** [YC32] Hạn thanh toán = TG tạo đơn hàng bán (ngayDonHang) + số ngày được nợ */
  ...
  const coBangCoDonGiaTuDau =
    initialDhb != null || Boolean(prefillChiTiet && prefillChiTiet.length > 0)
  const hienThiFooterTongTien = coBangCoDonGiaTuDau || mauHienTai === 'codongia'
  const currentColumns = useMemo((): readonly string[] => {
    if (!hienThiFooterTongTien) return insertVariantColumns(mauDhbKhongDonGiaDisplay as readonly string[])
    return insertVariantColumns((apDungVatGtgt ? mauDhbCoDonGiaDisplay : mauDhbCoDonGiaKhongVatDisplay) as readonly string[])
  ...
  const phieuNhanLienKetTuDonMua = useMemo(() => {
    ...
    return all.filter((p) => (p.doi_chieu_don_mua_id ?? '').trim() === initialDhb.id)
  }, [initialDhb?.id, phieuNhanTuBaoGia])

  const showLienQuanPhieuNhanHang =
    !phieuNhanTuBaoGia &&
    Boolean(initialDhb?.id) &&
    tinhTrang === TINH_TRANG_DON_HANG_BAN_CT_DA_GUI_KHACH
  ...
  return (
    <div className={bgDetailStyles.htqlBanHangChungTuFormShell} style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, background: 'var(--bg-primary)' }}>
      ...
      <div style={toolbarWrap}>
        {readOnly && effectiveReadOnly && !donDaNhanHangXem && !donHuyBoChiXem && !viewOnlyLocked && !donDaThuTienKhoa && (
        ...
        >
          <button type="button" style={toolbarBtn} disabled={effectiveReadOnly || initialDhb != null || phieuNhanTuBaoGia}>
            <FileText size={14} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>Mẫu <ChevronDown size={10} /></span>
          </button>
          {dropdownMau && !effectiveReadOnly && initialDhb == null && !phieuNhanTuBaoGia && (
      ...
      {/* [BaoGia] Bỏ modal chọn đơn hàng bán đối chiếu - không áp dụng cho Đơn hàng bán */}
      {false && (
        <Modal
          open={false}
          onClose={() => {}}
          title="Chọn đơn hàng bán"
          size="md"
        >
              ...
              return (
                <>
                  {bgChonTuDonList.map((bg) => {
                    const ngayDdMm = formatIsoToDdMmYyyy(bg.ngay_don_hang)
                    return (
                      <div
                        key={bg.id}
                        role="button"
                      ...
                      >
                        <span style={{ fontWeight: 600 }}>{bg.so_don_hang}</span>
                  ...
                  {bgChonTuDonList.length === 0 && (
                    <div style={{ padding: 16, fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>
                      Không có đơn hàng bán đủ điều kiện đối chiếu.
                    </div>
                  )}
                </>
              )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1 1 0', minHeight: 0, gap: 2, marginBottom: 2 }}>
          ...
          {(() => {
            ...
            <div style={{ ...fieldRowDyn, alignItems: 'center' }}>
              <label style={labelStyle}>{phieuNhanTuBaoGia ? 'Nhận hàng từ:' : 'Đơn hàng bán'}</label>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center' }}>
                {effectiveReadOnly ? (
                  ...
                  >
                    {/* [BaoGia] Bỏ logic link xem đơn hàng bán đối chiếu */}
                    {false ? null : (
                      <span
                        style={{
                          color: 'var(--text-muted)',
              ...
              ? ReactDOM.createPortal(
                  (() => {
                    ...
                    const listNvthhNhanTu = baoGiaListChoPhieuNhanTuBaoGia().filter((bg) => {
                      if (!q) return true
                      const ma = (bg.so_don_hang ?? '').toLowerCase()
                    ...
                    return (
                      ...
                      >
                        ...
                        {listNvthhNhanTu.length === 0 && (
                          <div style={{ padding: 12, fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>
                            Không có đơn hàng bán ở tình trạng Chờ duyệt.
                          </div>
                        )}
                      </div>
                    )
            ...
            <div style={{ ...fieldRowDyn, flexWrap: 'wrap', gap: 8, width: '100%', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto', minWidth: 0 }}>
                ...
                <input
                  ...
                  disabled
                  tabIndex={-1}
                  title={hanThanhToanDayDu ? 'TG tạo đơn hàng bán + số ngày được nợ' : 'Chưa đủ TG tạo hoặc số ngày được nợ — đang hiển thị ngày hiện tại'}
                />
              </div>
            </div>
          ...
          >
            <div style={{ ...groupBoxTitle, ...(phieuNhanTuBaoGia ? { marginBottom: 4 } : {}) }}>
              {phieuNhanTuBaoGia ? (phieuNhanTabChung === 'hoa-don' ? 'Hóa đơn' : 'Chứng từ') : 'Đơn hàng bán'}
            </div>
                          ...
                          >
                            {showColAsReadOnly ? (
                              ...
                              <div style={{ position: 'relative', width: '100%', display: 'inline-block' }}>
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const prev = r[idx] as DonHangBanGridLineRow
                                    const nextRow = {
                                      ...prev,
                                      [col]: ma,
                                    ...
                                    } as unknown as DonHangBanGridLineRow
                                    const isCodongia = initialDhb != null || mauHienTai === 'codongia'
                                    if (cleared || !item) {
                                      ...
                                      if (isCodongia) {
                                        ...
                                        nextRow['Đơn giá'] = getDonGiaBanDonHangBanLine(
                                          item,
                                          (nextRow['ĐVT'] ?? '').trim() || (item.dvt_chinh ?? ''),
                                          sl,
                                          donGiaContextFromDonHangBanLine(nextRow),
                                        )
                              ...
                              (() => {
                                ...
                                if (opts && opts.length > 0) {
                                  return (
                                    <select
                                      ...
                                      value={line['Độ dày/ ĐL'] ?? opts[0]}
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = { ...r[idx], 'Độ dày/ ĐL': e.target.value } as unknown as DonHangBanGridLineRow
                                        if ((initialDhb != null || mauHienTai === 'codongia') && row._vthh) {
                                          const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
                                          row['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, (row['ĐVT'] ?? '').trim() || (row._vthh.dvt_chinh ?? ''), sl, donGiaContextFromDonHangBanLine(row))
                                        }
                                        r[idx] = row
                                        setLines(r)
                                      }}
                                ...
                                return <input className="misa-input-solo" style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }} value={line['Độ dày/ ĐL'] ?? ''} onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], 'Độ dày/ ĐL': e.target.value } as unknown as DonHangBanGridLineRow; setLines(r) }} />
                              })()
                            ) : col === 'Khổ giấy' ? (
                              (() => {
                                ...
                                if (opts && opts.length > 0) {
                                  return (
                                    <select
                                      className="misa-input-solo"
                                      style={{ ...inputStyle, width: '100%', minHeight: 22, height: 22, cursor: 'pointer' }}
                                      value={line['Khổ giấy'] ?? opts[0]}
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = { ...r[idx], 'Khổ giấy': e.target.value } as unknown as DonHangBanGridLineRow
                                        if ((initialDhb != null || mauHienTai === 'codongia') && row._vthh) {
                                          const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
                                          row['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, (row['ĐVT'] ?? '').trim() || (row._vthh.dvt_chinh ?? ''), sl, donGiaContextFromDonHangBanLine(row))
                                        }
                                ...
                                return <input className="misa-input-solo" style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }} value={line['Khổ giấy'] ?? ''} onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], 'Khổ giấy': e.target.value } as unknown as DonHangBanGridLineRow; setLines(r) }} />
                              })()
                            ) : col === 'Nội dung' ? (
                              <input
                                className="misa-input-solo"
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'Nội dung': e.target.value } as unknown as DonHangBanGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'ĐVT' ? (
                                ...
                                if (hasDvtDropdown) {
                                  return (
                                    <select
                                      ...
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = r[idx] as DonHangBanGridLineRow
                                        const newDvt = e.target.value
                                        const updates = { ...row, 'ĐVT': newDvt } as unknown as DonHangBanGridLineRow
                                        updates['Số lượng'] = calculateSoLuongFromKichThuoc(updates)
                                        if ((initialDhb != null || mauHienTai === 'codongia') && row._vthh) {
                                          const sl = Math.max(0, parseFloatVN(updates['Số lượng'] ?? '')) || 1
                                          updates['Đơn giá'] = getDonGiaBanDonHangBanLine(row._vthh, newDvt, sl, donGiaContextFromDonHangBanLine(updates))
                                        }
                                        r[idx] = updates
                                        setLines(r)
                                      }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updated = { ...r[idx], 'mD': raw } as unknown as DonHangBanGridLineRow
                                ...
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mD': '' } as unknown as DonHangBanGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const raw = formatSoTien(normalizeKichThuocInput(e.target.value))
                                  const updated = { ...r[idx], 'mR': raw } as unknown as DonHangBanGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mR': '' } as unknown as DonHangBanGridLineRow
                                    row['Số lượng'] = calculateSoLuongFromKichThuoc(row)
                                    row = syncDonGiaTheoBacVaSl(row)
                                    r[idx] = row
                                    setLines(r)
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(e.target.value) } as unknown as DonHangBanGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(String(n)) } as unknown as DonHangBanGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  let row = { ...r[idx], [col]: val } as unknown as DonHangBanGridLineRow
                                  if (row._vthh && (initialDhb != null || mauHienTai === 'codongia')) {
                                ...
                                onBlur={() => {
                                  ...
                                  const r = [...lines]
                                  let next = { ...r[idx], [col]: formatSoTienHienThi(val) } as unknown as DonHangBanGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'Đơn giá': formatSoTien(e.target.value) } as unknown as DonHangBanGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'Thành tiền' ? (
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], '% thuế GTGT': e.target.value } as unknown as DonHangBanGridLineRow
                                  setLines(r)
                                }}
                              >
                              ...
                              <input
                                ...
                                style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }}
                                value={line[col] ?? ''}
                                onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], [col]: e.target.value } as unknown as DonHangBanGridLineRow; setLines(r) }}
                              />
                            )}
                          </td>
                        )
      ...
      {vthhDropdownRowIndex !== null && vthhDropdownRect && ReactDOM.createPortal(
        <div
          ...
          style={{
            ...
            /* Trên overlay modal Đơn hàng bán (BanHang.module.css .modalOverlay z-index: 4000); thấp hơn 4000 thì list bị che, tưởng «không lọc được». */
            zIndex: 4100,
            fontSize: 11,
          }}
        >
