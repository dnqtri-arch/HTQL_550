...
/** Đồng bộ `thue_suat_gtgt` / `thue_suat_gtgt_dau_ra` (token chuẩn, ưu tiên đầu ra; sửa dữ liệu cũ lệch hoặc dạng 0,08). */
function normalizeVthhThueFields(r: VatTuHangHoaRecord): VatTuHangHoaRecord {
  const dauRaw = String(r.thue_suat_gtgt_dau_ra ?? '').trim()
  const gtgRaw = String(r.thue_suat_gtgt ?? '').trim()
  const dauTokEarly = thueGtgtStdToken(dauRaw)
  const gtgTokEarly = thueGtgtStdToken(gtgRaw)
  if (
    (dauRaw === 'Tự nhập' || dauTokEarly === 'Tự nhập') &&
    gtgTokEarly !== 'Tự nhập' &&
    gtgRaw &&
    THUE_GTGT_STD_SET.has(gtgTokEarly)
  ) {
    return { ...r, thue_suat_gtgt: gtgTokEarly, thue_suat_gtgt_dau_ra: gtgTokEarly }
  }
  const dau = thueGtgtStdToken(dauRaw)
  const gtg = thueGtgtStdToken(gtgRaw)
  if (THUE_GTGT_STD_SET.has(dau) && THUE_GTGT_STD_SET.has(gtg) && dau && gtg && dau !== gtg) {
    return { ...r, thue_suat_gtgt: dau, thue_suat_gtgt_dau_ra: dau }
  }
  const prefer =
    (THUE_GTGT_STD_SET.has(dau) && dau && dau !== 'Chưa xác định' ? dau : '') ||
    (THUE_GTGT_STD_SET.has(gtg) && gtg && gtg !== 'Chưa xác định' ? gtg : '') ||
    dau ||
    gtg
  if (THUE_GTGT_STD_SET.has(prefer) && prefer && prefer !== 'Tự nhập') {
    if (r.thue_suat_gtgt === prefer && r.thue_suat_gtgt_dau_ra === prefer) return r
    return { ...r, thue_suat_gtgt: prefer, thue_suat_gtgt_dau_ra: prefer }
  }
  return r
}
...
function loadFromStorage(): VatTuHangHoaRecord[] {
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed)) return parsed.map(normalizeVthhThueFields)
    }
  } catch {
    // ignore
  }
  return DU_LIEU_MAU.map(normalizeVthhThueFields)
}
...
export async function vatTuHangHoaGetAll(): Promise<VatTuHangHoaRecord[]> {
  if (cache) return htqlSortCopyNewestFirst([...cache])
  cache = loadFromStorage()
  return htqlSortCopyNewestFirst([...cache])
}
...
export async function vatTuHangHoaPost(
  ...
  const newRow: VatTuHangHoaRecord = normalizeVthhThueFields({ ...payload, id })
  list.push(newRow)
  saveToStorage(list)
  cache = list
  return newRow
...
export async function vatTuHangHoaPut(
  ...
  const updated: VatTuHangHoaRecord = normalizeVthhThueFields({ ...payload, id })
  list[idx] = updated
  saveToStorage(list)
  cache = list
  return updated
