...
export async function donViTinhDemVthhSuDung(ma_dvt: string): Promise<number> {
  const m = ma_dvt.trim()
  if (!m) return 0
  try {
    const list = await vatTuHangHoaGetAll()
    let n = 0
    for (const r of list) {
      if ((r.dvt_chinh ?? '').trim() === m) n++
      for (const q of r.don_vi_quy_doi ?? []) {
        if ((q.dvt ?? '').trim() === m) n++
      }
    }
    return n
...
export async function donViTinhDangDuongTrongVatTu(ma_dvt: string): Promise<boolean> {
  return (await donViTinhDemVthhSuDung(ma_dvt)) > 0
}
