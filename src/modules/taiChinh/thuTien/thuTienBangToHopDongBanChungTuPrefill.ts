/**
 * Map Báo giá → prefill form Hợp đồng bán (ChungTu) — module tách biệt, không import ngược từ thuTienBangForm.
 */
...
function mapAttachments(att: ThuTienBangRecord['attachments']): HopDongBanChungTuRecord['attachments'] {
  if (!att?.length) return undefined
  return att.map((a) => ({ ...a }))
}

/** Chuẩn bị header + chi tiết để mở form ĐHB mới từ một báo giá (đủ trường đồng dạng BG). */
export function buildHopDongBanChungTuPrefillFromThuTienBang(
  ...
  const prefillHdbCt: Partial<HopDongBanChungTuRecord> = {
    ...
    ngay_lap_hop_dong: row.ngay_thu_tien_bang,
