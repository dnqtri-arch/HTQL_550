...
/** Alias tương thích — cùng nghĩa với ThuTienBangKyValue. */
export type KyValue = ThuTienBangKyValue

const MODULE_PREFIX = 'TT'

/** Dữ liệu mẫu báo giá */
const MOCK_DON: ThuTienBangRecord[] = [
  {
    id: 'bg1',
    tinh_trang: 'Chưa thực hiện',
    ngay_thu_tien_bang: '2026-03-10',
    so_thu_tien_bang: 'TT00001',
    ngay_giao_hang: null,
  ...
  },
  {
    ...
    ngay_thu_tien_bang: '2026-03-12',
    so_thu_tien_bang: 'TT00002',
  ...
  },
  {
    id: 'bg3',
    tinh_trang: 'Chưa thực hiện',
    ngay_thu_tien_bang: '2026-03-15',
    so_thu_tien_bang: 'TT00003',
    ngay_giao_hang: null,
...
const MOCK_CHI_TIET: ThuTienBangChiTiet[] = [
  {
    id: 'ct1',
    thu_tien_bang_id: 'bg1',
    ma_hang: 'VT00001',
...
const STORAGE_KEY_THU_TIEN_BANG = 'htql_thu_tien_bang_list'
const STORAGE_KEY_CHI_TIET = 'htql_thu_tien_bang_chi_tiet'

function normalizeThuTienBang(d: Partial<ThuTienBangRecord> & { id: string; de_xuat_id?: string }): ThuTienBangRecord {
  ...
  return {
    ...
    ngay_thu_tien_bang: d.ngay_thu_tien_bang ?? '',
    so_thu_tien_bang: d.so_thu_tien_bang ?? '',
    ...
    attachments: Array.isArray((d as { attachments?: ThuTienBangAttachmentItem[] }).attachments)
      ? (d as { attachments: ThuTienBangAttachmentItem[] }).attachments
      : undefined,
    ...
    ly_do_thu_phieu: d.ly_do_thu_phieu ?? undefined,
    thu_tien_mat: typeof d.thu_tien_mat === 'boolean' ? d.thu_tien_mat : undefined,
    thu_qua_ngan_hang: typeof d.thu_qua_ngan_hang === 'boolean' ? d.thu_qua_ngan_hang : undefined,
    ngay_hach_toan: d.ngay_hach_toan,
    phieu_tai_khoan_id: d.phieu_tai_khoan_id?.trim() || undefined,
  }
}

function loadFromStorage(): { thuTienBang: ThuTienBangRecord[]; chiTiet: ThuTienBangChiTiet[] } {
  try {
    const rawThuTienBang = typeof htqlEntityStorage !== 'undefined' ? htqlEntityStorage.getItem(STORAGE_KEY_THU_TIEN_BANG) : null
    const rawCt = typeof htqlEntityStorage !== 'undefined' ? htqlEntityStorage.getItem(STORAGE_KEY_CHI_TIET) : null
    const thuTienBang = rawThuTienBang ? JSON.parse(rawThuTienBang) : null
    const chiTiet = rawCt ? JSON.parse(rawCt) : null
    if (Array.isArray(thuTienBang) && Array.isArray(chiTiet)) {
      return { thuTienBang: thuTienBang.map((d: Partial<ThuTienBangRecord> & { id: string }) => normalizeThuTienBang(d)), chiTiet }
    }
  } catch {
    /* ignore */
  }
  return { thuTienBang: [...MOCK_DON], chiTiet: [...MOCK_CHI_TIET] }
}

function saveToStorage(): void {
  try {
    if (typeof htqlEntityStorage !== 'undefined') {
      htqlEntityStorage.setItem(STORAGE_KEY_THU_TIEN_BANG, JSON.stringify(_thuTienBangList))
...
const STORAGE_KEY_DRAFT = 'htql_thu_tien_bang_draft'

export function getThuTienBangDraft(): ThuTienBangDraftLine[] | null {
  try {
...
export function setThuTienBangDraft(lines: Array<Record<string, string> & { _dvtOptions?: string[]; _vthh?: unknown }>): void {
  try {
    if (typeof htqlEntityStorage !== 'undefined') {
...
export function clearThuTienBangDraft(): void {
  try {
...
let _thuTienBangList: ThuTienBangRecord[] = _initial.thuTienBang
let _chiTietList: ThuTienBangChiTiet[] = _initial.chiTiet

export function thuTienBangReloadFromStorage(): void {
  const { thuTienBang, chiTiet } = loadFromStorage()
  _thuTienBangList = thuTienBang
...
export function thuTienBangGetAll(filter: ThuTienBangFilter): ThuTienBangRecord[] {
  const { tu, den } = filter
  if (!tu || !den) return [..._thuTienBangList]
  return _thuTienBangList.filter((d) => {
    const ngay = d.ngay_thu_tien_bang
    return ngay >= tu && ngay <= den
  })
}

export function thuTienBangGetChiTiet(thuTienBangId: string): ThuTienBangChiTiet[] {
  return _chiTietList.filter((c) => c.thu_tien_bang_id === thuTienBangId)
}
...
export function thuTienBangBiKhoaChinhSuaTheoTinhTrang(tinh_trang: string): boolean {
...
export function thuTienBangCapNhatTuMenuTaoGd(
  thuTienBangId: string,
  patch: { tinh_trang: string; dien_giai?: string },
): void {
  const row = _thuTienBangList.find((d) => d.id === thuTienBangId)
  if (!row) return
  const ct = thuTienBangGetChiTiet(thuTienBangId)
  const base = thuTienBangBuildCreatePayloadFromRecord(row, ct)
  thuTienBangPut(thuTienBangId, {
    ...base,
...
export const HTQL_THU_TIEN_BANG_RELOAD_EVENT = 'htql-bao-gia-reload'

/**
 * Sau khi xóa ĐHB: nếu không còn đơn hàng bán nào có `thu_tien_bang_id` trùng, hoàn tác báo giá
 * từ «Đã chuyển ĐHB» → «Mới tạo» (YC51).
 */
export function thuTienBangHoanTacKhiHetLienKetDonHangBan(
  thuTienBangId: string | null | undefined,
  cacDonHangBanConLai: { thu_tien_bang_id?: string }[],
): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  const still = cacDonHangBanConLai.some((d) => (d.thu_tien_bang_id ?? '').trim() === id)
  if (still) return
  const row = _thuTienBangList.find((r) => r.id === id)
  if (!row) return
  if ((row.tinh_trang ?? '').trim() !== TINH_TRANG_BG_DA_CHUYEN_DHB) return
  thuTienBangCapNhatTuMenuTaoGd(id, { tinh_trang: TINH_TRANG_BG_MOI_TAO })
}

/**
 * Sau khi xóa HĐ bán: nếu không còn hợp đồng nào có `thu_tien_bang_id` trùng, hoàn tác báo giá
 * từ «Đã chuyển HĐ» → «Mới tạo» (YC51).
 */
export function thuTienBangHoanTacKhiHetLienKetHopDongBan(
  thuTienBangId: string | null | undefined,
  cacHopDongConLai: { thu_tien_bang_id?: string }[],
): void {
  const id = (thuTienBangId ?? '').trim()
  if (!id) return
  const still = cacHopDongConLai.some((d) => (d.thu_tien_bang_id ?? '').trim() === id)
  if (still) return
  const row = _thuTienBangList.find((r) => r.id === id)
  if (!row) return
...
export function thuTienBangBuildCreatePayloadFromRecord(row: ThuTienBangRecord, ct: ThuTienBangChiTiet[]): ThuTienBangCreatePayload {
  return {
    ...
    ngay_thu_tien_bang: row.ngay_thu_tien_bang,
    so_thu_tien_bang: row.so_thu_tien_bang,
    ...
    ly_do_thu_phieu: row.ly_do_thu_phieu,
    thu_tien_mat: row.thu_tien_mat,
    thu_qua_ngan_hang: row.thu_qua_ngan_hang,
...
export function thuTienBangSetTinhTrang(thuTienBangId: string, tinh_trang: string): void {
  const row = _thuTienBangList.find((d) => d.id === thuTienBangId)
  if (!row) return
  const ct = thuTienBangGetChiTiet(thuTienBangId)
...
export async function thuTienBangPost(payload: ThuTienBangCreatePayload): Promise<ThuTienBangRecord> {
  const year = getCurrentYear()
  const hint = hintMaxSerialForYearPrefix(year, MODULE_PREFIX, _thuTienBangList.map((d) => d.so_thu_tien_bang))
  const soTt = await allocateMaHeThongFromServer({
    seqKey: 'TT',
  ...
  const thuTienBangRow: ThuTienBangRecord = {
    id,
    ...
    ngay_thu_tien_bang: payload.ngay_thu_tien_bang,
    so_thu_tien_bang: soTt,
    ...
    ly_do_thu_phieu: payload.ly_do_thu_phieu,
    thu_tien_mat: payload.thu_tien_mat,
    thu_qua_ngan_hang: payload.thu_qua_ngan_hang,
    ngay_hach_toan: payload.ngay_hach_toan,
    phieu_tai_khoan_id: payload.phieu_tai_khoan_id?.trim() || undefined,
  }
  _thuTienBangList.push(thuTienBangRow)
...
export function thuTienBangPut(thuTienBangId: string, payload: ThuTienBangCreatePayload): void {
  ...
  _thuTienBangList[idx] = {
    ...
    tinh_trang: payload.tinh_trang,
    ngay_thu_tien_bang: payload.ngay_thu_tien_bang,
    so_thu_tien_bang: payload.so_thu_tien_bang,
    ngay_giao_hang: payload.ngay_giao_hang,
    khach_hang: payload.khach_hang,
    dia_chi: payload.dia_chi ?? '',
    nguoi_giao_hang: payload.nguoi_giao_hang ?? undefined,
...
export function getDefaultThuTienBangFilter(): ThuTienBangFilter {
...
export function thuTienBangSoDonHangTiepTheo(): string {
  const year = getCurrentYear()
  let maxSo = 0
  for (const d of _thuTienBangList) {
    const s = (d.so_thu_tien_bang || '').trim()
...
/** Gói API mặc định cho `ThuTienBangApiProvider` (dùng từ module Thu tiền / Đơn hàng bán). */
export const thuTienBangApiImpl: ThuTienBangApi = {
  getAll: thuTienBangGetAll,
  getChiTiet: thuTienBangGetChiTiet,
  delete: thuTienBangDelete,
  getDefaultFilter: getDefaultThuTienBangFilter,
  getDateRangeForKy,
  KY_OPTIONS,
  post: thuTienBangPost,
  put: thuTienBangPut,
  soDonHangTiepTheo: thuTienBangSoDonHangTiepTheo,
  getDraft: getThuTienBangDraft,
  setDraft: setThuTienBangDraft,
  clearDraft: clearThuTienBangDraft,
}
