...
import { mau_tt_bang_khongdongia, mau_tt_bang_codongia, mau_tt_bang_codongia_khong_vat } from './thuTienBangGridMau'
import {
  formatSoNguyenInput,
  formatNumberDisplay,
  formatSoTien,
...
import {
  ...
  getDonGiaBanThuTienBangLine,
  migrateThuTienBangLinesToCoDonGia,
  enrichThuTienBangGridLinesWithVthh,
} from '../../../utils/thuTienBangDonGiaBan'
import {
  thuTienBangGetAll,
  thuTienBangGetChiTiet,
  getDefaultThuTienBangFilter,
  type ThuTienBangCreatePayload,
...
import {
  ThuTienBangDinhKemModal,
  chuanHoaDuongDanDinhKemThuTienBang,
  partMccForPath,
  type ThuTienBangDinhKemPendingRow,
} from './thuTienBangDinhKemModal'
import { layLichSuThuTienChoKhach, coLichSuThuTien } from './lichSuThuTienKhach'
import {
  layChungTuConNoTheoKhach,
  demSoLanThuTruocDoChoMa,
  type ChungTuCongNoRow,
} from './chungTuCongNoKhach'
import {
  taiKhoanGetAll,
...
import { buildDienGiaiPhieuThu } from '../../../utils/dienGiaiPhieuThu'
import { getCongTyTaiKhoanNganHang } from '../../../utils/congTyTaiKhoanNganHang'
import { useThuTienBangApi } from './thuTienBangApiContext'
import { TINH_TRANG_NVTHH_DA_NHAP_KHO } from './thuTienBangApi'
...
import {
  loaiThuChiQueryKey,
  loaiThuChiQueryFn,
  loaiThuChiLyDoPhieuThuOptions,
  loaiThuChiChuanHoaLyDoPhieuThu,
  laLyDoPhieuThuKhachHang,
...
type ThuTienBangGridLineRow = DonHangMuaGridLineRow & {
  'mD'?: string
  'mR'?: string
  'Lượng'?: string
}
...
export interface ThuTienFormProps {
  onClose: () => void
  /** Sau khi lưu thành công; bản ghi mới/cập nhật (dùng liên kết Đơn hàng bán — YC62). */
  onSaved?: (savedRecord?: ThuTienBangRecord) => void
  ...
  initialDon?: ThuTienBangRecord | null
  initialChiTiet?: ThuTienBangChiTiet[] | null
  /** Dữ liệu đổ sẵn khi tạo mới (chuyển từ chứng từ khác), không phải chế độ xem */
  prefillDon?: Partial<ThuTienBangRecord> | null
  prefillChiTiet?: ThuTienBangChiTiet[] | null
  /** Sau khi bấm Lưu (chỉ lưu): chuyển form sang chế độ xem đơn vừa lưu, không đóng form */
  onSavedAndView?: (don: ThuTienBangRecord) => void
  formTitle?: string
  soDonLabel?: string
  // [ThuTienBang] Bỏ logic đối chiếu đơn mua - không áp dụng cho Báo giá
  ...
  phieuNhanTuThuTienBang?: boolean
...
function deriveApDungVatGtgtTuBanGhi(r: ThuTienBangRecord | Partial<ThuTienBangRecord> | null | undefined): boolean {
  ...
  const v = (r as ThuTienBangRecord).ap_dung_vat_gtgt
  if (typeof v === 'boolean') return v
  return true
}

/** [YC37] Một dòng trống theo mẫu — Lượng mặc định 1. */
function createEmptyThuTienBangLine(mau: 'codongia' | 'khongdongia'): ThuTienBangGridLineRow {
  ...
  return acc as unknown as ThuTienBangGridLineRow
}

/** Chi tiết BG → dòng lưới (SPHH + Đơn giá bán + kích thước). */
function chiTietToThuTienBangLines(ct: ThuTienBangChiTiet[]): ThuTienBangGridLineRow[] {
...
function deriveHinhThucSelectedIdsFromRecord(
  d: Partial<ThuTienBangRecord> & { hinh_thuc?: string; dia_chi_cong_trinh?: string } | null | undefined
): string[] {
  if (!d) return []
...
function thuTienBangListForDoiChieuNhanHang(): ThuTienBangRecord[] {
  const all = thuTienBangGetAll({ ...getDefaultThuTienBangFilter(), tu: '', den: '' })
  return all.filter((bg) => bg.tinh_trang !== 'Hủy bỏ')
}

/** Hiển thị ô «Nhận hàng từ»: `mã BG: tiêu đề` (tiêu đề = so_chung_tu_cukcuk). */
function formatNhanHangTuTuThuTienBang(bg: ThuTienBangRecord): string {
  const ma = (bg.so_thu_tien_bang ?? '').trim()
...
function thuTienBangListChoPhieuNhanTuThuTienBang(): ThuTienBangRecord[] {
  const all = thuTienBangGetAll({ ...getDefaultThuTienBangFilter(), tu: '', den: '' })
...
function mauKhongDonGiaLines(): ThuTienBangGridLineRow[] {
  return [createEmptyThuTienBangLine('khongdongia')]
}

/** Mẫu có đơn giá — [YC37] mặc định 1 dòng. */
function mauCoDonGiaLines(): ThuTienBangGridLineRow[] {
...
export function ThuTienForm({ onClose, onSaved, onHeaderPointerDown, headerDragStyle, dragging, readOnly = false, initialDon, initialChiTiet, prefillDon, prefillChiTiet, onMinimize, onMaximize, onSavedAndView: _onSavedAndView, formTitle: _formTitle, soDonLabel: soDonLabelProp, phieuNhanTuThuTienBang = false, phieuNhanThemMoiTuDanhSach = false, viewOnlyLocked = false, thuTienPhieu = false, phieuThuTuMenuBanHang = false }: ThuTienFormProps) {
  ...
  // [ThuTienBang] Bỏ logic đối chiếu - chỉ dùng phieuNhanTuThuTienBang
  const laPhieuNhanNvthh = Boolean(phieuNhanTuThuTienBang)
  ...
  const chiTietGridLocked = Boolean(phieuNhanTuThuTienBang) && !effectiveReadOnly && !phieuNhanThemMoiTuDanhSach
  ...
  const [attachments, setAttachments] = useState<ThuTienBangAttachmentItem[]>(() => {
  ...
  const [dktkPendingUploadRows, setDktkPendingUploadRows] = useState<ThuTienBangDinhKemPendingRow[]>([])
  const [phieuChiDktkPendingRows, setPhieuChiDktkPendingRows] = useState<ThuTienBangDinhKemPendingRow[]>([])
  const patchAttachmentsFromUser = useCallback((next: SetStateAction<ThuTienBangAttachmentItem[]>) => {
    setAttachments(next)
  ...
  const [hinhThucSelectedIds, setHinhThucSelectedIds] = useState<string[]>(() => {
    const fromRec = deriveHinhThucSelectedIdsFromRecord((initialDon ?? prefillDon) as ThuTienBangRecord | null | undefined)
    if (fromRec.length > 0) return fromRec
    if (phieuNhanTuThuTienBang && initialDon == null && prefillDon == null) return defaultPhieuNhanHinhThucNhapKhoIds()
    return fromRec
  })
    ...
    if (isViewMode && initialDon) {
      if (phieuNhanTuThuTienBang) {
        ...
        if (lk) {
          ...
          if (d) return formatNhanHangTuTuThuTienBang(d)
        }
        return (initialDon.so_chung_tu_cukcuk ?? '').toUpperCase()
      }
      return initialDon.so_chung_tu_cukcuk ?? ''
    }
    if (prefillDon && phieuNhanTuThuTienBang) {
      const pd = prefillDon as ThuTienBangRecord
      if ((pd.so_thu_tien_bang ?? '').trim()) return formatNhanHangTuTuThuTienBang(pd)
    }
    if (prefillDon?.so_chung_tu_cukcuk != null) {
      return phieuNhanTuThuTienBang
  ...
  const [ngayDonHang, setNgayDonHang] = useState<Date | null>(() => {
    if (isViewMode && initialDon) return parseIsoToDate(initialDon.ngay_thu_tien_bang)
    if (phieuNhanTuThuTienBang) return new Date()
    if (prefillDon?.ngay_thu_tien_bang) return parseIsoToDate(prefillDon.ngay_thu_tien_bang)
    return new Date()
  })
  const [soDonHang, setSoDonHang] = useState(() => (isViewMode && initialDon ? initialDon.so_thu_tien_bang : api.soDonHangTiepTheo()))
  ...
  const [ngayHachToan, setNgayHachToan] = useState<Date | null>(() => {
    ...
    const nd =
      isViewMode && initialDon
        ? parseIsoToDate(initialDon.ngay_thu_tien_bang)
        : prefillDon?.ngay_thu_tien_bang
          ? parseIsoToDate(prefillDon.ngay_thu_tien_bang)
          : new Date()
  ...
  const [apDungVatGtgt, setApDungVatGtgt] = useState(() => deriveApDungVatGtgtTuBanGhi((initialDon ?? prefillDon) as ThuTienBangRecord | null))
  ...
  const [lines, setLines] = useState<ThuTienBangGridLineRow[]>(() => {
    if (thuTienPhieu) return []
    if (isViewMode && initialChiTiet && initialChiTiet.length > 0) return chiTietToThuTienBangLines(initialChiTiet)
    if (prefillChiTiet && prefillChiTiet.length > 0) return chiTietToThuTienBangLines(prefillChiTiet)
  ...
  const [phieuThuChiTietLines, setPhieuThuChiTietLines] = useState<PhieuThuChiTietRow[]>(() => {
    if (thuTienPhieu && initialChiTiet && initialChiTiet.length > 0) {
      return parsePhieuThuFromChiTiet(initialChiTiet)
    }
    if (thuTienPhieu && prefillChiTiet && prefillChiTiet.length > 0) {
      return parsePhieuThuFromChiTiet(prefillChiTiet)
    }
    if (thuTienPhieu) return [emptyPhieuThuRow()]
    return []
  })
  ...
  const [lyDoThuPhieu, setLyDoThuPhieu] = useState(() =>
    loaiThuChiChuanHoaLyDoPhieuThu((initialDon ?? prefillDon)?.ly_do_thu_phieu),
  )
  const [thuTienMatPhieu, setThuTienMatPhieu] = useState(() => {
    const r = (initialDon ?? prefillDon) as ThuTienBangRecord | undefined
    if (typeof r?.thu_tien_mat === 'boolean') return r.thu_tien_mat
    return true
  })
  ...
  /** Chọn TK NH khi có nhiều dòng «Thu qua NH» trong module Tài khoản. */
  ...
  const khListPhieuThuLocCongNo = useMemo(() => {
    if (!thuTienPhieu || !laLyDoPhieuThuKhachHang(lyDoThuPhieu)) return khList
  ...
  const loaiKhachHangHienThi = useMemo((): 'ca_nhan' | 'doanh_nghiep' | null => {
    ...
    const saved = (initialDon ?? prefillDon) as ThuTienBangRecord | undefined
    if (saved?.loai_khach_hang === 'ca_nhan' || saved?.loai_khach_hang === 'doanh_nghiep') return saved.loai_khach_hang
    return null
  }, [khachHangId, khList, khachHangDisplay, initialDon?.loai_khach_hang, (prefillDon as ThuTienBangRecord | undefined)?.loai_khach_hang])
  ...
  // [ThuTienBang] Logic phiếu NVTHH - chỉ dùng khi phieuNhanTuThuTienBang = true
  ...
  const [phieuChiAttachments, setPhieuChiAttachments] = useState<ThuTienBangAttachmentItem[]>(() => {
    const r = (initialDon ?? prefillDon) as ThuTienBangRecord | undefined
    const a = r?.phieu_chi_attachments
    return Array.isArray(a) && a.length ? a.map((x) => ({ ...x })) : []
  })
  const patchPhieuChiAttachmentsFromUser = useCallback((next: SetStateAction<ThuTienBangAttachmentItem[]>) => {
  ...
  // [ThuTienBang] Bỏ logic đối chiếu đơn mua - không áp dụng cho Báo giá thông thường
  ...
  // const [viewThuTienBangRecord, setViewThuTienBangRecord] = useState<ThuTienBangRecord | null>(null)

  /** Phiếu NVTHH: bỏ margin dưới từng dòng — khoảng cách dọc = `gap` tab panel (cùng kho nhập → ĐĐGH 1). */
  const fieldRowDyn = useMemo(
    (): React.CSSProperties => ({ ...fieldRow, marginBottom: phieuNhanTuThuTienBang ? 0 : FIELD_ROW_GAP }),
    [phieuNhanTuThuTienBang]
  )
  ...
  const lyDoPhieuThuOptions = useMemo(
    () => loaiThuChiLyDoPhieuThuOptions(loaiThuChiRows),
    [loaiThuChiRows],
  )
  ...
  const soDonLabelHienThi = useMemo(() => {
    if (!phieuNhanTuThuTienBang) return soDonLabel
  ...
  }, [phieuNhanTuThuTienBang, phieuNhanTabChung, soDonLabel])
  ...
  const tieuDePhieuNvthhDayDu = useMemo(() => {
    if (!phieuNhanTuThuTienBang) return ''
    const chungTu =
  ...
  }, [phieuNhanTuThuTienBang, hinhThucSelectedIds, hinhThucList, chungTuMuaPttt, chungTuMuaLoaiHd, khachHangDisplay])
  ...
  }, [phieuNhanTuThuTienBang, thamChieu, phieuSoHoaDon])

  useEffect(() => {
    dienGiaiPhieuTuChinhRef.current = false
  }, [initialDon?.id, phieuNhanTuThuTienBang])

  useEffect(() => {
    if (!phieuNhanTuThuTienBang || initialDon != null) return
    if (dienGiaiPhieuTuChinhRef.current) return
    setDienGiai(dienGiaiPhieuMacDinh)
  }, [phieuNhanTuThuTienBang, initialDon, dienGiaiPhieuMacDinh])

  useEffect(() => {
    if (!phieuNhanTuThuTienBang || phieuNhanTabChung !== 'phieu-chi' || initialDon != null) return
    setPhieuChiKH(khachHangDisplay)
    setPhieuChiDiaChi(diaChi)
  }, [phieuNhanTuThuTienBang, phieuNhanTabChung, khachHangDisplay, diaChi, initialDon])
  ...
  }, [thuTienPhieu, ngayDonHang, initialDon?.id])

  useEffect(() => {
    if (!thuTienPhieu || !thuQuaNHPhieu) return
  ...
  }, [thuTienPhieu, thuQuaNHPhieu, tkThuQuaNhCandidates, tknhThuQuaNhId])

  useEffect(() => {
    if (!thuTienPhieu || effectiveReadOnly) return
    const kh = khachHangDisplay.trim()
    ...
    const lanTruoc = demSoLanThuTruocDoChoMa(kh, ma, exclude)
    const lanThu = lanTruoc + 1
    setDienGiai(
      buildDienGiaiPhieuThu({
        khachHang: kh,
        maDon: ma,
        soPhaiThu: soPhai,
        soChuaThuTruocLanNay: soChua,
        thuLanNay: thuNay,
        lanThu,
      }),
    )
  ...
  useEffect(() => {
    ...
    if (prefillDon) {
      ...
      if (phieuNhanTuThuTienBang) {
        ...
        if ((pd.so_thu_tien_bang ?? '').trim()) setThamChieu(formatNhanHangTuTuThuTienBang(pd))
      ...
      if (prefillDon.ngay_thu_tien_bang != null && (!phieuNhanTuThuTienBang || thuTienPhieu))
        setNgayDonHang(parseIsoToDate(prefillDon.ngay_thu_tien_bang))
      if (thuTienPhieu && prefillDon) {
        const ext = prefillDon as ThuTienBangRecord
        if (ext.ngay_hach_toan?.trim()) {
          const dh = parseYyyyMmDdToLocalDate(ext.ngay_hach_toan)
          if (dh) setNgayHachToan(dh)
        } else if (prefillDon.ngay_thu_tien_bang) {
          const d0 = parseIsoToDate(prefillDon.ngay_thu_tien_bang)
      ...
      // so_thu_tien_bang: luôn dùng số tự sinh của báo giá / phiếu nhận
    }
    ...
    if (prefillChiTiet && prefillChiTiet.length > 0) {
      if (thuTienPhieu) {
        ...
        setLines(chiTietToThuTienBangLines(prefillChiTiet))
      }
    }
  }, [isViewMode, prefillDon, prefillChiTiet, laPhieuNhanNvthh, phieuNhanTuThuTienBang, thuTienPhieu])
  ...
  const applyChonThuTienBangDoiChieu = useCallback(
    (bg: ThuTienBangRecord) => {
      const label = (bg.so_thu_tien_bang ?? '').trim()
      setThamChieu(phieuNhanTuThuTienBang ? formatNhanHangTuTuThuTienBang(bg) : label)
      // [ThuTienBang] Bỏ setSelectedDoiChieuDonMuaId
      ...
      setDiaChiCongTrinh((bg as ThuTienBangRecord & { dia_chi_cong_trinh?: string }).dia_chi_cong_trinh ?? '')
      ...
      const ct = thuTienBangGetChiTiet(bg.id)
      setLines(chiTietToThuTienBangLines(ct))
    },
    [phieuNhanTuThuTienBang, khList, chungTuMuaPttt],
  )

  useEffect(() => {
    if (!phieuNhanTuThuTienBang || chungTuMuaPttt !== 'chuyen_khoan' || effectiveReadOnly) return
  ...
  }, [phieuNhanTuThuTienBang, chungTuMuaPttt, khachHangId, khList, effectiveReadOnly])
  ...
  useEffect(() => {
    ...
    const so = (initialDon.so_thu_tien_bang ?? '').trim() || 'BG'
    const khPart = partMccForPath('')
    setAttachments(chuanHoaDuongDanDinhKemThuTienBang(raw, so, khPart))
    setAttachmentsDirty(false)
  }, [initialDon?.id])
  ...
  useEffect(() => {
    ...
    setAttachments((prev) => {
      ...
      const next = chuanHoaDuongDanDinhKemThuTienBang(prev, so, khPartDinhKem)
      const same = next.every(
        (n: ThuTienBangAttachmentItem, i: number) => n.virtual_path === prev[i]?.virtual_path && n.name === (prev[i]?.name ?? '')
      )
  ...
  }, [phieuNhanTuThuTienBang, soDonHang, khPartDinhKem])
  ...
  useEffect(() => {
    ...
    const d = initialDon as ThuTienBangRecord & { hinh_thuc?: string; dia_chi_cong_trinh?: string }
    ...
    if (phieuNhanTuThuTienBang && initialDon.doi_chieu_don_mua_id) {
      const linked = thuTienBangGetAll({ ...getDefaultThuTienBangFilter(), tu: '', den: '' }).find((x) => x.id === initialDon.doi_chieu_don_mua_id)
      setThamChieu(
        linked ? formatNhanHangTuTuThuTienBang(linked) : (initialDon.so_chung_tu_cukcuk ?? '').toUpperCase(),
      )
    } else {
      setThamChieu(initialDon.so_chung_tu_cukcuk ?? '')
    }
    setNgayDonHang(parseIsoToDate(initialDon.ngay_thu_tien_bang))
    if (thuTienPhieu) {
      const isoHt = (initialDon as ThuTienBangRecord).ngay_hach_toan?.trim()
      if (isoHt) {
        const dh = parseYyyyMmDdToLocalDate(isoHt)
        if (dh) setNgayHachToan(dh)
      } else {
        const d0 = parseIsoToDate(initialDon.ngay_thu_tien_bang)
        if (d0) setNgayHachToan(new Date(d0.getFullYear(), d0.getMonth(), d0.getDate()))
      }
    }
    setSoDonHang(initialDon.so_thu_tien_bang)
    ...
    if (initialChiTiet && initialChiTiet.length > 0) {
      if (thuTienPhieu) setPhieuThuChiTietLines(parsePhieuThuFromChiTiet(initialChiTiet))
      else setLines(chiTietToThuTienBangLines(initialChiTiet))
    } else {
      if (!thuTienPhieu) setLines([])
    }
    ...
    if (rawAtt.length === 0) setAttachments([])
      ...
      const so = (initialDon.so_thu_tien_bang ?? '').trim() || 'BG'
      const khPart = partMccForPath('')
      setAttachments(chuanHoaDuongDanDinhKemThuTienBang(rawAtt, so, khPart))
    }
    setAttachmentsDirty(false)
  ...
  }, [thuTienPhieu, initialDon?.id, initialDon?.phieu_tai_khoan_id])
  ...
  useEffect(() => {
    ...
    setLines((prev) => enrichThuTienBangGridLinesWithVthh(prev, vatTuList))
  }, [vatTuList, prefillChiTiet])

  // [ThuTienBang] Bỏ logic đối chiếu - không cần enrich từ selectedDoiChieuDonMuaId
  ...
  const calculateSoLuongFromKichThuoc = (line: ThuTienBangGridLineRow): string => {
  ...
  const syncDonGiaTheoBacVaSl = (row: ThuTienBangGridLineRow): ThuTienBangGridLineRow => {
    ...
    return { ...row, 'Đơn giá': getDonGiaBanThuTienBangLine(row._vthh, dvt, sl) } as unknown as ThuTienBangGridLineRow
  }
  ...
  const handleChonVthh = (vthh: VatTuHangHoaRecord, rowIndex: number) => {
    ...
    if (isCodongia) {
      ...
      const sl = Math.max(0, parseFloatVN(row['Số lượng'] ?? '')) || 1
      row['Đơn giá'] = getDonGiaBanThuTienBangLine(vthh, (row['ĐVT'] ?? '').trim() || (vthh.dvt_chinh ?? ''), sl)
    }
  ...
  const buildPayload = (): ThuTienBangCreatePayload => {
    ...
    const tienCkTinh = thuTienPhieu ? 0 : Math.max(0, parseFloatVN(tienCkInput || '0'))
      ...
      phieuNhanTuThuTienBang
        ...
        : thuTienPhieu
          ...
          : ngayGiaoHang
            ...
            : null
    const payload: ThuTienBangCreatePayload = {
      ...
      ngay_thu_tien_bang: toIsoDate(ngayDonHang) || toIsoDate(new Date()),
      so_thu_tien_bang: soDonHang.trim() || 'BG',
      ...
      ap_dung_vat_gtgt: thuTienPhieu ? false : apVat,
      tl_ck: thuTienPhieu
        ? undefined
        : (() => {
      ...
      tien_ck: thuTienPhieu ? undefined : tienCkTinh > 0 ? tienCkTinh : undefined,
      ly_do_thu_phieu: thuTienPhieu ? lyDoThuPhieu : undefined,
      thu_tien_mat: thuTienPhieu ? thuTienMatPhieu : undefined,
      thu_qua_ngan_hang: thuTienPhieu ? thuQuaNHPhieu : undefined,
      ngay_hach_toan: thuTienPhieu && ngayHachToan ? toIsoDate(ngayHachToan) : undefined,
      phieu_tai_khoan_id: (() => {
        if (!thuTienPhieu) return undefined
        if (thuQuaNHPhieu) {
      ...
      so_chung_tu_cukcuk: thuTienPhieu ? '' : thamChieu.trim(),
      // [ThuTienBang] Bỏ logic đối chiếu đơn mua
      ...
      attachments:
        attachments.length > 0
          ? chuanHoaDuongDanDinhKemThuTienBang(attachments, soDonHang.trim() || 'DHM', khPartDinhKem)
          : undefined,
      ...(phieuNhanTuThuTienBang
        ? {
            ...
            phieu_chi_attachments:
              phieuChiAttachments.length > 0
                ? chuanHoaDuongDanDinhKemThuTienBang(phieuChiAttachments, soDonHang.trim() || 'DHM', khPartDinhKem)
                : undefined,
          }
        : {}),
    }
  ...
  const validateBeforeSave = (): boolean => {
    ...
    if (!thuTienPhieu && !thamChieu.trim()) {
      return fail(
        'doi_chieu',
        laPhieuNhanNvthh
          ? phieuNhanTuThuTienBang
    ...
    const tgGiaoHangTenLoi =
      phieuNhanTuThuTienBang && phieuNhanTabChung === 'phieu-chi' ? 'TG chi' : 'TG nhận hàng'
    if (phieuNhanTuThuTienBang) {
      if (!ngayGiaoHang) {
    ...
    } else if (!thuTienPhieu && !ngayGiaoHang) {
      return fail(
        'tg_nhan',
    ...
    if (!phieuNhanTuThuTienBang && !thuTienPhieu && ngayGiaoHang && ngayDonHang) {
  ...
  const handleLuuVaDong = async () => {
    ...
    try {
      ...
      let savedRecord: ThuTienBangRecord | undefined
      if (initialDon) {
        api.put(initialDon.id, payload)
        savedRecord = thuTienBangGetAll({ ...getDefaultThuTienBangFilter(), tu: '', den: '' }).find((x) => x.id === initialDon.id)
      } else {
  ...
  const phieuNhanLienKetTuDonMua = useMemo(() => {
    if (!initialDon?.id || phieuNhanTuThuTienBang) return []
  ...
  const lichSuData = useMemo(() => {
    if (phieuNhanTuThuTienBang) return null
    return layLichSuThuTienChoKhach(khachHangDisplay, { excludeThuTienBangId: initialDon?.id })
  }, [khachHangDisplay, initialDon?.id, phieuNhanTuThuTienBang])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, background: 'var(--bg-primary)' }}>
        ...
        >
          {phieuNhanTuThuTienBang
            ...
            : (() => {
                ...
                if (thuTienPhieu) return `${mode} PHIẾU THU TIỀN${khPart}`
      ...
      <div style={toolbarWrap}>
        ...
        >
          <button type="button" style={toolbarBtn} disabled={effectiveReadOnly || initialDon != null || phieuNhanTuThuTienBang}>
            <FileText size={14} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>Mẫu <ChevronDown size={10} /></span>
          </button>
          {dropdownMau && !effectiveReadOnly && initialDon == null && !phieuNhanTuThuTienBang && (
                  ...
                  onMouseDown={(e) => {
                    ...
                    if (key === 'codongia') {
                      if (mauHienTai === 'khongdongia') {
                        setLines((prev) => (prev.length > 0 ? migrateThuTienBangLinesToCoDonGia(prev, vatTuList) : mauCoDonGiaLines()))
                      } else {
      ...
      {/* [ThuTienBang] Bỏ modal chọn báo giá đối chiếu - không áp dụng cho Báo giá */}
      {false && (
        <Modal
          open={false}
          onClose={() => {}}
        ...
        >
          <div style={{ maxHeight: 320, overflowY: 'auto' }}>
            {(() => {
              const bgChonTuDonList = thuTienBangListForDoiChieuNhanHang()
              return (
                <>
                  {bgChonTuDonList.map((bg) => {
                    const ngayDdMm = formatIsoToDdMmYyyy(bg.ngay_thu_tien_bang)
                    return (
                      <div
                        key={bg.id}
                        role="button"
                        ...
                        onMouseDown={(e) => {
                          e.preventDefault()
                          applyChonThuTienBangDoiChieu(bg)
                          // [ThuTienBang] Bỏ setShowTimDonMuaHangPopup
                        }}
                      >
                        <span style={{ fontWeight: 600 }}>{bg.so_thu_tien_bang}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1 1 0', minHeight: 0, gap: 2, marginBottom: 2 }}>
          ...
          {(() => {
            ...
            const thongTinChungFields = (
              <>
            {!thuTienPhieu && (!effectiveReadOnly || phieuNhanTuThuTienBang || (thamChieu ?? '').trim() !== '') && (
            <div style={{ ...fieldRowDyn, alignItems: 'center' }}>
              <label style={labelStyle}>{phieuNhanTuThuTienBang ? 'Nhận hàng từ:' : 'Tên báo giá'}</label>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center' }}>
                {effectiveReadOnly ? (
                  ...
                  >
                    {/* [ThuTienBang] Bỏ logic link xem báo giá đối chiếu */}
                    {false ? null : (
                      <span
                        style={{
                          color: 'var(--text-muted)',
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0, position: 'relative', display: 'flex', height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT }}>
                      <input
                        ...
                        onFocus={() => {
                          // [ThuTienBang] Bỏ logic đối chiếu - không mở dropdown tự động
                        }}
                        onClick={() => {
                          // [ThuTienBang] Bỏ logic đối chiếu
                        }}
                        readOnly={false}
                        disabled={false}
                        placeholder={
                          phieuNhanTuThuTienBang ? 'Bấm để chọn đơn Chưa thực hiện…' : 'Nhập tên báo giá'
                        }
                      />
                    </div>
                  </div>
                )}
                {hienThiFooterTongTien && !thuTienPhieu && (
            ...
            {/* [ThuTienBang] Bỏ logic dropdown đối chiếu đơn mua */}
            {false
              ? ReactDOM.createPortal(
                  (() => {
                    const q = thamChieu.trim().toLowerCase()
                    const listNvthhNhanTu = thuTienBangListChoPhieuNhanTuThuTienBang().filter((bg) => {
                      if (!q) return true
                      const ma = (bg.so_thu_tien_bang ?? '').toLowerCase()
                ...
                <div style={{ ...fieldRowDyn, alignItems: 'center', width: '100%', minWidth: 0, flexWrap: 'wrap', gap: '6px 10px' }}>
                  ...
                  <div style={{ flex: '0 1 200px', minWidth: 140, maxWidth: 280, position: 'relative', display: 'flex', height: LOOKUP_CONTROL_HEIGHT, minHeight: LOOKUP_CONTROL_HEIGHT }}>
                    <select
                      ...
                      value={
                        lyDoPhieuThuOptions.length === 0
                          ? lyDoThuPhieu
                          : lyDoPhieuThuOptions.includes(lyDoThuPhieu)
                            ? lyDoThuPhieu
                            : (lyDoPhieuThuOptions[0] ?? '')
                      }
                      onChange={(e) => setLyDoThuPhieu(e.target.value)}
                      disabled={effectiveReadOnly}
                    >
                      {lyDoPhieuThuOptions.map((o) => (
                ...
                >
                  ...
                  {khListPhieuThuLocCongNo.filter((kh) => matchSearchKeyword(`${kh.ma_kh} ${kh.ten_kh}`, khSearchKeyword)).length === 0 && (
                    <div style={{ padding: '10px', fontSize: 11, color: 'var(--text-muted)' }}>
                      {thuTienPhieu && laLyDoPhieuThuKhachHang(lyDoThuPhieu)
            ...
            {!thuTienPhieu && !phieuNhanTuThuTienBang && laToChucKh ? (
              <div style={{ ...fieldRowDyn, alignItems: 'center', width: '100%', minWidth: 0 }}>
            ...
            {thuTienPhieu && !laPhieuNhanNvthh && !laCaNhanKh && daChonKhachHangPhieu && (
              <div style={{ ...fieldRowDyn, alignItems: 'center', width: '100%', minWidth: 0 }}>
                <label style={labelStyle}>Người nộp</label>
                <input
                  style={{ ...inputStyle, flex: 1, minWidth: 0 }}
                  value={tenNguoiLienHe}
                  onChange={(e) => setTenNguoiLienHe(e.target.value)}
              ...
              <>
                {thuQuaNHPhieu && (
                  <div style={{ ...fieldRowDyn, alignItems: 'center', width: '100%', minWidth: 0, flexWrap: 'wrap', gap: 8 }}>
                    <label style={labelStyle}>Nộp vào TK</label>
            ...
            {!thuTienPhieu && (!phieuNhanTuThuTienBang || chungTuMuaCachThanhToan === 'chua_thanh_toan') && (
            ...
            return phieuNhanTuThuTienBang ? (
              <div style={{ flex: '1 1 320px', minWidth: 280, display: 'flex', flexDirection: 'column', alignSelf: 'stretch', marginBottom: 0, minHeight: 0 }}>
                <div className={thuTienBangDetailStyles.detailTabBar} role="tablist" aria-label="Thông tin chứng từ">
                  <button
                    type="button"
                    role="tab"
                    aria-selected={phieuNhanTabChung === 'phieu-nhap'}
                    className={phieuNhanTabChung === 'phieu-nhap' ? thuTienBangDetailStyles.detailTabActive : thuTienBangDetailStyles.detailTab}
                  ...
                  <button
                    ...
                    className={phieuNhanTabChung === 'phieu-chi' ? thuTienBangDetailStyles.detailTabActive : thuTienBangDetailStyles.detailTab}
                    style={phieuTabBarBtnCompact}
                  ...
                  <button
                    ...
                    className={phieuNhanTabChung === 'hoa-don' ? thuTienBangDetailStyles.detailTabActive : thuTienBangDetailStyles.detailTab}
                    style={phieuTabBarBtnCompact}
                ...
                <div
                  className={thuTienBangDetailStyles.detailTabPanel}
                  style={{
                    flex: 1,
                    minHeight: 0,
                    display: 'flex',
          ...
          <div
            style={{
              ...groupBox,
              width: phieuNhanTuThuTienBang ? 266 : 268,
              minWidth: phieuNhanTuThuTienBang ? 266 : 268,
              marginBottom: 0,
              flexShrink: 0,
              ...(phieuNhanTuThuTienBang ? { padding: '6px 8px', gap: FIELD_ROW_GAP } : {}),
              display: 'flex',
              flexDirection: 'column',
              alignSelf: 'stretch',
              minHeight: 0,
          ...
          >
            <div style={{ ...groupBoxTitle, ...(phieuNhanTuThuTienBang ? { marginBottom: 4 } : {}) }}>
              {phieuNhanTuThuTienBang ? (phieuNhanTabChung === 'hoa-don' ? 'Hóa đơn' : 'Chứng từ') : 'Thu tiền'}
            </div>
            {phieuNhanTuThuTienBang && phieuNhanTabChung === 'hoa-don' ? (
              <>
                <div style={fieldRowDyn}>
              ...
              <>
                {phieuNhanTuThuTienBang ? (
                  ...
                  <>
                    ...
                    <div style={fieldRowDyn}>
                      <label style={{ ...labelStyle, minWidth: LABEL_DON_HANG_BOX }}>Ngày thu</label>
                      <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={fieldRowDyn}>
              <label style={{ ...labelStyle, minWidth: LABEL_DON_HANG_BOX }}>{thuTienPhieu ? 'Người thu' : 'NV bán hàng'}</label>
              <DonHangBoxValueLikeNvMuaHang
                readOnly={effectiveReadOnly}
                fixedBandWidth={phieuNhanTuThuTienBang ? NVTHH_DON_HANG_BOX_VALUE_BAND_PX : undefined}
                trailingSlot={
                  !effectiveReadOnly ? (
            ...
            {showLienQuanPhieuNhanHang && (
              <>
                <div className={thuTienBangDetailStyles.lienQuanPhieuNhanTitle}>Liên quan</div>
                    ...
                    phieuNhanLienKetTuDonMua.map((p) => {
                      ...
                      return (
                        <div key={p.id} style={{ marginBottom: 6, fontSize: 11, textAlign: 'left', lineHeight: 1.35 }}>
                          ...
                          <button
                            type="button"
                            className={thuTienBangDetailStyles.lienQuanPhieuNhanLink}
        ...
        >
          ...
          >
                            ...
                            <th
                              ...
                              style={{
                                ...
                                textAlign:
                                  h === 'STT'
                                    ? 'center'
                                    : ['Số phải thu', 'Số đã thu', 'Còn lại'].includes(h)
                                      ? 'right'
                                      : h === ''
                                        ? 'center'
                                        : 'left',
                      ...
                      <th
                        ...
                        style={{
                          ...
                          textAlign:
                            h === 'STT'
                              ? 'center'
                              : ['Số phải thu', 'Số chưa thu', 'Thu lần này', 'Công nợ theo lần thu này'].includes(h)
                                ? 'right'
                                : 'left',
                          paddingLeft: 5,
                          paddingRight: 6,
            ...
            >
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 11, tableLayout: 'auto', minWidth: tableMinWidth }}>
                ...
                <tbody>
                  {lines.length === 0 ? (
                    ...
                    lines.map((line, idx) => (
                      <tr key={idx} className={thuTienBangChiTietStyles.chiTietDataRow}>
                        <td
                          style={{
                            ...gridTdStyle,
                          ...
                          >
                            {showColAsReadOnly ? (
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (cleared || !item) {
                                      ...
                                      if (isCodongia) {
                                        ...
                                        nextRow['Đơn giá'] = getDonGiaBanThuTienBangLine(item, (nextRow['ĐVT'] ?? '').trim() || (item.dvt_chinh ?? ''), sl)
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'Nội dung': e.target.value } as unknown as ThuTienBangGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'ĐVT' ? (
                                ...
                                if (hasDvtDropdown) {
                                  return (
                                    <select
                                      ...
                                      onChange={(e) => {
                                        const r = [...lines]
                                        const row = r[idx] as ThuTienBangGridLineRow
                                        const newDvt = e.target.value
                                        const updates = { ...row, 'ĐVT': newDvt } as unknown as ThuTienBangGridLineRow
                                        ...
                                        if ((initialDon != null || mauHienTai === 'codongia') && row._vthh) {
                                          ...
                                          updates['Đơn giá'] = getDonGiaBanThuTienBangLine(row._vthh, newDvt, sl)
                                        }
                                        r[idx] = updates
                                        setLines(r)
                                      }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updated = { ...r[idx], 'mD': raw } as unknown as ThuTienBangGridLineRow
                                ...
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mD': '' } as unknown as ThuTienBangGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const raw = formatSoTien(normalizeKichThuocInput(e.target.value))
                                  const updated = { ...r[idx], 'mR': raw } as unknown as ThuTienBangGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  if (n <= 0) {
                                    ...
                                    const r = [...lines]
                                    let row = { ...r[idx], 'mR': '' } as unknown as ThuTienBangGridLineRow
                                    row['Số lượng'] = calculateSoLuongFromKichThuoc(row)
                                    row = syncDonGiaTheoBacVaSl(row)
                                    r[idx] = row
                                    setLines(r)
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(e.target.value) } as unknown as ThuTienBangGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                                onBlur={() => {
                                  ...
                                  const updated = { ...r[idx], 'Lượng': formatSoNguyenInput(String(n)) } as unknown as ThuTienBangGridLineRow
                                  updated['Số lượng'] = calculateSoLuongFromKichThuoc(updated)
                                  r[idx] = syncDonGiaTheoBacVaSl(updated)
                                  setLines(r)
                                }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  let row = { ...r[idx], [col]: val } as unknown as ThuTienBangGridLineRow
                                  if (row._vthh && (initialDon != null || mauHienTai === 'codongia')) {
                                    const sl = Math.max(0, parseFloatVN(val)) || 1
                                    row['Đơn giá'] = getDonGiaBanThuTienBangLine(
                                      row._vthh,
                                ...
                                onBlur={() => {
                                  ...
                                  const r = [...lines]
                                  let next = { ...r[idx], [col]: formatSoTienHienThi(val) } as unknown as ThuTienBangGridLineRow
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], 'Đơn giá': formatSoTien(e.target.value) } as unknown as ThuTienBangGridLineRow
                                  setLines(r)
                                }}
                              />
                            ) : col === 'Thành tiền' ? (
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const r = [...lines]
                                  r[idx] = { ...r[idx], '% thuế GTGT': e.target.value } as unknown as ThuTienBangGridLineRow
                                  setLines(r)
                                }}
                              >
                              ...
                              <input
                                ...
                                style={{ ...inputStyle, width: '100%', border: '1px solid transparent', minHeight: 22, height: 22 }}
                                value={line[col] ?? ''}
                                onChange={(e) => { const r = [...lines]; r[idx] = { ...r[idx], [col]: e.target.value } as unknown as ThuTienBangGridLineRow; setLines(r) }}
                              />
                            )}
                          </td>
                        )
              ...
              {!chiTietReadOnly && !chiTietGridLocked && !thuTienPhieu && (
                  ...
                  <div style={{ marginLeft: OFFSET_TRAI_COT_MA_SPHH, display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setLines([
                          ...lines,
                          createEmptyThuTienBangLine(mauHienTai === 'codongia' ? 'codongia' : 'khongdongia'),
                        ])
                      }}
        ...
        {showLichSuPanel && lichSuData && (
          ...
          >
            ...
            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '6px 8px', fontSize: 10 }}>
              {([
                { key: 'bg', title: 'Thu tiền', list: lichSuData.thuTienBang },
      ...
      <ThuTienBangDinhKemModal
        ...
        readOnly={effectiveReadOnly}
        soThuTienBang={soDonHang}
        maKhPathPart={khPartDinhKem}
        ngayGiaoHang={ngayGiaoHang}
        ngayThuTienBang={ngayDonHang}
