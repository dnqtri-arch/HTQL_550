...
export function parseSoThuTienBangForAttachmentFile(soDon: string): string {
  const t = soDon.trim()
  let m = t.match(/^(\d{4})\s*\/\s*TT\s*\/\s*(\d+)$/i)
...
export function buildBgAttachmentBaseName(
  ...
  maKhPathPart: string,
  ngayGiao: Date | null,
  ngayThuTienBang: Date | null,
  index: number
): string {
  const bgPart = parseSoThuTienBangForAttachmentFile(soDon)
  const khPart = (maKhPathPart || 'kh_unknown').trim().toLowerCase()
  const tgNgay = ngayGiao ?? ngayThuTienBang ?? new Date()
...
export function buildBgAttachmentFullVirtualPath(soThuTienBang: string, maKhPathPart: string, fileName: string): string {
  ...
  return `${buildBgAttachmentFolderVirtualPath(soThuTienBang, maKhPathPart)}/${base}`
}
...
export function rebuildBgAttachmentStoredFileName(
  ...
  const index = m[6]
  const bgPart = parseSoThuTienBangForAttachmentFile(soThuTienBang)
  const khPart = (maKhPathPart || 'kh_unknown').trim().toLowerCase()
 ...
 */
export function chuanHoaDuongDanDinhKemThuTienBang(
  items: ThuTienBangAttachmentItem[],
  soThuTienBang: string,
  maKhPathPart: string
): ThuTienBangAttachmentItem[] {
  return items.map((a) => {
    const name = rebuildBgAttachmentStoredFileName(a.name, soThuTienBang, maKhPathPart)
    return {
      ...a,
      name,
      virtual_path: buildBgAttachmentFullVirtualPath(soThuTienBang, maKhPathPart, name),
    }
  })
}
...
export function duongDanHienThi(item: ThuTienBangAttachmentItem, soThuTienBang: string, maKhPathPart: string): string {
  const full = buildBgAttachmentFullVirtualPath(soThuTienBang, maKhPathPart, item.name)
  const thuMuc = duongDanThuMucLuuTru(full)
  if (thuMuc) return thuMuc
  return `${buildBgAttachmentFolderVirtualPath(soThuTienBang, maKhPathPart)}/`
}
...
export type ThuTienBangDinhKemPendingRow = {
  id: string
  ten: string
  sizeBytes: number
  phanTram: number
...
export type ThuTienBangDinhKemModalProps = {
  open: boolean
  onClose: () => void
  /** Neo panel ngay dưới nút Đính kèm trên toolbar */
  anchorRef: React.RefObject<HTMLElement | null>
  attachments: ThuTienBangAttachmentItem[]
  onChange: (next: ThuTienBangAttachmentItem[]) => void
  readOnly?: boolean
  soThuTienBang: string
   ...
   */
  pendingUploadRows?: ThuTienBangDinhKemPendingRow[]
  onPendingUploadRowsChange?: React.Dispatch<React.SetStateAction<ThuTienBangDinhKemPendingRow[]>>
}
...
export function ThuTienBangDinhKemModal({
  open,
  onClose,
  anchorRef,
  attachments,
  ...
  const [pendingTienTrinhLocal, setPendingTienTrinhLocal] = useState<ThuTienBangDinhKemPendingRow[]>([])
  ...
  const xuLyTepTuMang = useCallback(
    async (list: File[]) => {
      ...
      const hangMoi: ThuTienBangAttachmentItem[] = []
      ...
      for (let i = 0; i < listXuLy.length; i++) {
        ...
        const tenGoc = buildBgAttachmentBaseName(soThuTienBang, maKhPathPart, ngayGiaoHang, ngayThuTienBang, chiSo)
      ...
      const withMeta: ThuTienBangAttachmentItem[] = hangMoi.map((x) => ({
        ...x,
        saved_at: now,
        virtual_path: buildBgAttachmentFullVirtualPath(soThuTienBang, maKhPathPart, x.name),
      }))
      onChange([...attachments, ...withMeta])
    },
    [attachments, onChange, readOnly, soThuTienBang, maKhPathPart, ngayGiaoHang, ngayThuTienBang, baoLoiDinhKem, xacNhanDefer]
  )
        ...
        {attachments.length === 0 && pendingTienTrinh.length === 0 ? (
          ...
          >
            {attachments.map((item, idx) => {
              ...
              const duongDan = duongDanHienThi(item, soThuTienBang, maKhPathPart)
