...
import { chiTienBangGetAll, chiTienBangGetChiTiet } from './chiTienBangApi'
...
const BAO_GIA_ALL: ChiTienBangFilter = { ky: 'tat-ca', tu: '', den: '' }
...
export type LichSuBanHangNhom = {
  ChiTienBang: LichSuBanHangMuc[]
  donHangBan: LichSuBanHangMuc[]
  hopDong: LichSuBanHangMuc[]
}

/** Tối đa 3 giao dịch gần nhất mỗi loại; có thể loại báo giá đang sửa. */
export function layLichSuChiTienChoKhach(
  khachHangTen: string,
  options?: { excludeChiTienBangId?: string; excludeDonHangBanId?: string }
): LichSuBanHangNhom | null {
  const t = khachHangTen.trim()
  if (!t) return null

  const allBg = chiTienBangGetAll(BAO_GIA_ALL)
  const ChiTienBang = allBg
    .filter((r: ChiTienBangRecord) => khachKhop(r.khach_hang, t) && r.id !== options?.excludeChiTienBangId)
    .sort((a: ChiTienBangRecord, b: ChiTienBangRecord) =>
      (b.ngay_chi_tien_bang ?? '').localeCompare(a.ngay_chi_tien_bang ?? ''),
    )
    .slice(0, 3)
    .map((r: ChiTienBangRecord) => {
      const ct = chiTienBangGetChiTiet(r.id)
      return {
        id: r.id,
        tenHienThi: r.so_chi_tien_bang,
        tongThanhToan: r.tong_thanh_toan,
        chiTiet: ct.map((c: ChiTienBangChiTiet) => ({
          ten_hang: c.ten_hang,
          dvt: c.dvt,
          so_luong: c.so_luong,
          don_gia: c.don_gia,
  ...
  return { ChiTienBang, donHangBan, hopDong }
}

export function coLichSuChiTien(n: LichSuBanHangNhom | null): boolean {
  if (!n) return false
  return n.ChiTienBang.length > 0 || n.donHangBan.length > 0 || n.hopDong.length > 0
}
