/**
 * Chứng từ Đơn hàng bán / Hợp đồng bán còn công nợ (còn lại > 0) theo tên khách — phục vụ phiếu chi tiền.
 */
...
function isPhieuChiCongNo(r: ChiTienBangRecord): boolean {
  return r.ly_do_chi_phieu === 'chi_nha_cung_cap' || r.ly_do_chi_phieu === 'chi_khac'
}

function tienThuChoMaChungTu(ct: ChiTienBangChiTiet[], maChungTu: string): number {
...
export function layChungTuConNoTheoKhach(
  ...
  const allTtb = chiTienBangGetAll({ ky: 'tat-ca', tu: '', den: '' })
  const excludeId = options?.excludeChiTienBangId

  /** Tổng tiền đã lập trên phiếu thu (mọi trạng thái ghi sổ). */
  function tongLapChoMa(ma: string): number {
    let sum = 0
    for (const r of allTtb) {
      if (!isPhieuChiCongNo(r)) continue
...
export function demSoLanChiTruocDoChoMa(
  khachHangTen: string,
  maChungTu: string,
  excludeChiTienBangId?: string,
): number {
  ...
  for (const r of allTtb) {
    if (!isPhieuChiCongNo(r)) continue
    if (excludeChiTienBangId && r.id === excludeChiTienBangId) continue
    if (!khachKhop(r.khach_hang ?? '', t)) continue
    const ct = chiTienBangGetChiTiet(r.id)
...
export function tinhDaThuVaConLaiChoDonHangBan(row: DonHangBanChungTuRecord): {
  ...
  for (const r of allTtb) {
    ...
    tongLap += t
    if (daGhiSoPhieuChi(r.id)) daGhiSo += t
  }
