/**
 * Map Báo giá → prefill form Đơn hàng bán (ChungTu) — module tách biệt, không import ngược từ ChiTienBangForm.
 */
...
function mapAttachments(att: ChiTienBangRecord['attachments']): DonHangBanChungTuRecord['attachments'] {
  if (!att?.length) return undefined
  return att.map((a) => ({ ...a }))
}

/** Chuẩn bị header + chi tiết để mở form ĐHB mới từ một báo giá (đủ trường đồng dạng BG). */
export function buildDonHangBanChungTuPrefillFromChiTienBang(
  ...
  const prefillDhb: Partial<DonHangBanChungTuRecord> = {
    ...
    ngay_don_hang: row.ngay_chi_tien_bang,
