/**
 * Map Báo giá → prefill form Hợp đồng bán (ChungTu) — module tách biệt, không import ngược từ ChiTienBangForm.
 */
...
const PREFILL_DON_HANG_BAN_ID = '__prefill_chi_tien_bang__'

function mapAttachments(att: ChiTienBangRecord['attachments']): HopDongBanChungTuRecord['attachments'] {
  if (!att?.length) return undefined
  return att.map((a) => ({ ...a }))
}

/** Chuẩn bị header + chi tiết để mở form ĐHB mới từ một báo giá (đủ trường đồng dạng BG). */
export function buildHopDongBanChungTuPrefillFromChiTienBang(
  row: ChiTienBangRecord,
  chiTiet: ChiTienBangChiTiet[],
  soHopDongTiepTheo: string,
): {
  prefillHdbCt: Partial<HopDongBanChungTuRecord>
  prefillChiTiet: HopDongBanChungTuChiTiet[]
} {
  const prefillHdbCt: Partial<HopDongBanChungTuRecord> = {
    ...
    ngay_lap_hop_dong: row.ngay_chi_tien_bang,
    so_hop_dong: soHopDongTiepTheo,
    ngay_cam_ket_giao: row.ngay_giao_hang,
    ...
    bao_gia_id: row.id,
    so_bao_gia_goc: row.so_chi_tien_bang,
  ...
  const prefillChiTiet: HopDongBanChungTuChiTiet[] = chiTiet.map((c, i) => ({
    id: `prefill-ct-${i}`,
    hop_dong_ban_chung_tu_id: PREFILL_DON_HANG_BAN_ID,
    stt: c.stt,
    ma_hang: c.ma_hang,
    ten_hang: c.ten_hang,
...
export function buildHopDongBanChungTuPrefillCloneFromSaved(
  row: HopDongBanChungTuRecord,
  chiTiet: HopDongBanChungTuChiTiet[],
  soHopDongTiepTheo: string,
): {
  prefillHdbCt: Partial<HopDongBanChungTuRecord>
  prefillChiTiet: HopDongBanChungTuChiTiet[]
  ...
  const prefillHdbCt: Partial<HopDongBanChungTuRecord> = { ...rest, so_hop_dong: soHopDongTiepTheo }
  const prefillChiTiet: HopDongBanChungTuChiTiet[] = chiTiet.map((c, i) => ({
    ...c,
    id: `prefill-clone-${i}`,
    hop_dong_ban_chung_tu_id: PREFILL_DON_HANG_BAN_ID,
